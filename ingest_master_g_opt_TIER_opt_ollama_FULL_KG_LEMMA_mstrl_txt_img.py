...
OLLAMA_TIMEOUT_S = int(os.getenv("OLLAMA_TIMEOUT_S", "300")) #180
...
MATH_BROAD_PAT = re.compile(
    r"(?i)("
    r"formula|equation|equazione|modello|model|theorem|teorema|"  # Keyword generiche
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Estrazione Universale (PDF):
    1. Testo Nativo (veloce).
    2. Immagini Embedded (grafici raster).
    3. Matematica/Schemi (rilevamento ibrido Testo+Vettori -> Visione).
    """
    
    # 1. Estrazione del testo nativo (base per il KG e ricerca testuale)
    ...
    # Helper per contare elementi vettoriali (linee, curve, path)
    def _count_vectors(p: fitz.Page) -> int:
        try:
            ops = 0
            # get_drawings restituisce i percorsi vettoriali (linee frazione, radici, assi grafici)
    ...
    try:
        ...
        for i, page_text in enumerate(native_text_pages):
            ...
            # A) CHUNK TESTO BASE
            # ---------------------------------------------------------
            # Salviamo sempre il testo, anche se la pagina ha formule.
            # Il testo serve per il contesto del chunk.
            ...
            if i < len(doc):
                ...
                # ---------------------------------------------------------
                # Queste sono immagini "incollate" nel PDF (JPG/PNG).
                        ...
                        with t_conn.cursor() as t_cur:
                            for img_idx, img in enumerate(page_objs):
                                ...
                                try:
                                    ...
                                    # Filtro dimensioni minime (evita icone)
                                    ...
                                    # Vision Analysis (Grafici/Tabelle)
                                    analysis = None
                ...
                # ---------------------------------------------------------
                # Logica Generalista:
                # Se c'è testo matematico O ci sono vettori (linee, grafici vettoriali),
                # scatta una foto ad alta risoluzione e chiedi al modello Vision.
                
                has_math_text = bool(MATH_BROAD_PAT.search(page_text))
                vector_count = _count_vectors(page)
                
                # Soglia vettoriale bassa (10) per catturare anche semplici linee di frazione
                has_vectors = vector_count > 10 

                if PDF_VISION_ENABLED and (has_math_text or has_vectors):
                    try:
                        # 1. Render ad alta risoluzione (300 DPI) per catturare pedici piccoli
                        # PNG è fondamentale per la nitidezza delle linee
                        pix = page.get_pixmap(dpi=300, alpha=False)
                        hq_bytes = pix.tobytes("png")

                        # 2. Chiamata specifica per Formule
                        # Nota: La funzione extract_formulas_vision usa internamente il prompt
                        # che forza LaTeX e ignora il resto se non è matematica.
                        math_json = extract_formulas_vision(hq_bytes)

                        # 3. Se il modello Vision conferma che ci sono formule:
                        if math_json and math_json.get("formulas"):
                            
                            sem_math = build_formula_semantic_chunk(page_no, math_json)
                            
                            # Crea un chunk specifico "formula"
                            final_chunks.append({
                                "text_raw": json.dumps(math_json, ensure_ascii=False),
                                "text_sem": sem_math,
                                "toon_type": "formula", # Tipo speciale per filtering
                                "page_no": page_no,
                                "metadata": {
                                    "source": filename,
