...
class State(rx.State):
    ...
    async def handle_submit(self):
        ...
        try:
            ...
            # ✅ 5.2 Answerability check: per query non-news serve almeno 1 fonte Tier A/B
            if (not is_news_query(user_query)) and (not has_sufficient_ab_sources(sources)):
                self.messages.append(
                    ChatMessage(
                        id=str(uuid.uuid4()),
                        role="assistant",
                        content=(
                            "Non ho abbastanza fonti affidabili (Tier A/B) per rispondere con sicurezza.\n\n"
                            "Suggerimenti:\n"
                            "- carica documenti Tier A (metodologie/teoria) o Tier B (casi d’uso/tesi) sul tema\n"
                            "- oppure riformula la domanda in modo più specifico."
                        ),
                        sources=sources,  # così vedi comunque le fonti trovate
                    )
                )
                yield rx.scroll_to("chat_bottom")
                return
            ...
            # ✅ Inserisci guardrails come SYSTEM message (forte)
            guardrails = tier_guardrail_instructions(user_query)
            messages_payload = [{"role": "system", "content": guardrails}] + messages_payload

            # il tuo comportamento attuale: eviti doppio user consecutivo
