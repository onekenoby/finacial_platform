...
RERANK_CANDIDATES = int(os.getenv("RERANK_CANDIDATES", "40"))     # rerank at most N (cost control)
FINAL_SOURCES = int(os.getenv("FINAL_SOURCES", "10"))             # final context sources
MAX_PER_PAGE = int(os.getenv("MAX_PER_PAGE", "2"))                # diversify results: max K per page
MAX_PER_DOC = int(os.getenv("MAX_PER_DOC", "6"))                  # diversify results: max K per document
...
def get_neighbor_chunk_ids(chunk_ids: List[str], limit: int = GRAPH_MAX_NEIGHBOR_CHUNKS) -> List[str]:
    """
    Expand context with neighboring chunks within same Document:
    (:Document)-[:HAS_CHUNK]->(:Chunk)
    We pull chunks of the same document around the seed chunks.
    """
    if not chunk_ids or not neo4j_driver:
        return []
    query = """
    ...
    WHERE c.id IN $ids
    WITH d, collect(c) AS seeds
    MATCH (d)-[:HAS_CHUNK]->(c2:Chunk)
    WHERE abs(c2.chunk_index - seeds[0].chunk_index) <= 2
    RETURN DISTINCT c2.id AS cid
    LIMIT $lim
    """
    out = []
    try:
        ...
        print(f"⚠️ Neo4j Query Error (neighbors): {e}")
    # Remove already present ids
    out = [x for x in out if x not in set(chunk_ids)]
    return out
...
def retrieve_v2(query_text: str) -> List[SourceItem]:
    ...
    if reranker:
        try:
            cross_inputs = [[query_text, c["content"][:1200]] for c in candidates]  # cap for speed
...
def short_text(s: str, n: int = 320) -> str:
    """Short one-liner for popups/cards to avoid heavy rendering."""
    ...
    return rx.flex(
        ...
        rx.vstack(
            rx.box(
                ...
                rx.text(f"Powered by {LLM_MODEL_NAME} + Qdrant + Neo4j", color="gray", size="2", align="center"),
                padding_y="1em",
                width="100%",
                text_align="center",
            ),
