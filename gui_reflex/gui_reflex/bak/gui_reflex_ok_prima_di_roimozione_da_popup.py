    ...
    async def handle_submit(self):
            ...
            try:
                ...
                if analytics_mode:
                    ...
                    final_prompt_content = f"### SYSTEM ###\n{system_instructions}\n\n### QUESTION ###\n{user_query}{language_reminder}"
                else:
                    # 1. RECUPERO DATI (Hybrid Search + Rerank)
                    sources, debug_md = retrieve_v2(user_query)
                    
                    # 2. RAGGRUPPAMENTO FONTI (FORMATO "CLEAN BLOCK")
                    c_a_list, c_b_list, c_c_list, c_g_list = [], [], [], []

                    for i, s in enumerate(sources, start=1):
                        # FORMATO OTTIMIZZATO PER LLM:
                        # Usa ">>>" per separare nettamente l'header dal contenuto.
                        # Niente ID tecnici complessi, solo l'indice [i].
                    ...
                    # 3. COSTRUZIONE PROMPT DI SISTEMA (Definita PRIMA dell'uso)
                ...
                # Rimuove l'ultimo messaggio user (quello appena inserito) per sostituirlo con il prompt arricchito
                ...
                if llm_client:
                    stream = llm_client.chat.completions.create(
                        model=LLM_MODEL_NAME, messages=messages_payload, temperature=0.1, stream=True
                    )
                    for chunk in stream:
                        ...
                        rx.vstack(
                            rx.foreach(
                                ...
                                lambda msg: rx.vstack(
                                    rx.hstack(
                                        rx.avatar(
                                            fallback=rx.cond(msg.role == "user", "U", "AI"),
                                            variant="solid",
                                            color_scheme=rx.cond(msg.role == "user", "blue", "teal"),
                                        ),
                                        rx.box(
                                            rx.markdown(msg.content),
                                            
                                            # AI SIMBOLICA: triangle_alert è l'icona corretta per la tua versione
                                            rx.cond(
                                                msg.content.contains("⚠️ CONFLITTO RILEVATO"),
                                                rx.callout(
                                                    "Incoerenza rilevata con la Metodologia Tier A (Ground Truth).",
                                                    icon="triangle_alert", 
                                                    color_scheme="red",
                                                    role="alert",
                                                    margin_top="1rem",
                                            ...
                                            rx.cond(
                                                ...
                                                rx.hstack(
                                                    rx.badge(
                                                        f"{rx.cond(msg.sources, msg.sources.length(), 0)} fonti",
                                                        variant="soft", 
                                                        on_click=lambda: State.toggle_inline_sources(msg.id),
                                                        cursor="pointer",
                                                    ),
                                            ...
                                            background_color=rx.cond(msg.role == "user", "rgba(0,0,0,0.03)", "white"),
                                            padding="1rem",
                                            border_radius="lg",
                                            border=rx.cond(msg.role == "assistant", "1px solid #E5E7EB", "none"),
                                            max_width="85%",
                                        ),
                                        width="100%",
...
def render_inline_sources(msg: ChatMessage):
    """Visualizza i dettagli delle fonti con indicazione del Database."""
    return rx.vstack(
        rx.text("📚 Fonti Documentali:", font_weight="bold", size="2"),
        rx.foreach(
            msg.sources,
            lambda s: rx.hstack(
                # Badge TIER (Esistente)
                rx.badge(
                    s.tier, 
                    color_scheme=rx.cond(s.tier == "A", "red", "gray"), 
                    variant="soft"
                ),
                
                # NUOVO: Badge DATABASE
                rx.badge(
                    s.db_origin,
                    color_scheme=rx.cond(
                        s.db_origin == "Qdrant", "violet",     # Qdrant = Viola
                        rx.cond(
                            s.db_origin == "Postgres", "blue", # Postgres = Blu
                            rx.cond(
                                s.db_origin == "Neo4j", "orange", # Neo4j = Arancio
                                "cyan"                            # Hybrid = Ciano
                            )
                        )
                    ),
                ...
                # Nome file e pagina
                rx.text(f"{s.filename} - Pag. {s.page}", size="1"),
                spacing="2",
                align_items="center"
            )
        ),
                                    ...
                                    lambda msg: rx.vstack(
                                        rx.hstack(
                                            ...
                                            rx.box(
                                                ...
                                                # AI SIMBOLICA: Corretto alert_triangle -> triangle_alert
                                                rx.cond(
                                                    msg.content.contains("⚠️ CONFLITTO RILEVATO"),
                                                    rx.callout(
                                                        "Incoerenza rilevata con la Metodologia Tier A (Ground Truth).",
                                                ...
                                                rx.cond(
                                                    ...
                                                    rx.hstack(
                                                        rx.badge(
                                                            f"{rx.cond(msg.sources, msg.sources.length(), 0)} fonti",
                                                            variant="soft", # Corretto da subtle
                                                            on_click=lambda: State.toggle_inline_sources(msg.id),
                                                            cursor="pointer",
                                                        ),
                                                        rx.badge(
                                                            f"{rx.cond(msg.sources, msg.sources.length(), 0)} fonti", # Conta gli elementi in lista
                                                            variant="soft", 
                                                            on_click=lambda: State.toggle_inline_sources(msg.id),
                                                            cursor="pointer",
                                                        ),
