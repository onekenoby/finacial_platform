    ...
    def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if f_js and f_js.get("formulas"):
                chunk_text += f"--- CONTENUTO MATEMATICO PAGINA {p_no} ---\n"
                if f_js.get("summary_it"):
                    chunk_text += f"Sintesi: {f_js.get('summary_it')}\n\n"
                
                # Aggiungiamo le formule trascritte
                for form in f_js.get("formulas", []):
                    latex = form.get('latex', '').replace('$', '') # Pulizia
                    chunk_text += f"Formula: $${latex}$$\n"
                    chunk_text += f"Descrizione: {form.get('plain')}\n"
                    chunk_text += f"Significato: {form.get('meaning_it')}\n\n"
            else:
                # Fallback se il JSON è vuoto ma Qwen ha risposto testo libero (raro col prompt JSON)
                # O se la pagina non aveva formule ma testo normale
                # In questo caso, potremmo usare un prompt Markdown, ma per ora teniamo il JSON
                chunk_text = f"--- PAGINA {p_no} (Nessuna formula strutturata rilevata) ---\n"
                chunk_text += str(ai_json_str)[:1000] # Debug output raw
