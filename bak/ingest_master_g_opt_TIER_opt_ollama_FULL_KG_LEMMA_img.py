...
FORMULA_VISION_PROMPT = f"""
You are a careful scientific parser. The image contains mathematical or financial formulas.
Your goal is to extract the content in two formats: SEMANTIC (for search) and VECTOR-CODE (LaTeX for rendering).
...
{{
  ...
  "formulas": [
    {{
      "latex": "LaTeX code strictly for rendering. NO surrounding $. Use standard commands.",
      "plain": "normalized plain text version for reading",
      "meaning_it": "MANDATORY: Provide an in-depth explanation in Italian of the mathematical and financial significance. Describe purpose (e.g., volatility, trading signals).",
      "keywords": ["finance_term1", "math_term2"],
      "variables": [
        {{"name": "alpha", "meaning": "Smoothing parameter used to weight recent data."}},
        {{"name": "P_t", "meaning": "Price of the asset at time t."}}
      ]
    }}
  ],
  "summary_it": "Short summary of the mathematical content of the page."
}}

Rules:
- latex: Must be valid and compile-ready.
- meaning_it: MUST be in Italian and highly descriptive.
- Max {VISION_MAX_FORMULAS_PER_PAGE} formulas.
"""
...
def is_text_layer_corrupt(text: str) -> bool:
    """
    Rilevatore Universale di Corruzione / Contenuto Non-Testuale.
    Non si basa su parole chiave, ma sulla statistica dei caratteri.
    
    Ritorna TRUE (-> Attiva Visione) se:
    1. Ci sono marcatori di corruzione noti.
    2. La densità di simboli/numeri supera il 40% (tipico di formule/tabelle).
    3. La densità di parole valide (alfabetiche > 2 char) è sotto il 50%.
    """
    if not text: return False
    
    # 1. Marcatori "Hard" (Quelli noti restano utili per velocità)
    known_garbage = ["2C0D", "•", "–", "—", "˜", "\uf0b7", "× ×"]
    if any(m in text for m in known_garbage):
        return True

    # Pulizia base per analisi statistica
    clean = text.replace("\n", " ").strip()
    if len(clean) < 50: return False # Troppo corto per giudicare

    # 2. Analisi Caratteri
    total_chars = len(clean)
    alpha_chars = sum(1 for c in clean if c.isalpha())
    digit_chars = sum(1 for c in clean if c.isdigit())
    symbol_chars = sum(1 for c in clean if not c.isalnum() and not c.isspace())

    # SEGNALE A: Troppi simboli (LaTeX esploso, grafici ascii, spazzatura)
    # Esempio: "y = \sqrt{x} + \alpha" -> molti simboli
    if (symbol_chars / total_chars) > 0.15: # Sopra il 15% di simboli è sospetto
        return True

    # SEGNALE B: Troppi numeri (Tabelle finanziarie o indici esplosi)
    # Esempio: "2023 100 200 4.5%"
    if (digit_chars / total_chars) > 0.20: # Sopra il 20% di numeri
        return True

    # SEGNALE C: Poche parole reali (Testo non umano)
    words = clean.split()
    if not words: return True
    
    valid_words = [w for w in words if w.isalpha() and len(w) > 2]
    valid_word_ratio = len(valid_words) / len(words)
    
    # Se meno del 50% delle "parole" sono parole reali -> Probabile spazzatura o Math
    if valid_word_ratio < 0.5:
        return True

    return False

def smart_clean_text(text: str) -> str:
    """Pulisce il testo e rimuove pattern ripetitivi (WwWw)."""
    if not text: return ""
    
    # 1. Rimuovi caratteri spazzatura specifici
    text = re.sub(r'[\x95\x96\x97\x98\x81\x80\x8d\uf0b7\uf020]', ' ', text)
    
    # 2. Rileva e rimuove pattern ripetitivi ossessivi (es. "WwWwWwWw")
    # Cerca sequenze di 2 caratteri ripetute più di 6 volte
    if re.search(r'(.{1,2})\1{6,}', text):
        return "" # Chunk corrotto da watermark ripetitivo
    ...
    for line in lines:
        ...
        # Filtra intestazioni note
        if any(x in line.lower() for x in ["formulae sheet", "maths tables", "financial management (fm)", "s17"]):
            continue
...
# 1. Regex "Grilletto": Include i difetti specifici del tuo PDF (2c0d, simboli spazzatura)
MATH_CANDIDATE_PAT = re.compile(
    r"(?i)("
    r"formulae\s+sheet|"
    r"maths\s+tables|"
    r"economic\s+order\s+quantity|"
    r"2c0d|"             # <--- GRILLETTO FONDAMENTALE (Formula EOQ rotta)
    r"miller[-.\s]?orr|"
    r"[•–—˜\uf0b7]"      # <--- GRILLETTO SPAZZATURA (Bullet points rotti)
    r")"
)

# 2. Prompt "Tuttofare": Istruisce l'AI a fare da OCR intelligente
FULL_PAGE_OCR_PROMPT = """
You are a strict, robotic OCR engine. Your ONLY job is to transcribe the text and formulas visible in the image.

RULES:
1. **Verbatim Only**: Transcribe EXACTLY what is written. Do NOT add headers, titles, or sections that are not physically present.
2. **No Hallucinations**: If you see a list ending, STOP. Do NOT auto-complete lists with external knowledge (e.g. do not add 'Monte Carlo' if not visible).
3. **Formulas**: Output LaTeX exactly as seen. If a formula is blurry/unreadable, write [UNCLEAR].
4. **No Repetition**: Do not repeat the same formula ($$ P = ... $$) multiple times unless it is actually printed multiple times.
5. **Output format**: Return ONLY the Markdown string. No conversational filler.
"""
    ...
    for _ in range(LLM_RETRIES + 1):
        try:
            resp = chat(
                ...
                format="", # Togli "json" se vuoi markdown libero per le formule, o lascialo se il prompt lo richiede.
                           # Per FULL_PAGE_OCR_PROMPT meglio lasciare stringa libera (format="") o non specificarlo.
                options={
                    "temperature": 0.0,      # Deterministico
                    "num_predict": int(max_tokens),
                    "repeat_penalty": 1.15,  # <--- CRUCIALE: Penalizza i loop WwWwW / Formule ripetute
                    "top_p": 0.2             # Taglia le allucinazioni creative
                },
            )
...
def recurse_text_chunking(text: str, base_meta: Dict[str, Any], max_chars: int = 1000) -> List[Dict[str, Any]]:
    ...
    if len(text) <= max_chars:
        ...
        # Formato: "Documento: X | Pagina: Y | Contenuto..."
        ...
        return [{
            ...
            "page_no": p_no,
            "toon_type": "text",
            "section_hint": "text_body",
            "image_id": None, # Nessuna immagine per chunk testuali
            "metadata_override": base_meta
        }]

    # LOGICA RICORSIVA: Il testo è troppo lungo, dobbiamo spezzarlo
    chunks = []
    ...
    for sep in separators:
        # Cerchiamo un separatore che sia presente e che divida il testo in modo sensato
        # (cioè non un separatore che appare solo alla fine)
        if sep in text:
    ...
    # 2. Se nessun separatore funziona (caso raro, testo enorme senza spazi), taglio brutale
    if not split_char:
        mid = len(text) // 2
        return (
...
def is_text_layer_corrupt(text: str) -> bool:
    """Rileva se il testo estratto è spazzatura (font non mappati)."""
    if not text: return False
    # Cerca marcatori di corruzione specifici del tuo file
    if "2C0D" in text: return True
    if re.search(r'[•–—˜\uf0b7]', text): return True
    
    # Cerca densità di numeri orfani (formule esplose)
    lines = text.split('\n')
    if len(lines) > 5:
        numeric_lines = sum(1 for line in lines if re.fullmatch(r'[\d\s\W]+', line.strip()) and len(line) > 5)
        if (numeric_lines / len(lines)) > 0.40: return True
    return False

def smart_clean_text(text: str) -> str:
    """Pulisce il testo mantenendo la struttura."""
    # Rimuove simboli non stampabili e spazzatura nota
    text = re.sub(r'[•–—˜\uf0b7]', ' ', text)
    # Rimuove sequenze di soli numeri spaziati (es. "3 3 4 1") se non sembrano dati
    lines = [line.strip() for line in text.split('\n')]
    clean_lines = []
    for line in lines:
        if not line: continue
        # Salta Header noti ripetitivi
        if "Financial Management (FM)" in line or "Formulae Sheet" in line: continue
        clean_lines.append(line)
    return "\n".join(clean_lines)

def recurse_text_chunking(text: str, base_meta: Dict[str, Any], max_chars: int = 1000) -> List[Dict[str, Any]]:
    """Divide il testo (o Markdown AI) in chunk rispettando la semantica."""
    text = text.strip()
    if not text: return []
    
    # Caso base: rientra nel limite
    if len(text) <= max_chars:
        toon_type = "text"
        if "$$" in text: toon_type = "formula"
        elif "| --- |" in text: toon_type = "table"
        
        return [{
            "text_raw": text,
            "text_sem": f"Doc: {base_meta.get('source')} | Pag: {base_meta.get('page')}\n{text}",
            "page_no": base_meta.get("page"),
            "toon_type": toon_type,
            "section_hint": base_meta.get("type", "text"),
            "image_id": base_meta.get("original_image_id"), # Link immagine DB
            "metadata_override": base_meta
        }]

    # Logica ricorsiva di taglio (paragrafo -> frase -> spazio)
    separators = ["\n\n", "\n", ". ", " "]
    split_char = ""
    for sep in separators:
    ...
    if not split_char: # Fallback taglio netto
        mid = len(text) // 2
        return recurse_text_chunking(text[:mid], base_meta, max_chars) + recurse_text_chunking(text[mid:], base_meta, max_chars)

    chunks = []
    current_chunk = ""
    for part in text.split(split_char):
        candidate = part if not current_chunk else (current_chunk + split_char + part)
        if len(candidate) <= max_chars:
            current_chunk = candidate
        else:
            if current_chunk: chunks.extend(recurse_text_chunking(current_chunk, base_meta, max_chars))
            current_chunk = part
    ...
    # =========================================================================
    # ESECUZIONE PARALLELA (ThreadPool)
    # =========================================================================
    with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as executor:
        ...
        for future in as_completed(future_to_page):
            try:
                ...
                if page_res:
                    out_chunks.extend(page_res)
                
                completed_count += 1
                if completed_count % 5 == 0 or completed_count == total_pages:
                    print(f"   🔄 [{(completed_count/total_pages)*100:.0f}%] Analisi chunk {completed_count}/{total_pages}...", end="\r")
            except Exception as e:
                print(f"   ⚠️ Thread Error: {e}")

    print("") # Newline post-progress
    
    # Ordina per numero pagina (i thread finiscono in ordine casuale)
    out_chunks.sort(key=lambda x: x["page_no"])

    # Aggiungi Context Windows (Sliding Window)
    return add_context_windows(out_chunks)



    # =========================
    # PDF extraction (HYPER-SMART v3.3)
    # =========================
# =========================
# PDF extraction (HYPER-SMART v3.3)
# =========================
# =========================
# 1. PDF EXTRACTION (HYPER-SMART v3.3 - Vision Supremacy)
# =========================
# =========================
# PDF EXTRACTION (HYPER-SMART v3.4 - Anti-Hallucination)
# =========================
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    print(f"   🚀 Ingestion Start: {total_pages} pagine | Mode: Vision-Supremacy (Anti-Hallucination)")
    ...
    def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raw_text = p_obj.get_text("text")
            
            # Controlli Boolean (Usa le funzioni globali definite fuori)
            is_math_heavy = bool(MATH_CANDIDATE_PAT.search(raw_text))
            is_corrupt = is_text_layer_corrupt(raw_text)
            is_visual = bool(CHART_CANDIDATE_PAT.search(raw_text)) or len(p_obj.get_image_info()) > 0

            # DECISIONE CRITICA
            use_vision_ocr = PDF_VISION_ENABLED and (is_math_heavy or is_corrupt or is_visual)

            # RAMO A: VISIONE AI (Con protezioni Anti-Loop)
            if use_vision_ocr:
                # FIX 3: DPI aumentato a 160. 
                # Se è troppo basso (130), il modello vede sfocato e inventa.
                    ...
                    try:
                        with t_conn.cursor() as t_cur:
                            img_id = pg_save_image(
                                ...
                                f"Page_{p_no}_Vision", cur=t_cur
                            )
                        t_conn.commit()
                ...
                try:
                    ...
                    # FIX 2: Parametri "Zero Creatività"
                    # Passiamo le opzioni direttamente a Ollama
                    ai_markdown = llm_chat_multimodal(
                        prompt=FULL_PAGE_OCR_PROMPT,
                        image_bytes=vision_bytes,
                        model=VISION_MODEL_NAME,
                        max_tokens=2500
                    )
                    # NOTA: Assicurati che llm_chat_multimodal usi 'options' (vedi sotto)

                    # FIX 3: Loop Detector (Safety Net)
                    # Conta le formule LaTeX ripetute
                    if ai_markdown:
                        latex_formulas = re.findall(r'\$\$(.*?)\$\$', ai_markdown, re.DOTALL)
                        if len(latex_formulas) > 4:
                            # Controlla se c'è una formula ripetuta più di 3 volte identica
                            from collections import Counter
                            counts = Counter(latex_formulas)
                            most_common, count = counts.most_common(1)[0]
                            
                            if count > 4:
                                print(f"   ⚠️ HALLUCINATION DETECTED p.{p_no}: Formula ripetuta {count} volte. Troncamento attivo.")
                                # Strategia di recupero: Tieni solo la prima parte del testo prima che inizi il loop impazzito
                                # O semplicemente scartalo e usa un messaggio di errore
                                ai_markdown = "⚠️ [CONTENT REDACTED DUE TO AI HALLUCINATION LOOP] ⚠️\n" + ai_markdown[:500]

                    if ai_markdown:
                        # FIX 4: Disabilita KG per le pagine Vision (riduce il tempo e il rischio di estrarre "Monte Carlo")
                        meta = {
                            "source": filename, 
                            "page": p_no, 
                            "type": "vision_ocr", 
                        ...
                        chunks = recurse_text_chunking(ai_markdown, meta, max_chars=CHUNK_MAX_CHARS)
                        local_results.extend(chunks)
                        
                except Exception as e:
                    print(f"   ⚠️ Vision Error p.{p_no}: {e}")

            # RAMO B: TESTO STANDARD
            else:
                rect = p_obj.rect
                clip = fitz.Rect(0, rect.height * 0.08, rect.width, rect.height * 0.92)
                
                blocks = p_obj.get_text("blocks", clip=clip)
                clean_txt_blocks = []
                for b in blocks:
                    clean = smart_clean_text(b[4])
                    if len(clean) > 20: clean_txt_blocks.append(clean)
                
                full_text = "\n\n".join(clean_txt_blocks)
                
                if full_text:
                    meta = {"source": filename, "page": p_no, "type": "text", "original_image_id": None}
                    chunks = recurse_text_chunking(full_text, meta, max_chars=CHUNK_MAX_CHARS)
                    local_results.extend(chunks)

        except Exception as e:
            print(f"   ❌ Error p.{p_no}: {e}")
        finally:
            if 'doc_worker' in locals():
...
# =========================
# 2. FILE DISPATCHER (Deve stare QUI, subito dopo extract_pdf_chunks)
# =========================
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Dispatcher centrale: DEVE chiamare extract_pdf_chunks (Vision Supremacy).
    """
    ext = os.path.splitext(file_path)[1].lower()
    if ext == ".pdf":
        # ✅ QUESTO è il punto critico. Deve chiamare la funzione NUOVA.
