...
LLM_MODEL_NAME = os.getenv("LLM_MODEL_NAME", "ministral-3:3b")
...
def llm_chat_multimodal(prompt: str, image_bytes: bytes, model: str, max_tokens: int = 4000) -> str:
    ...
    Replica il successo del test con Ministral 3B (Text+Vision via Ollama).
    """
    if not image_bytes:
        return ""

    # 1. Conversione Base64 (Necessaria anche per la lib nativa)
    try:
        img_b64 = base64.b64encode(image_bytes).decode("utf-8")
    except Exception as e:
        print(f"   ❌ Errore Base64: {e}")
        return ""

    try:
        # 2. Chiamata Nativa
        # Ministral 3B risponde bene con temperature 0.0 per compiti deterministici
        response: ChatResponse = chat(
            ...
            options={
                ...
                "num_ctx": 4096,    # Contesto ampio per l'immagine
            }
        )
        
        # 3. Estrazione Contenuto Sicura
        content = ""
        if hasattr(response, 'message') and response.message.content:
            content = response.message.content
        elif isinstance(response, dict): # Fallback per versioni lib diverse
...
# ==============================================================================
# ==============================================================================
# MOTORE VISIONE (DIRECT TEXT - CLEAN MATH)
# ==============================================================================
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Versione 'Direct Text': Trascrive la pagina come testo formattato (Unicode/Markdown)
    usando Ministral 3B, saltando il parsing JSON.
    Replica la logica 'Best Result' testata dall'utente.
    """
    out_chunks = []
    filename = os.path.basename(file_path)
    ...
    print(f"   🚀 Ingestion: {total_pages} pagine | Engine: {VISION_MODEL_NAME} (Vision Direct)")
    ...
    def process_page_worker(p_idx: int):
        ...
        try:
            ...
            # 1. Rendering a 120 DPI (Come da test vincente)
            zoom = VISION_DPI / 72
            mat = fitz.Matrix(zoom, zoom)
            pix = page.get_pixmap(matrix=mat, alpha=False)
            img_bytes = pix.tobytes("png")
            
            # 2. Salva img nel DB (per debug/audit e visualizzazione in frontend)
            img_id = None
            t_conn = pg_get_conn()
            if t_conn:
                try:
            ...
            # 3. Chiamata AI (Ministral 3B via Ollama Native)
            # Ritorna testo Markdown/Unicode pulito (Formule, Tabelle, Testo)
            ...
            # 4. Validazione e Chunking
            if chunk_text and "NO_CONTENT" not in chunk_text and len(chunk_text) > 10:
                # Aggiungiamo Header per contesto RAG
                full_text = f"Documento: {filename} | Pagina: {p_no}\n{chunk_text}"
                
                # Metadati per la pipeline
                meta = {
                    "source": filename, 
                    "page": p_no, 
                    "type": "vision_math",
                ...
                # Chunking ricorsivo sul testo trascritto
                # Questo preparerà i dati per Qdrant (vettori) e Neo4j (entità dal testo)
    ...
    # Ministral è leggero, con P5000 possiamo osare 3-4 worker
