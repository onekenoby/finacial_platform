...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    VERSIONE FIXATA: 
    - Processi do_f (formule) e do_c (immagini) INDIPENDENTI
    - Cattura garantita di grafici vettoriali e immagini embedded
    - Differenziazione semantica text_raw vs text_sem
    """
    ...
    try:
        ...
        def process_page_worker(p_idx):
            ...
            # --- VISION & STORAGE (Processi separati) ---
            if do_f or do_c:
                try:
                    t_conn = pg_get_conn()
                    try:
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_f:
                                ...
                                if (f_js and f_js.get("has_formulas")) or is_low_text_page:
                                    ...
                                    # USA (f_js or {}) per evitare il crash se f_js è None
                                    formulas_list = (f_js or {}).get("formulas", [])
                                    raw_latex = "\n".join([f.get("latex", "") for f in formulas_list if f.get("latex")])
                                    sem_desc = build_formula_semantic_chunk(p_no, f_js) if f_js else f"Formula rilevata a pagina {p_no}"
                            ...
                            # B. GESTIONE IMMAGINI / GRAFICI (Indipendente)
                            if do_c:
                                found_valid_crop = False
                                if has_imgs:
                                    # Analizza le immagini incorporate (bitmap)
                                    for img_meta in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                        ...
                                        # --- PROTEZIONE NEL BLOCCO IMMAGINI (CROP) ---
                                        if len(crop_jpg) >= MIN_IMAGE_BYTES:
                                            ...
                                            # FIX: Aggiunta protezione or {}
                                            c_safe = c_js or {}
                                            raw_info = f"Immagine: {c_safe.get('kind', 'visual')} - {c_safe.get('title', '')}"
                                            sem_desc = build_chart_semantic_chunk(p_no, c_js or {}, prefix="IMAGE")
                                ...
                                # Fallback: Cattura la pagina intera se ci sono grafici vettoriali o se i crop non bastano
                                if (not found_valid_crop) or is_visual_page or is_low_text_page:
                                    ...
                                    if c_js and c_js.get("kind") != "other":
                                        ...
                                        sem_desc = build_chart_semantic_chunk(p_no, c_js or {}, prefix="VISUAL")
