...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict]:
    ...
    for page_idx in range(len(doc)):
        ...
        #---------------------------------------------
        used = 0
        seen_img_ids_on_page = set()  # opzionale: evita duplicati stessi img_id nella pagina
        ...
        for _, imd in candidates:
            ...
            try:
                ...
                if cached:
                    # ✅ Cache HIT: non rifare vision, usa desc salvata
                    img_id, desc_ai = cached[0], (cached[1] or "")
                else:
                    # ✅ Cache MISS: prepara bytes per vision
                    ...
                    if vis and isinstance(vis, dict):
                        ...
                        # Vision fallita: non ho descrizione => non salvo e non creo chunk
                        desc_ai = ""

                    if not desc_ai.strip():
                        continue

                    # ✅ Log immagine ORIGINAL su PG (solo se non cached)
                ...
                # se cached ma desc vuota, skip
                if not desc_ai.strip() or not img_id or img_id <= 0:
                    continue

                # opzionale: evita duplicare stessa immagine nella pagina
                if img_id in seen_img_ids_on_page:
                    continue
                seen_img_ids_on_page.add(img_id)
                ...
                # meglio loggare: aiuta a capire quando una pagina viene saltata
                print(f"   ⚠️ PDF image pipeline error page={page_no}: {e}")
                continue
        #----------------------------------------------
