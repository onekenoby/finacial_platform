...
BATCH_SIZE = 10  # Numero di chunk da processare prima di scrivere su DB
...
def flush_postgres_batch(batch_data: List[Tuple]):
    """Scrive un batch di chunk su Postgres in una sola transazione"""
...
def flush_neo4j_batch(batch_graph_data: List[Dict], doc_type: str):
    """Scrive un batch di sottografi su Neo4j usando UNWIND"""
    if not batch_graph_data: return
    
    query = """
    UNWIND $batch AS row
    MERGE (c:Chunk {id: row.chunk_id, type: $doc_type})
    ...
    FOREACH (edge IN row.edges | 
        MERGE (source:Entity {name: edge.source})
        MERGE (target:Entity {name: edge.target})
        MERGE (source)-[r:RELATED_TO]->(target)
        WITH r, edge
        CALL apoc.create.setRelType(r, edge.relation) YIELD rel
        RETURN count(*)
    )
    """
    # Nota: Poiché APOC setRelType è complesso in Cypher puro senza plugin, 
    # usiamo una versione semplificata Python-side per le relazioni dinamiche se APOC non c'è.
    # Versione ottimizzata senza APOC (più sicura):
    ...
    with neo4j_driver.session() as session:
        for item in batch_graph_data:
            ...
            session.run("""
                ...
                WITH c
                UNWIND $nodes AS n
                MERGE (e:Entity {name: n.id}) ON CREATE SET e.type = n.type
                MERGE (e)-[:MENTIONED_IN]->(c)
            """, cid=chunk_id, dtype=doc_type, nodes=item.get('nodes', []))
            
            # 2. Relazioni (Loop perché i tipi sono dinamici)
            for edge in item.get('edges', []):
                if "source" not in edge or "target" not in edge or "relation" not in edge: continue
                rel_type = "".join(c for c in edge['relation'].upper().replace(" ", "_") if c.isalnum() or c == '_')
...
# --- SINGLE DB HANDLERS (Per Logs e Immagini - rimangono singoli per ora) ---
    ...
    try:
        ...
        response = llm_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": [{"type": "text", "text": "Describe this image in ENGLISH. Extract key data points."}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}]}
...
def process_single_file(file_path: str, doc_type: str):
    ...
    try:
        ...
        for i, text_chunk in enumerate(chunks_text):
            ...
            # 1. AI: Embedding (Singolo, necessario per Qdrant)
            vec = embedder.encode(text_chunk, normalize_embeddings=True).tolist()
            qdrant_buffer.append(models.PointStruct(
                id=chunk_uuid, 
                vector=vec, 
                payload={"filename": filename, "type": doc_type, "chunk_index": i, "toon_type": toon_type}
            ))
            
            # 2. AI: Graph Extraction (Singolo)
            g_data = extract_graph_data(text_chunk, doc_type, filename)
            if g_data: 
                g_data['chunk_id'] = chunk_uuid # Aggiungo ID per il batching
                neo4j_buffer.append(g_data)
            
            # 3. Preparazione Postgres
            meta = {"qdrant_uuid": chunk_uuid, "original_path": file_path}
            postgres_buffer.append((log_id, i, toon_type, text_chunk, text_chunk, Json(meta))) # Tuple per execute_values
            
            count += 1
            print(f".", end="", flush=True) # Feedback visivo "dot progress"

            # --- FLUSH BUFFER (Se pieno) ---
            if len(qdrant_buffer) >= BATCH_SIZE:
                print(f" [Flushing Buffer {len(qdrant_buffer)}] ", end="")
        ...
        # --- FLUSH FINALE (Per i rimanenti) ---
        if qdrant_buffer:
            print(f" [Flushing Final {len(qdrant_buffer)}] ", end="")
            qdrant_client.upsert(collection_name=QDRANT_COLLECTION, points=qdrant_buffer)
            flush_postgres_batch(postgres_buffer)
            flush_neo4j_batch(neo4j_buffer, doc_type)
            print("✅")
...
def main():
    ...
    folder_map = {"financial_reports": "financial", "educational": "educational", "client_profiles": "client"}
