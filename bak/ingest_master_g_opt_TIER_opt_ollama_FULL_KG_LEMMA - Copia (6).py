...
def clean_markdown_structure(md_content: str) -> str:
    """
    Analizza il Markdown e rimuove sezioni strutturali pesanti 
    (Indici, Sommari, Bibliografia) e rumore web.
    """
    # Dividiamo il documento in sezioni basate sui titoli (#)
    sections = re.split(r'(\n#+ .*)', md_content)
    cleaned_sections = []
    
    # Heuristic: Saltiamo la primissima sezione se è molto corta (spesso è la copertina)
    start_idx = 0
    if len(sections) > 1 and len(sections[0].strip()) < 300:
        start_idx = 2 # Salta prologo e primo titolo di copertina
        
    for i in range(start_idx, len(sections)):
        section_text = sections[i]
        
        # Se il titolo della sezione contiene parole chiave junk, la scartiamo
        # Utilizziamo la regex STRUCTURAL_PAT già definita nel file
        if bool(STRUCTURAL_PAT.search(section_text[:400])):
            continue
            
        # Rimuoviamo link web, email o pubblicità (comuni nei footer convertiti)
...
def extract_pdf_as_markdown_assets(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Trasforma il PDF in un flusso Markdown virtuale.
    Semplifica il PDF separando il testo dalle immagini 'parcheggiate' in RAM.
    Include un SELEZIONATORE DI IMMAGINI per scartare loghi e icone.
    """
    doc = fitz.open(file_path)
    filename = os.path.basename(file_path)
    asset_park = {} # Dizionario RAM per le immagini: {asset_id: bytes}
    virtual_md_content = []
    
    print(f"   🔄 Conversione PDF -> MD Asset Pipeline: {len(doc)} pagine...")

    for p_idx in range(len(doc)):
        page = doc[p_idx]
        p_no = p_idx + 1
        
        # 1. Estrazione Testo (con hint di struttura)
        p_text = page.get_text("text")
        if is_structural_page(p_text): continue # Salta indici/sommari
        
        # Pulizia testo
        p_text = normalize_ws(p_text)
        virtual_md_content.append(f"\n# PAGE {p_no}\n{p_text}")

        # 2. SELEZIONATORE IMMAGINI (Chirurgico)
        ...
        for img_idx, img in enumerate(image_list):
            ...
            # FILTRO SELEZIONATORE: 
            # Scarta se < 7KB (icone) o se dimensioni sospette (es. banner stretti)
            if len(img_bytes) < 7000: continue 
            
            asset_id = f"img_p{p_no}_{img_idx}.jpg"
            asset_park[asset_id] = img_bytes
            
            # Inserisce il tag nel flusso testuale
            virtual_md_content.append(f"\n![{asset_id}]({asset_id})\n")

    full_virtual_md = "\n".join(virtual_md_content)
    
    # Ora usiamo la logica Markdown sui dati estratti dal PDF
...
def process_virtual_md_chunks(content: str, asset_park: dict, filename: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Prende il Markdown virtuale e lo trasforma in chunk pronti per l'ingestion,
    interpellando la Vision AI solo per gli asset parcheggiati.
    """
    out_chunks = []
    # Auto-cleaning finale del testo estratto
    content = clean_markdown_structure(content)
    raw_paras = re.split(r'\n(?=# PAGE)|\n\n', content)

    for idx, para in enumerate(raw_paras):
        if len(para.strip()) < MIN_CHUNK_LEN: continue

        chunk_data = {
            "text_raw": para,
            "text_sem": f"Doc: {filename} | Content: {para[:60]}...\n{para}",
            "page_no": 1, # Normalizzato
            "toon_type": "text",
            "section_hint": find_section_hint(para)
        }

        # Cerca se il chunk contiene un riferimento a un asset in RAM
        ...
        if img_match and PDF_VISION_ENABLED:
            ...
            if img_bytes:
                # VISION AI CHIRURGICA su asset in RAM
                c_js = extract_chart_via_vision(img_bytes)
                if c_js and c_js.get("kind") != "other":
                    chunk_data["toon_type"] = "immagine"
                    chunk_data["text_sem"] = build_chart_semantic_chunk(1, c_js, prefix="PDF_ASSET")
                    
                    # Salvataggio in Postgres
                    conn = pg_get_conn()
                    try:
                        with conn.cursor() as cur:
                            chunk_data["image_id"] = pg_save_image(log_id, img_bytes, "image/jpeg", f"ASSET_{asset_id}", cur)
                        conn.commit()
                    finally: pg_put_conn(conn)
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Funzione dispatcher aggiornata:
    Se PDF -> Attiva pipeline Markdown Virtuale con Asset in RAM.
    Se MD  -> Attiva estrattore Markdown standard.
    """
    ext = os.path.splitext(file_path)[1].lower()
    
    if ext == ".pdf":
        # NUOVA STRATEGIA: Trasforma in MD e parcheggia immagini in RAM
        return extract_pdf_as_markdown_assets(file_path, log_id)
        
    elif ext == ".md":
        # Strategia standard per file già in Markdown
