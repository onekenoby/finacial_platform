...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    # --- Reset Vision stats (per documento) ---
    ...
    try:
                    ...
                    try:
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_c:
                                ...
                                # Stat: pagina risolta SOLO con crop (max 1 per pagina)
        ...
        with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as p_executor:
            ...
            for fut in as_completed(futures):
                try:
                    ...
                    # Progress thread-safe (pagine completate)
                    with _VISION_STATS_LOCK:
                        VISION_STATS["pages_done"] += 1
                        done = VISION_STATS["pages_done"]
        ...
        print(
            f"\n   📊 Vision Report Finale | Pagine Totali: {VISION_STATS['pages_total']} | Done: {VISION_STATS['pages_done']} | "
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    if ext == ".pdf":
        ...
        return extract_markdown_chunks(file_path, log_id) # <--- Supporto MD
    return []
