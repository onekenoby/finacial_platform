...
def extract_chart_via_vision(img_bytes: bytes) -> Optional[Dict[str, Any]]:
    vbytes = _downscale_and_compress_for_vision(img_bytes)
    if not vbytes: return None
    
    raw = llm_chat_multimodal(CHART_VISION_PROMPT, vbytes, VISION_MODEL_NAME)
    js = safe_json_extract(raw)
    
    # Validazione minima: basta che esista il campo kind o summary_it
    if isinstance(js, dict) and (js.get("kind") or js.get("summary_it")):
        return js
    return None
    ...
    try:
        resp = chat(
            ...
            options={
                ...
                "num_predict": 2500,   # Ampio spazio per grafi complessi 
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        def process_page_worker(p_idx):
            ...
            # --- 3. VISION & STORAGE (Processi separati) ---
            if do_f or do_c:
                try:
                    t_conn = pg_get_conn()
                    try:
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_f:
                                ...
                                f_js = f_js_raw or {"has_formulas": False, "formulas": []} # Protezione
                                ...
                                if f_js.get("has_formulas") or is_low_text_page:
                                    ...
                                    sem_desc = build_formula_semantic_chunk(p_no, f_js) if f_js_raw else f"Formula a pagina {p_no}"
                                    
                                    local_results.append({
                                        "text_raw": raw_latex if raw_latex else f"Formule pagina {p_no}", 
                                        "text_sem": sem_desc, "page_no": p_no, "toon_type": "formula",
                                        "section_hint": s_hint, "image_id": img_id, "metadata_override": f_js
                                    })

                            # B. GESTIONE IMMAGINI / GRAFICI
                            if do_c:
                                ...
                                if has_imgs:
                                    for img_meta in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                        ...
                                        if len(crop_jpg) >= MIN_IMAGE_BYTES:
                                            ...
                                            local_results.append({
                                                "text_raw": raw_info, "text_sem": sem_desc, "page_no": p_no,
                                                "toon_type": "immagine", "image_id": img_id, "metadata_override": c_js
                                            })
                                            found_valid_crop = True

                                # Fallback Vettoriale (Full Page)
                                if (not found_valid_crop) or is_visual_page or is_low_text_page:
                                    full_jpg = render_full_page_jpeg(p_obj, dpi=VISION_DPI)
                ...
                for idx, ch in enumerate(batch):
                    ...
                    with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as executor:
                        ...
                        for p_no, indexed_chunks in pages_map.items():
                            ...
                            # --- NUOVA LOGICA SELETTIVA ---
                            # Attiviamo l'LLM per il Grafo solo se:
                            # 1. Il testo è abbastanza lungo (> 400 caratteri)
                            # 2. SE è Tier A (Metodologia) -> SEMPRE (è il cuore logico)
                            # 3. SE è Tier B/C -> SOLO se is_keyword_candidate_hybrid è True
                            
                            should_extract_kg = len(combined_text) > 400 and (
                                tier == "A" or is_keyword_candidate_hybrid(combined_text)
                            )

                            if should_extract_kg:
                                # 1. Lanci l'analisi della pagina
                                fut = executor.submit(llm_extract_kg, filename, p_no, combined_text, model_for_kg)
                                
                                # 2. CREI IL LEGAME (Questa è la riga che chiedevi):
                                # Serve a ricordarsi: "quando il thread 'fut' finisce, i risultati vanno a questi chunk"
                                futures_kg[fut] = indexed_chunks 
                            else:
                                # Se la pagina è stata scartata dal filtro (es. Tier B poco denso),
                                # saltiamo semplicemente al prossimo ciclo senza fare nulla.
                                continue                   
