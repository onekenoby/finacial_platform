...
VISION_MODEL_NAME = os.getenv("VISION_MODEL_NAME", "qwen/qwen3-vl-8b")
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict]:
    ...
    for page_no in range(len(doc)):
        ...
        if PDF_VISION_ONLY_IF_TEXT_SCARSO:
            allow_vision = len(page_text) < PDF_MIN_TEXT_LEN_FOR_NO_VISION
                ...
                try:
                    ...
                    if cached:
                        ...
                        if vis and isinstance(vis, dict):
                            ...
                            desc_ai = f"[{kind}] " + "; ".join([str(x) for x in key_points[:8]])
                            if numbers:
                            ...
                            if entities:
                                desc_ai += " | entities: " + "; ".join([
                                    f"{e.get('type','Entity')}:{e.get('label','')}" for e in entities[:10]
                                ])
                        else:
                            desc_ai = ""
        ...
        for ch in split_paragraphs(normalize_ws(combined_raw), CHUNK_MAX_CHARS, CHUNK_OVERLAP_CHARS):
        ...
        for batch_start in range(0, len(chunks), EMBED_BATCH_SIZE):
            ...
            for local_i, ch in enumerate(batch):
                ...
                if kg_done < MAX_KG_CHUNKS_PER_DOC and len(text_sem) >= KG_MIN_LEN:
                    if is_keyword_candidate(text_sem):
                        should_try_kg = True
