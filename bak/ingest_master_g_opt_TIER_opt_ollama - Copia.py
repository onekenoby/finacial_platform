...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "150")) #
...
def is_keyword_candidate(text: str) -> bool:
    """Trigger ibrido: attiva se vede Nomi Propri O Keyword Finanziarie."""
    if not text or len(text) < 150: return False
    
    clean_text = safe_normalize_text(text)
    
    # 1. Controllo Nomi Propri (Cina, Canada, Huawei...)
    proper_nouns = _ENTITY_PROPER_NOUNS.findall(clean_text)
    if len(set(proper_nouns)) >= 2:
        return True
        
    # 2. Controllo Keyword Finanziarie (risk, dazi, inflation...)
    return bool(_KG_PAT.search(clean_text))
...
# ===== Neo4j part - VERSIONE PULITA E CORRETTA =====
                ...
                # B) Gestione KG (Testo e Vision) - QUI USIAMO LA NORMALIZZAZIONE
                elif KG_ENABLED and toon_type in ("image_vision", "text"):
                    analysis_text = safe_normalize_text(ch["text_sem"])
                    
                    # Il trigger ora vede "Ottawa" invece di "OƩawa"
                    if is_keyword_candidate(analysis_text) and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                        kg_nodes, kg_edges = llm_extract_kg(filename, page_no, analysis_text)
                        kg_chunks_used += 1

                    # C) Caricamento finale record Neo4j
                    if NEO4J_ENABLED:
                        neo4j_rows.append({
