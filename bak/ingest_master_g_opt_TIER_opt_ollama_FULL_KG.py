...
def llm_extract_kg(filename: str, page_no: int, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    js = safe_json_extract(raw)
    if not js:
        # retry: prompt più “rigido” e input accorciato
        kg_text2 = f"SOURCE: {filename} p.{page_no}\nCONTENT:\n{normalize_ws(text)[:5000]}"
        raw2 = llm_chat(
...
def extract_pdf_text_by_page_pdfminer(file_path: str) -> list[str]:
    """
    Estrae testo per pagina usando pdfminer.six.
    - 1 pass sul file
    - restituisce una lista: pages[i] = testo pagina i
    """
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        pdfminer_pages = []
        use_pdfminer = (PDF_TEXT_EXTRACTOR == "pdfminer")
        if use_pdfminer:
            t0 = time.time()
            pdfminer_pages = extract_pdf_text_by_page_pdfminer(file_path)
            # se fallisce o non matcha il numero pagine, fallback a fitz
            if not pdfminer_pages or len(pdfminer_pages) != len(doc):
                pdfminer_pages = []
                use_pdfminer = False
        ...
        for page_idx in range(total_pages):
            ...
            if use_pdfminer:
                page_text = normalize_ws(pdfminer_pages[page_idx])
            else:
                page_text = normalize_ws(page.get_text("text"))
    ...
    try:
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            for j, ch in enumerate(batch):
                ...
                # ===== Neo4j part (FULL COVERAGE - NO FILTERS) =====
                # Obiettivo: esplodere OGNI chunk in nodi/relazioni (KG), senza duplicazioni.
                kg_nodes, kg_edges = [], []

                # A) Formule (deterministiche) -> tabella dedicata
                if toon_type == "formula_page":
                    ...
                    for f in formulas[:VISION_MAX_FORMULAS_PER_PAGE]:
                        ...
                        neo4j_formula_rows.append({
                            ...
                            "latex_raw": raw_latex,
                            "plain": f.get("plain"),
                            "meaning_it": f.get("meaning_it"),
                            "keywords": f.get("keywords", []),
                            "page_no": page_no,
                            "filename": filename,
                        })

                # B) KG (sempre) -> testo normalizzato per stabilità (ma salviamo in DB l'originale)
                if KG_ENABLED and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                    clean = safe_normalize_text(ch.get("text_sem", ""))
                    clean = CID_RE.sub(" ", clean).strip()
                    clean = " ".join(clean.split())

                    # Se troppo corto, aggiungiamo un wrapper per forzare entità minime
                    if clean and len(clean) < KG_MIN_LEN:
                        clean = (
                            "Estrai entità e relazioni anche se il testo è breve. "
                            "Se non trovi relazioni, crea comunque 1-3 entità coerenti e 0-2 relazioni.\n\n"
                            f"TESTO:\n{clean}"
                        )

                    # Se empty, prova comunque con text_raw (fallback)
                    if not clean:
                        fallback = (ch.get("text_raw") or "")
                        clean = safe_normalize_text(fallback)
                        clean = CID_RE.sub(" ", clean).strip()
                        clean = " ".join(clean.split())

                    if clean:
                        kg_nodes, kg_edges = llm_extract_kg(filename, page_no, clean)
                        kg_edges = canonicalize_edges(kg_edges)
                    else:
                        kg_nodes, kg_edges = [], []
