...
def dispatch_document(file_path: str, root_dir: str) -> dict:
    ...
    # FIX: Se non c'è una sottocartella per l'ontology, prova a ricavarla 
    # dal sidecar o usa un valore più descrittivo del Tier
    if len(parts) >= 2:
        ontology = parts[1]
    else:
        # Esempio: se sei in TIER_A_METHODOLOGY, usa 'methodology' come ontology
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    VERSIONE FIXATA: 
    - toon_type mappati su 'formula' e 'immagine'
    - Salvataggio garantito in ingestion_images con log di conferma
    - Rimozione filtri restrittivi LLM
    """
    ...
    try:
        ...
        def process_page_worker(p_idx):
            ...
            do_f = PDF_VISION_ENABLED and (is_math_page or is_low_text_page)
            do_c = PDF_VISION_ENABLED and (is_visual_page or has_imgs or is_low_text_page)
                    ...
                    try:
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_f:
                                ...
                                # Se l'AI trova formule o se la pagina è "low text" (forziamo salvataggio)
                                if (f_js and f_js.get("has_formulas")) or is_low_text_page:
                                    img_id = pg_save_image(log_id, full_jpg, "image/jpeg", f"Formula p.{p_no}", cur=t_cur)
                                    print(f"      📸 [PG] Immagine FORMULA salvata (ID: {img_id}) - Pagina {p_no}")
                                    
                                    sem = build_formula_semantic_chunk(p_no, f_js) if f_js else f"Formula rilevata a pagina {p_no}"
                                    local_results.append({
                                        "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                        "toon_type": "formula", # <-- FIX: NOME CORRETTO
                                        "section_hint": s_hint,
                                        "image_id": img_id,
                            ...
                            if do_c:
                                # Priorità ai ritagli delle immagini reali
                                if has_imgs:
                                    for img_meta in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                        ...
                                        if len(crop_jpg) >= MIN_IMAGE_BYTES:
                                            ...
                                            # Salviamo sempre se è un'immagine reale estratta
                                            img_id = pg_save_image(log_id, crop_jpg, "image/jpeg", f"Crop p.{p_no}", cur=t_cur)
                                            print(f"      📸 [PG] Immagine CROP salvata (ID: {img_id}) - Pagina {p_no}")
                                            
                                            sem = build_chart_semantic_chunk(p_no, c_js or {}, prefix="IMAGE")
                                            local_results.append({
                                                "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                                "toon_type": "immagine", # <-- FIX: NOME CORRETTO
                                                "section_hint": s_hint,
                                                "image_id": img_id,
                                                "metadata_override": c_js
                                            })
                                
                                # Fallback per grafici vettoriali (es. Figura 2.1 nel tuo PDF)
                                elif is_visual_page or is_low_text_page:
                                    full_jpg = render_full_page_jpeg(p_obj, dpi=VISION_DPI)
                                    c_js = extract_chart_via_vision(full_jpg)
                                    
                                    img_id = pg_save_image(log_id, full_jpg, "image/jpeg", f"Visual Full p.{p_no}", cur=t_cur)
                                    print(f"      📸 [PG] Immagine FULL PAGE salvata (ID: {img_id}) - Pagina {p_no}")
                                    
                                    sem = build_chart_semantic_chunk(p_no, c_js or {}, prefix="VISUAL")
                                    local_results.append({
                                        "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                        "toon_type": "immagine", # <-- FIX: NOME CORRETTO
                                        "section_hint": s_hint,
...
def process_single_file(file_path: str, doc_type: str, doc_meta: dict):
    ...
    try:
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            # --- DENTRO process_single_file (Loop dei batch) ---
            for j, ch in enumerate(batch):
                ...
                # FIX: Aggiungi toon_type e image_id al payload!
                payload = {
                    "tier": tier, 
                    "ontology": ontology, 
                    "filename": filename, 
                    "page": p_no, 
                    "toon_type": ch.get("toon_type", "text"), # <--- FONDAMENTALE
                    "image_id": ch.get("image_id"),           # <--- FONDAMENTALE PER LA GUI
                ...
                # Il toon_type qui come terzo elemento popola la colonna dedicata in Postgres
