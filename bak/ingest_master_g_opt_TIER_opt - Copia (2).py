...
SET c.chunk_index = r.chunk_index,
    ...
    c.page = r.page_no,
    c.text = left(r.text_sem, 800),
    c.section_hint = coalesce(r.section_hint, "")
MERGE (p)-[:HAS_CHUNK]->(c)

// 4) Entities: MENTIONED_IN verso Page (NON verso Chunk)
WITH r, p
CALL (r, p) {
...
// 5) Relazioni tra entità (come prima)
WITH r
CALL (r) {
