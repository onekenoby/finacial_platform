...
KG_TEXT_MAX_CHARS = int(os.getenv("KG_TEXT_MAX_CHARS", "3000"))   # chars sent to KG model per page
KG_MAX_TRIPLES = int(os.getenv("KG_MAX_TRIPLES", "30"))           # soft cap (sanitize already caps)
KG_TIMEOUT = int(os.getenv("KG_TIMEOUT", "90"))                   # seconds per KG task/page
...
{{
  ...
  "formulas": [
    {{
      ...
      "meaning_it": "MANDATORY: Provide an in-depth explanation in Italian of the mathematical and financial significance of the formula. Describe its purpose (e.g., moving averages, volatility, trading signals) and how it should be interpreted for analysis.",
      "keywords": ["finance_term1", "math_term2"],
      "variables": [
        {{"name": "alpha", "meaning": "Smoothing parameter used to weight recent data (e.g., in EMA calculations)."}},
        {{"name": "P_t", "meaning": "Price of the asset at time t, representing the current observation in the time series."}}
      ]
    }}
  ],
  "notes": ["uncertainties"]
...
Rules:
- meaning_it: MUST be in Italian and highly descriptive.
...

{
  ...
  "summary_it": "MANDATORY: Provide a detailed analytical and discursive description in Italian (min. 5 lines). Describe the image content, key trends, peak values, logical conclusions, and the specific financial/technical context for RAG indexing."
...
Your job: output ONLY what is clearly visible in the image.
You may transcribe clearly readable text (title/subtitle/legend/source).
You may describe purely visual comparisons/trends (higher/lower, increase/decrease).
You MUST NOT guess or estimate any numeric values not explicitly written.

ABSOLUTE PROHIBITIONS:
- Do NOT invent numbers, percentages, dates, labels, countries, sources.
- Do NOT estimate values from bar heights or line positions.
- Do NOT provide causes, implications, or commentary.
- Do NOT assume what colors mean unless a legend explicitly states it.

IMPORTANT:
- If both a MAIN TITLE and a SUBTITLE are visible, put them in the correct fields.
- If years are visible, list them EXACTLY as written (no guessing).
- If the legend shows colors meaning YEAR (or category), state that explicitly.
- If legend is not readable, do NOT infer what colors mean.

If something is present but not readable, write "NOT READABLE".
...

{
  ...
  "title": "MAIN title text if clearly visible, else 'NOT READABLE'",
  "subtitle": "subtitle text if clearly visible, else 'NOT READABLE'",

  "source": "clearly readable source text or 'NOT READABLE'",
  "timeframe": "years/time period exactly as written, else 'NOT READABLE'",

  "what_is_visible_it": "2–6 lines in ITALIAN describing ONLY visible elements",
  "observations_it": [
    "visible structural facts (axes, legend, categories, labels) - NO guesses"
  ],
  "visual_trends_it": [
    "purely visual comparisons/trends (NO numbers, NO causes)"
  ],

  "legend_it": {
    "is_readable": true,
    "mapping": [
      {"label": "legend label exactly as written", "color_or_style": "description exactly as visible"}
    ]
  },

  "numbers": [
    {"label":"visible label","value":"visible number","unit":"visible unit or ''","period":"visible period or ''"}
  ],

  "unreadable_parts": ["elements present but not readable"],
  "confidence": 0.0
}

STRICT RULES:
- Fill 'numbers' ONLY if numeric values are explicitly written and clearly readable.
- If no numbers are readable, return [].
- Do NOT translate bars into numbers.
- If legend_it.is_readable is false, legend_it.mapping must be [] and you MUST NOT infer what colors represent.
- confidence 0.2–0.4: titles readable but values unclear
  0.5–0.7: titles, categories, legend readable; trends visible
  0.8–1.0: all elements readable
"""
...
You are given:
- VISION_JSON: a STRICT visual extraction (title, source, timeframe, visible elements, unreadable parts, confidence)
- PAGE_TEXT: raw page text (may be noisy or incomplete)

TASK:
- Write a qualitative financial analysis in ITALIAN, using natural and professional language.
- Base your reasoning ONLY on what is visible or explicitly stated in VISION_JSON.
- Do NOT invent numbers, rankings, or quantitative comparisons.
- Focus on what the chart is trying to highlight at a strategic or narrative level.
- Mention limitations ONCE, then move on
- Focus on intent, framing, relevance
- Human financial analyst tone
- Avoid repetitive hedging expressions; use conditional language sparingly.

IMPORTANT:
- If confidence is low or numbers are missing, mention this ONCE and move on.
- Avoid repeating methodological limitations.
- Prefer explaining the possible intent, framing, and relevance of the chart.

STYLE:
- Human, analytical, and concise.
- Like a financial analyst explaining a chart to a colleague.
        ...
        for idx, para in enumerate(raw_paras):
            ...
            if img_matches and PDF_VISION_ENABLED:
                ...
                if os.path.exists(full_img_path) and os.path.getsize(full_img_path) > 5000:
                    ...
                    if c_js and (c_js.get("summary_it") or c_js.get("kind") != "other"):
                        # build_chart_semantic_chunk userà summary_it per l'embedding
                        chunk_data["text_sem"] = build_chart_semantic_chunk(chunk_data["page_no"], c_js)
                        chunk_data["metadata_override"] = c_js
                        print(f"   📝 Analisi Semantica generata: {c_js.get('summary_it')[:50]}...")     
...
def kg_text_is_meaningful(text: str) -> bool:
    """
    Decide se un testo è abbastanza 'reale' da giustificare
    una chiamata LLM di estrazione KG.

    Evita:
    - pagine solo immagini / markdown
    - OCR rumore
    - testo troppo corto o simbolico
    """
    if not text:
        return False
    ...
    # rimuove immagini markdown ![alt](path)
    t = re.sub(r"!\[[^\]]*\]\([^)]+\)", " ", t)

    # rimuove link markdown [x](y)
    t = re.sub(r"\[[^\]]*\]\([^)]+\)", " ", t)
...
// 3) Chunk (Perno della granularità)
WITH p, r
...
// 4) Entità: whitelist proprietà + PRESENT_IN
WITH r, c
CALL (r, c) {
  ...
  SET
      // --- WHITELIST PROPRIETÀ STABILI ---
      e.label = coalesce(e.label, n.label),
      e.name_norm = coalesce(n.props.name_norm, e.name_norm),
      e.aliases = coalesce(n.props.aliases, e.aliases),
      e.entity_class = coalesce(n.props.entity_class, e.entity_class),
      e.iso2 = coalesce(n.props.iso2, e.iso2),
      e.iso3 = coalesce(n.props.iso3, e.iso3),
      e.ticker = coalesce(n.props.ticker, e.ticker),
      e.wikidata_id = coalesce(n.props.wikidata_id, e.wikidata_id),
      e.chunk_freq = coalesce(e.chunk_freq, 0) + 1,
      e.last_seen = datetime()

  // Aggiorna kinds senza duplicati
  FOREACH (_ IN CASE
...
// 5) Relazioni tra entità (SAFE, NO COLLAPSE, CON EVIDENCE)
WITH r, c
CALL (r, c) {
  UNWIND coalesce(r.edges, []) AS rel
  WITH rel, r, c, coalesce(rel.props, {}) AS props
  WHERE rel.source IS NOT NULL
    AND rel.target IS NOT NULL
    AND rel.source <> rel.target
    AND rel.relation IS NOT NULL
    AND rel.relation <> ""

  MERGE (s:Entity {id: rel.source})
  MERGE (t:Entity {id: rel.target})

  // Edge unico per (source, target, predicate)
  MERGE (s)-[r_out:RELATES_TO {type: rel.relation}]->(t)

  SET
      // --- METADATI FINANCE-SAFE ---
      r_out.confidence = coalesce(props.confidence, r_out.confidence),
      r_out.evidence_chunk_id = coalesce(props.evidence_chunk_id, c.id, r_out.evidence_chunk_id),
      r_out.page_no = coalesce(props.page_no, c.page, r_out.page_no),
      r_out.doc_id = coalesce(props.doc_id, r.doc_id, r_out.doc_id),

      r_out.count = coalesce(r_out.count, 0) + 1,
      r_out.last_seen = datetime(),

      // Raw evidence corta (anti-rumore)
      r_out.raw_relation = substring(
          coalesce(props.raw_relation, rel.relation),
          0,
          280
      )
...
def extract_chart_via_vision(img_bytes: bytes, context_hint: str = "") -> Optional[Dict[str, Any]]:
    """
    Grounded Vision Extraction (anti-hallucination) with OCR evidence:
    - always defines ocr_text
    - JSON forced via llm_chat_multimodal (format=json, temp=0)
    - cache enabled
    """
    if not img_bytes:
        return None
    ...
    # ✅ ocr_text is ALWAYS defined
    ocr_text = ""
    try:
        ocr_text = ocr_extract_text(vbytes) or ""
    except Exception as e:
...
"""
    ...
    # if low readability => drop numbers
    if conf < 0.45:
        js["numbers"] = []
...
def build_formula_semantic_chunk(page_no: int, formulas_json: Dict[str, Any]) -> str:
    """
    Produce un testo “semantico” delle formule estratte dalla pagina,
    da concatenare al chunk/testo pagina per migliorare embedding + RAG.
    """
    formulas = (formulas_json or {}).get("formulas") or []
    if not formulas:
        return ""

    max_n = int(globals().get("VISION_MAX_FORMULAS_PER_PAGE", 12))

    lines = [f"--- FORMULE / EQUAZIONI - Pagina {page_no} ---"]
    for f in formulas[:max_n]:
        latex = (f.get("latex") or "").strip()
        plain = (f.get("plain") or "").strip()
        meaning = (f.get("meaning_it") or f.get("meaning") or "").strip()

        if latex:
            lines.append(f"Formula LaTeX: ${latex}$")
        if plain:
            lines.append(f"Formula (testo): {plain}")
        if meaning:
            lines.append(f"Significato (IT): {meaning}")

        lines.append("")  # separatore tra formule
...
def llm_extract_kg(filename: str, page_no, text: str, model_name: str):
    ...
    def _extract_balanced_nodes_prefix(raw: str) -> Optional[str]:
        """
        Ripara JSON troncato dentro "nodes":[ ... ] chiudendo
        correttamente l'array e aggiungendo edges vuote.
        """
        if not raw:
            return None
        s = raw.strip()
        if not s.startswith("{") or '"nodes"' not in s:
            return None
        ...
        while i < len(s):
            ...
            if in_str:
                ...
                if ch == '"':
                    ...
                    return None  # array già chiuso → niente repair
            i += 1

        if last_end is None:
            return None

        prefix = s[: last_end + 1].rstrip()
        return prefix + "\n  ],\n  \"edges\": []\n}"
    ...
    def _call_llm(payload_text: str, *, num_predict: int, num_ctx: int) -> str:
        """
        Chiamata Ollama via HTTP con timeout REALE.
        Così non resti mai appeso su chat().
        """
        ...
        # timeout: (connect, read). read <= KG_TIMEOUT - margine
        read_timeout = max(10, int(globals().get("KG_TIMEOUT", 90)) - 5)

        try:
            r = requests.post(url, json=body, timeout=(5, read_timeout))
            r.raise_for_status()
            data = r.json()
            return (data.get("message", {}) or {}).get("content", "") or ""
        except requests.exceptions.Timeout:
            print(f"   ⚠️ [KG-CHAT-TIMEOUT] {base} p{page_no_int} (>~{read_timeout}s)")
            return ""
        except Exception as e:
    ...
    num_ctx = int(globals().get("KG_NUM_CTX", 4096))
    ...
    if (not nodes and not edges):
        if not kg_text_is_meaningful(text or ""):
            print(f"   ⏭️ [KG-SKIP-RETRY-EMPTY] {base} p{page_no_int}: non-meaningful text")
            return [], []
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Thread-safe version:
    - ogni worker apre il PDF e carica la sua pagina (niente doc condiviso tra thread)
    - evita crash random di PyMuPDF in parallelo
    """
    out_chunks: List[Dict[str, Any]] = []
    filename = os.path.basename(file_path)

    # reset stats per questo documento (facoltativo ma consigliato)
    with _VISION_STATS_LOCK:
        VISION_STATS["pages_total"] = 0
        VISION_STATS["pages_with_imgs"] = 0
        VISION_STATS["pages_crop_only"] = 0
        VISION_STATS["pages_fullpage"] = 0

    # leggi numero pagine una volta
    ...
    def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
        ...
        # IMPORTANT: doc isolato per thread
        ...
        try:
            ...
            is_struct = is_structural_page(p_text, p_no=p_no)
            ...
            if do_f or do_c:
                ...
                try:
                    with t_conn.cursor() as t_cur:
                        ...
                        # A) FORMULE
                        if do_f:
                            full_jpg = render_full_page_jpeg(p_obj, dpi=FULLPAGE_DPI)

                            # SAFE STUB: formula extraction not implemented
                            f_js_raw = None
                            f_js = {"has_formulas": False, "formulas": []}

                            # If you still want to keep the full-page image when text is low, keep this condition:
                            if is_low_text_page:
                                img_id = pg_save_image(
                                    log_id, full_jpg, "image/jpeg",
                                    f"FullPage p.{p_no} (low-text)", cur=t_cur
                                )

                                sem_desc = f"Pagina {p_no}: contenuto prevalentemente visivo (testo scarso). Possibile presenza di formule/diagrammi non estratti automaticamente."

                                local_results.append({
                                    "text_raw": f"Pagina {p_no} (low-text image)",
                                    "text_sem": sem_desc,
                                    "page_no": p_no,
                                    "toon_type": "page_image",
                                    "section_hint": s_hint,
                                    "image_id": img_id,
                        ...
                        if do_c:
                            ...
                            if has_imgs:
                                ...
                                for img_meta in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                    bbox = img_meta["bbox"]
                                    crop_pix = p_obj.get_pixmap(clip=bbox, dpi=CROP_DPI)
                                    crop_jpg = crop_pix.tobytes("jpg")

                                    if len(crop_jpg) < MIN_IMAGE_BYTES:
                                        continue

                                    c_js = extract_chart_via_vision(crop_jpg) or {}
                                    img_id = pg_save_image(
                                    ...
                                    local_results.append({
                                        "text_raw": f"Immagine: {c_js.get('kind','visual')} - {c_js.get('title','')}",
                                        "text_sem": build_chart_semantic_chunk(p_no, c_js, prefix="IMAGE"),
                                        "page_no": p_no,
                                        "toon_type": "immagine",
                                        "image_id": img_id,
                                    ...
                                    elif c_js.get("summary_it"):
                                        found_valid_crop = True

                            need_fullpage = (not found_valid_crop) or is_low_text_page or (is_visual_page and not has_imgs)
                            if need_fullpage:
                                with _VISION_STATS_LOCK:
                                    VISION_STATS["pages_fullpage"] += 1

                                full_jpg = render_full_page_jpeg(p_obj, dpi=VISION_DPI)
                                c_js = extract_chart_via_vision(full_jpg) or {}
                                ...
                                if c_js.get("kind") != "other":
                                    ...
                                    local_results.append({
                                        "text_raw": f"Grafico/Pagina: {c_js.get('title','Senza titolo')} ({c_js.get('kind','visual')})",
                                        "text_sem": build_chart_semantic_chunk(p_no, c_js, prefix="VISUAL"),
                                        "page_no": p_no,
                                        "toon_type": "immagine",
                                        "image_id": img_id,
                                        "metadata_override": c_js
    ...
    with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as p_executor:
        ...
        for fut in as_completed(futures):
            try:
                ...
                done = len(out_chunks)  # progress “indicativo”
            except Exception as e:
...
def process_virtual_md_chunks(content: str, asset_park: dict, filename: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    for para in raw_paras:
        ...
        for sub_p in sub_segments:
            ...
            img_match = re.search(r'!\[.*?\]\((img_.*?\.jpg)\)', clean_sub_p)
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    if ext == ".pdf":
        return extract_pdf_as_markdown_assets(file_path, log_id)
    elif ext == ".md":
...
def process_single_file(file_path: str, source_type: str, doc_meta: dict):
    ...
    for i in range(0, len(chunks), EMBED_BATCH_SIZE):
        ...
        # ===============================
        # KG per-page (indexed globally)
        # ===============================
        # ===============================
        # KG per-page (indexed globally) - FIXED
        # ===============================
        ...
        if KG_ENABLED:
            ...
            for p_no, indexed_chunks in pages_map.items():
                ...
                ent = get_entity_density(text_for_ai)
                fin = count_unique_keywords(text_for_ai)

                if ent < MIN_ENTITY_DENSITY and fin < MIN_FIN_KEYWORDS:
                    continue
                if not is_keyword_candidate_hybrid(text_for_ai):
                    continue
            ...
            # collect results (OUTSIDE the submit loop)
            for p_no, fut in futures_kg.items():
                ...
                try:
                    ...
                    print(f"   ⚠️ KG ERROR p.{p_no} | {type(ex).__name__}: {ex}")
        ...
        for j, ch in enumerate(batch):
            ...
            pg_rows.append((
                log_id,
                int(ch.get("chunk_index") or global_idx),
                ch.get("toon_type"),
                ch.get("text_raw"),
                ch.get("text_sem"),
                json.dumps(ch.get("metadata") or {}, ensure_ascii=False),
                chunk_id
            ))

            # Neo4j (Chunk + optional KG)
            ...
            neo4j_rows.append({
                ...
                "chunk_index": int(ch.get("chunk_index") or global_idx),
                "toon_type": ch.get("toon_type"),
                "text_sem": ch.get("text_sem") or "",
                "section_hint": ch.get("section_hint") or "",
                "ontology": ontology,
                "nodes": kg_nodes,
                "edges": kg_edges
            })
