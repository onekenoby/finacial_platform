...
def get_entity_density(text: str) -> int:
    ...
    # Rimuoviamo i tag strutturali dal conteggio per non falsare la densità
    ...
    # Un'entità è valida se non appare mai in minuscolo nel chunk
    true_entities = {w for w in set(potential_entities) if w.lower() not in common_vocab}
    return len(true_entities)


    # 1. Estrazione di tutte le parole in Title Case (minimo 3 caratteri)
    # Il pattern [A-ZÀ-Ú] copre i principali set di caratteri internazionali
    potential_entities = re.findall(r'\b[A-ZÀ-Ú][a-zà-ú]{2,}\b', text)
    
    # 2. Creazione di un set di tutte le parole che appaiono in minuscolo nel testo
    # Questo è il nostro "dizionario dinamico di parole comuni" per questo chunk
    common_vocab = set(re.findall(r'\b[a-zà-ú]{3,}\b', text))
    
    # 3. FILTRO STATISTICO:
    # Un'entità è valida SOLO SE non appare mai in minuscolo nello stesso chunk.
    # - "Dazi" (inizio frase) vs "dazi" (testo) -> SCARTATO (parola comune)
    # - "Gemma" (modello) vs "gemma" (pietra) -> Se compaiono entrambi, scartiamo per prudenza
    # - "Lagarde" vs (nessun 'lagarde' minuscolo) -> ACCETTATO (entità vera)
    true_entities = {
        word for word in set(potential_entities) 
        if word.lower() not in common_vocab
    }
...
def ai_vision_gatekeeper(image_bytes: bytes) -> bool:
    """Chiede a Gemma 3 se l'immagine è un grafico o contiene dati utili."""
    if not image_bytes: return False
    prompt = "Is this image a financial chart, table or data diagram? Answer ONLY YES or NO."
    try:
        raw = llm_chat_multimodal(prompt, image_bytes, VISION_MODEL_NAME, max_tokens=2)
        return "YES" in raw.upper()
    except: return True # Nel dubbio, la teniamo
        ...
        for idx, para in enumerate(raw_paras):
            ...
            if img_matches and PDF_VISION_ENABLED:
                ...
                if os.path.exists(full_img_path) and os.path.getsize(full_img_path) > 5000:
                    ...
                    if c_js and c_js.get("kind") != "other":
                        chunk_data["toon_type"] = "immagine"
                        chunk_data["text_sem"] = build_chart_semantic_chunk(1, c_js, prefix="MD_IMG")
...
def llm_extract_kg(filename: str, page_no, text: str, model_name: str):
    ...
    if js is None:
        # retry con testo corto
        short_text = (text or "")[:1800]
        try:
            print(f"   🔁 [KG-RETRY] {base} p{page_no_int}")
            resp2 = chat(
                model=model_name,
                messages=[
                    {"role": "system", "content": KG_PROMPT},
    ...
    # normalize + sanitize
    # ------------------------------------------------------------
    js = _normalize_graph_schema(js)
    if js is None:
        return [], []

    try:
        nodes, edges = _sanitize_graph(js)
    except Exception as e:
...
def extract_pdf_as_markdown_assets(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    for p_idx in range(total_pages):
        ...
        if is_structural_page(p_text): continue
        
        p_text = normalize_ws(p_text)
        virtual_md_content.append(f"\n# PAGE {p_no}\n{p_text}")

        # ESTRAZIONE IMMAGINI CON PARAMETRO DINAMICO
...
def process_virtual_md_chunks(content: str, asset_park: dict, filename: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Versione 2.8 - ULTIMATE:
    Gestisce la conversione MD -> Chunks con salvataggio atomico delle immagini.
    """
    out_chunks = []
    # Pulizia strutturale per evitare indici e sommari
    ...
    for idx, para in enumerate(raw_paras):
        ...
        # Bypass lunghezza per i tag immagine: non scartiamo gli asset visivi
        has_image_tag = "![" in para and "](img_" in para
        if len(para_strip) < MIN_CHUNK_LEN and not has_image_tag:
            continue

        chunk_data = {
            "text_raw": para_strip,
            "text_sem": f"Doc: {filename} | Content: {para_strip[:60]}...\n{para_strip}",
            "page_no": 1, 
            "toon_type": "text",
            "section_hint": find_section_hint(para_strip)
        }

        page_match = re.search(r'# PAGE (\d+)', para)
        if page_match:
            chunk_data["page_no"] = int(page_match.group(1))

        # --- LOGICA ASSET VISUALI ---
        img_match = re.search(r'!\[.*?\]\((img_.*?\.jpg)\)', para)
        if img_match and PDF_VISION_ENABLED:
            asset_id = img_match.group(1)
            img_bytes = asset_park.get(asset_id)
            
            if img_bytes and len(img_bytes) >= MIN_ASSET_SIZE:
                if ai_vision_gatekeeper(img_bytes):
                    chunk_data["toon_type"] = "immagine" 
                    print(f"   🎯 Asset {asset_id} validato. Salvataggio in Postgres...")
                    
                    # Salvataggio immediato in Postgres
                    conn = pg_get_conn()
                    try:
                        with conn.cursor() as cur:
...
def process_single_file(file_path: str, doc_type: str, doc_meta: dict):
    """
    Versione 2.9 - PRODUCTION:
    - Esplosione KG forzata per News e Immagini.
    - Log degli errori esplicito per debugging.
    """
    _ = get_qdrant_client()
    _ = get_embedder()
    ...
    log_phase(filename, "Extraction", _ms(t_extract))
    ...
    try:
        ...
        if not valid_chunks:
            ...
            print(f"   ⚠️ {filename}: Nessun chunk valido trovato.")
            return

        qdrant_points, pg_rows, neo4j_rows, neo4j_formula_rows = [], [], [], []
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            if KG_ENABLED:
                ...
                for p_no, indexed_chunks in pages_map.items():
                    ...
                    # Gating più intelligente: entità OR keyword finanziarie
                    entity_count = get_entity_density(text_for_ai)
                    has_fin_keys = count_unique_keywords(text_for_ai) >= 2 # Almeno 2 concetti finance
                    is_struct = is_structural_page(combined_text, p_no=p_no)
                    has_image = any(c[1].get("toon_type") == "immagine" for c in indexed_chunks)

                    if is_struct:
                        should_extract_kg = False
                    elif p_no == 1:
                        should_extract_kg = len(text_for_ai) > 50
                    else:
                        # Permissività aumentata: basta sostanza testuale O immagini O keyword
                        should_extract_kg = (len(text_for_ai) > 100 and (entity_count >= 1 or has_fin_keys)) or has_image
                        
                        # Il gatekeeper interviene solo se non siamo sicuri (testo medio-breve e no immagini)
                        if should_extract_kg and not has_image and len(text_for_ai) < 1500:
                            should_extract_kg = ai_gatekeeper_decision(text_for_ai)

                    # LOG DI VERIFICA GATING
                    print(f"   [KG-GATE] p{p_no} len={len(text_for_ai)} ent={entity_count} fin={has_fin_keys} img={has_image} -> {'✅' if should_extract_kg else '❌'}")

                    if should_extract_kg:
                        fut = kg_executor.submit(llm_extract_kg, filename, p_no, text_for_ai, model_for_kg)
                ...
                if futures_kg:
                    for fut in as_completed(futures_kg):
                        ...
                        try:
                            ...
                            print(f"   ⚠️ KG Task error: {ex}")

            for j, ch in enumerate(batch):
                c_idx, p_no = i + j, ch.get("page_no", 1)
                t_type, img_id = ch.get("toon_type", "text"), ch.get("image_id")
                text_s = ch["text_sem"][:QDRANT_TEXT_MAX_CHARS]
                
                # Includiamo img_id nell'ID deterministico
                c_uuid = deterministic_chunk_id(doc_id, p_no, c_idx, t_type, text_s, img_id)

                payload = {
                    "tier": tier, "ontology": ontology, "filename": filename, 
                    "page": p_no, "toon_type": t_type, "image_id": img_id, 
                    "text_sem": text_s, "effective_date": effective_date
                }
                ...
                if NEO4J_ENABLED:
                    # Nodi Formula
                    if t_type == "formula":
                        f_meta = ch.get("metadata_override", {})
                        for f_item in f_meta.get("formulas", []):
                            f_latex = f_item.get("latex", "")
                            if f_latex:
                                f_id = sha256_hex(f_latex.encode("utf-8"))[:24]
                                neo4j_formula_rows.append({
                                    "chunk_id": c_uuid, "fid": f_id, "latex": f_latex, 
                                    "plain": f_item.get("plain", ""), "meaning_it": f_item.get("meaning_it", ""),
                                    "page_no": p_no, "filename": filename, "keywords": f_item.get("keywords", [])
                                })

                    # Nodi Entità (Esplosione)
                    kg_nodes, kg_edges = batch_kg_results.get(j, ([], []))
                    neo4j_rows.append({
                        "doc_id": doc_id, "filename": filename, "log_id": log_id,
                        "chunk_id": c_uuid, "chunk_index": c_idx, "toon_type": t_type, 
                        "page_no": p_no, "text_sem": ch["text_sem"], "ontology": ontology,
                        "nodes": kg_nodes, "edges": kg_edges,
                    })

            # Flush dei batch
            if len(qdrant_points) >= DB_FLUSH_SIZE:
                def run_flush(q, p, n, nf):
                ...
                flush_bg_executor.submit(run_flush, list(qdrant_points), list(pg_rows), list(neo4j_rows), list(neo4j_formula_rows))
                qdrant_points.clear(); pg_rows.clear(); neo4j_rows.clear(); neo4j_formula_rows.clear()
        ...
        if neo4j_formula_rows: flush_neo4j_formulas_batch(neo4j_formula_rows)

        # FINE ESECUZIONE: Se arriviamo qui, stampiamo il tempo
        # ... dopo flush_bg_executor.shutdown(wait=True) ...
        elapsed = int((time.time() - start_time) * 1000)
        pg_close_log(log_id, "COMPLETED", len(valid_chunks), elapsed)
        
        # SPOSTA QUI: Fuori dalle indentazioni pericolose
        print(f"\n✅ SUCCESS: {filename} completato in {elapsed}ms")
        shutil.move(file_path, os.path.join(PROCESSED_DIR, filename))

    except Exception as e:
        print(f"❌ ERRORE CRITICO in {filename}: {e}")
