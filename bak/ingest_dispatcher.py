...
# --- CLASSIFIER PROMPT (LISTA COMPLETA) ---
CLASSIFIER_PROMPT = """
...
Reply ONLY with the single category name (e.g., "RISK", "LEGAL", "GENERIC").
"""
...
def classify_document(file_path: str) -> str:
    ...
    if not preview.strip():
        print(f"⚠️ Anteprima vuota per {filename}, default: GENERIC")
        return "generic"
    ...
    try:
        ...
        if "RISK" in category_raw or "COMPLIANCE" in category_raw: return "risk"
        if "STRATEGY" in category_raw or "MANAGEMENT" in category_raw: return "strategy"
        if "LEGAL" in category_raw or "REGULATORY" in category_raw: return "legal"
        if "PRODUCT" in category_raw or "SERVICE" in category_raw: return "products"
        if "SUSTAINABILITY" in category_raw or "ESG" in category_raw: return "sustainability"
        if "OPERATION" in category_raw: return "operations"
        if "TECHNOLOGY" in category_raw: return "technology"
        if "EDUCATION" in category_raw: return "educational"
        if "CLIENT" in category_raw: return "client"
        if "FINANCIAL" in category_raw: return "financial"
        
        return "generic"
    except Exception as e:
        print(f"⚠️ Errore classificazione AI: {e}. Default: GENERIC")
        return "generic"

# --- DB & AI HANDLERS (Standard) ---
...
def flush_postgres_batch(batch_data: List[Tuple]):
    ...
    try:
        ...
        """, batch_data)
        conn.commit()
    except Exception as e: conn.rollback(); print(f"   ⚠️ Postgres Batch Error: {e}")
    ...
    with neo4j_driver.session() as session:
        for item in batch_graph_data:
            ...
            for node in item.get("nodes", []):
                if "id" not in node or "type" not in node: continue
                try:
                    session.run("MERGE (c:Chunk {id: $cid, type: $dtype}) MERGE (n:Entity {name: $name}) ON CREATE SET n.type = $type MERGE (n)-[:MENTIONED_IN]->(c)", cid=chunk_id, dtype=doc_type, name=node['id'], type=node['type'])
                except: pass
            # Archi
            for edge in item.get("edges", []):
                ...
                try:
                    ...
                    session.run(f"MATCH (a:Entity {{name: $s}}), (b:Entity {{name: $t}}) MERGE (a)-[:{rel_type}]->(b)", s=edge['source'], t=edge['target'])
                except: pass
