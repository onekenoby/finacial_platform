...
You are a Scientific Document OCR Engine.
Your task is to transcribe the VISUAL CONTENT of the page image into structured JSON.

### CRITICAL RULES:
1.  **IGNORE CORRUPT TEXT**: The text layer of this PDF is broken (contains '2C0D', '•', etc.). IGNORE IT. Trust only the pixels you see.
2.  **FORMULAS**: Identify mathematical formulas. Transcribe them into standard **LaTeX** format.
    * Example: If you see a square root, write "\\sqrt{...}".
3.  **NO CHART HALLUCINATIONS**: Do NOT describe lists of formulas or tables as "charts" or "graphs".
4.  **TABLES**: If you see a grid of numbers, transcribe it as a Markdown Table.

### OUTPUT JSON FORMAT:
{
  "page_type": "string (formula_sheet | data_table | text | mixed)",
  "content": [
    {
      "type": "formula",
      "latex": "P_0 = D_1 / (r - g)",
    ...
    {
      "type": "text",
      "value": "Section title or plain text paragraph."
    }
  ]
}
"""
...
def llm_chat_multimodal(prompt: str, image_bytes: bytes, model: str, max_tokens: int = 3500) -> str:
    """
    Versione UFFICIALE OLLAMA LIBRARY.
    Più pulita e pythonica rispetto a requests.
    """
    if not image_bytes:
        return ""

    # 1. Conversione Base64 (Fondamentale anche con la libreria ufficiale)
    try:
    ...
    print(f"   📡 Invio Vision a {model}...", end="\r")

    try:
        # 2. Chiamata diretta alla libreria
        response: ChatResponse = chat(
            ...
            options={
                ...
                "num_predict": int(max_tokens),# Token output
                "num_ctx": 4096                # Finestra contesto per l'immagine
            }
        )
        
        # 3. Estrazione Contenuto (Compatibile con versioni recenti della lib)
        # La libreria ritorna un oggetto, non un dict
        if hasattr(response, 'message') and hasattr(response.message, 'content'):
            return response.message.content or ""
        elif isinstance(response, dict): # Fallback per vecchie versioni
            return response.get('message', {}).get('content', "") or ""
            
        return ""

    except Exception as e:
        # Se il modello non esiste, la libreria lancia un'eccezione chiara
        if "not found" in str(e).lower():
            print(f"   ❌ Modello '{model}' non trovato! Scaricalo con 'ollama pull {model}'")
        else:
            print(f"   ⚠️ Errore Ollama Lib: {e}")
        return ""
...
# ==============================================================================
# ==============================================================================
# MOTORE VISIONE QWEN (PURE VISION - NO HYBRID FALLBACK)
# ==============================================================================
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    print(f"   🚀 Ingestion Start: {total_pages} pagine | LLM={LLM_MODEL_NAME} | VLM={VISION_MODEL_NAME}")


    try:
        tags = requests.get("http://127.0.0.1:11434/api/tags", timeout=3).json()
        models = [m.get("name","") for m in (tags.get("models") or [])]
        if VISION_MODEL_NAME not in models:
            print(f"   ❌ Model NOT FOUND in Ollama: {VISION_MODEL_NAME}")
            print(f"   ✅ Available: {models[:10]}{' ...' if len(models)>10 else ''}")
    except Exception:
        pass
    ...
    def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            full_jpg = render_full_page_jpeg(p_obj, dpi=200)
            ...
            if t_conn:
                try:
                    with t_conn.cursor() as t_cur:
                        img_id = pg_save_image(log_id, full_jpg, "image/jpeg", f"Page_{p_no}", cur=t_cur)
                    t_conn.commit()
            ...
            ai_json_str = llm_chat_multimodal(
                ...
                model=VISION_MODEL_NAME, # Assicurati sia qwen3-vl-8b-instruct
                max_tokens=3500 # Pagine dense richiedono token
            )
            ...
            if f_js and f_js.get("content"):
                chunk_text += f"--- PAGINA {p_no} ({f_js.get('page_type', 'unknown')}) ---\n"
                
                for item in f_js.get("content", []):
                    ctype = item.get("type")
                    
                    if ctype == "formula":
                        # LaTeX pulito
                        latex = item.get("latex", "").replace("$", "") # Pulizia extra
                        desc = item.get("description", "")
                        chunk_text += f"\n$$ {latex} $$\n" 
                        if desc: chunk_text += f"_{desc}_\n"
                        
                    elif ctype == "table":
                        # Tabella Markdown
                        md_table = item.get("markdown", "")
                        cap = item.get("caption", "")
                        if cap: chunk_text += f"\n**Tabella: {cap}**\n"
                        chunk_text += f"{md_table}\n"
                ...
                # Fallback estremo se il JSON fallisce ma Qwen ha parlato
                if ai_json_str and len(ai_json_str) > 10:
                     chunk_text += f"--- RAW TRANSCRIPTION PAGINA {p_no} ---\n{str(ai_json_str)[:2000]}"
            ...
            if len(chunk_text) > 50:
                meta = {
                    ...
                    "metadata_override": {
                        # Passiamo il raw JSON per il KG o usi futuri
                        "vision_data": f_js
                    }
                }
