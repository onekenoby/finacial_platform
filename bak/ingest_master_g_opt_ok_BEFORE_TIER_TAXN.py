...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "320"))
MAX_KG_CHUNKS_PER_DOC = int(os.getenv("MAX_KG_CHUNKS_PER_DOC", "6"))

KG_KEYWORDS = [
    "risk", "guidance", "forecast", "revenue", "earnings", "inflation", "rate", "spread",
    "debt", "cash", "profit", "loss", "covenant", "margin", "ebitda", "capex", "dividend",
    "default", "rating", "algorithm", "model", "regression", "market", "basket", "lift", "confidence", "support",
    "grafico", "tabella", "trend", "asse", "legenda", "chart", "table", "axis", "legend"
]
...
CHART_CANDIDATE_PAT = re.compile(
    r"\b(chart|graph|figure|table|diagram|heatmap|candlestick|ohlc|volume|axis|legend|"
    r"grafico|tabella|figura|diagramma|candela|volumi|asse|legenda)\b",
    re.IGNORECASE
)
...
# =========================
# Neo4j: deep graph + deterministic formulas
# =========================
NEO4J_BATCH_QUERY = """
UNWIND $rows AS r
...
SET c.chunk_index = r.chunk_index, c.toon_type = r.toon_type, c.page = r.page_no, c.text = left(r.text_sem, 800),
...
def llm_extract_kg(filename: str, page_no: int, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    raw = llm_chat(KG_PROMPT, kg_text, LLM_MODEL_NAME, max_tokens=LLM_MAX_TOKENS)
    js = safe_json_extract(raw)
    if not js:
        return [], []
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for page_idx in range(total_pages):
            ...
            if PDF_EXTRACT_EMBEDDED_IMAGES and PDF_VISION_ENABLED:
                try:
                    ...
                    for k, img in enumerate(img_list):
                        ...
                        if executor is not None:
                            ...
                            futures.append(fut)
                            meta_for_future.append((page_no, section_hint, iid, page_text, ibytes))
                        else:
                            # solo ref
                            out_chunks.append({
            ...
            if futures:
                for idx, fut in enumerate(as_completed(futures)):
                    try:
                        vis = fut.result()
                        page_no2, section_hint2, iid2, page_text2, _ = meta_for_future[idx]
                        if not vis:
                            continue
    ...
    try:
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            for j, ch in enumerate(batch):
                ...
                pg_rows.append((log_id, c_idx, toon_type, ch["text_raw"], ch["text_sem"], Json(payload)))

                # ===== Neo4j part =====
                kg_nodes, kg_edges = [], []

                # 1) Formule: NIENTE LLM → Formula nodes deterministici
                if toon_type == "formula_page":
                    # crea una formula per ogni entry in meta_override.formulas
                    ...
                    for f in formulas[:VISION_MAX_FORMULAS_PER_PAGE]:
                        ...
                        # fid deterministico (stabile) – usa contenuto formula
                ...
                # 2) KG LLM: solo su image_vision e su text keyword-rich (NON sulle formule)
                if KG_ENABLED and toon_type in ("image_vision", "text"):
                    # molto selettivo per velocità
                    rich = (toon_type == "image_vision") or (toon_type == "text" and is_keyword_candidate(ch["text_sem"]))
            ...
            if len(qdrant_points) >= DB_FLUSH_SIZE:
                qdrant_client.upsert(QDRANT_COLLECTION, qdrant_points)
    ...
    for root, _, files in os.walk(INBOX_DIR):
        for fname in files:
            if fname.lower().endswith(".pdf"):
                process_single_file(os.path.join(root, fname), "pdf")
