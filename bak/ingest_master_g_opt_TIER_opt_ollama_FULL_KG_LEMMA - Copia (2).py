...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    - Salvataggio garantito in ingestion_images con log 
    - Differenziazione semantica: text_raw (display) vs text_sem (embedding) 
    """
    out_chunks: List[Dict[str, Any]] = []
    doc = fitz.open(file_path)
    filename = os.path.basename(file_path) # Recuperato per arricchimento semantico
                        ...
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_f:
                                ...
                                if (f_js and f_js.get("has_formulas")) or is_low_text_page:
                                    ...
                                    # Differenziazione RAW/SEMANTIC 
                                    raw_latex = "\n".join([f.get("latex", "") for f in f_js.get("formulas", []) if f.get("latex")])
