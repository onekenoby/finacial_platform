...
def force_restart_ollama(num_parallel: str = "4") -> bool:
    ...
    for i in range(15):
        try:
            res = requests.get("http://localhost:11434/api/chat", timeout=1)
            if res.status_code == 200:
                print(f"   ✨ Ollama is READY (Parallel={num_parallel})")
                return True
        except:
...
def llm_chat_multimodal(prompt: str, image_bytes: bytes, model: str, max_tokens: int = 900) -> str:
    ...
    # ✅ Ollama images expects base64 strings (robusto su tutte le versioni)
        ...
        try:
            resp = chat(
                ...
                messages=[{
                    "role": "user",
                    "content": prompt,
                    "images": [img_b64],   # ✅ base64, non bytes
                }],
                format="json",
                options={
                    "temperature": 0.0,
            ...
            # ✅ compatibile sia con dict sia con ChatResponse
            content = ""
            if isinstance(resp, dict):
                content = (resp.get("message", {}) or {}).get("content", "") or ""
            else:
                # ChatResponse (pydantic) ha tipicamente resp.message.content
                try:
            ...
            # Debug leggero (solo se vuoto)
            print("   ⚠️ Vision: risposta vuota dal modello. Verifica che il modello sia davvero VL e caricato in Ollama.")
            return ""
    ...
    if last_err:
        print(f"   ❌ Vision fallita definitivamente: {last_err}")
    return ""
