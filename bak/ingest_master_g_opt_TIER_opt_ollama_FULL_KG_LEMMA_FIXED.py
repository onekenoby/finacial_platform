...
VISION_PARALLEL_WORKERS = int(os.getenv("VISION_PARALLEL_WORKERS", "2"))  # P5000: 2 consigliato
...
def is_structural_page(text: str) -> bool:
    ...
    # Se le parole strutturali appaiono nelle prime 300 battute, è una pagina di servizio
    return bool(STRUCTURAL_PAT.search(text[:400]))

def count_unique_keywords(text: str) -> int:
    """Conta quanti concetti finanziari DIVERSI sono presenti."""
...
def ai_gatekeeper_decision(text: str) -> bool:
    """
    Chiede a un modello ultra-rapido (2B) se il testo merita un Knowledge Graph.
    Questo pre-filtraggio salva minuti preziosi sulla P5000.
    """
    try:
        # Usiamo gemma2:2b perché è fulmineo e occupa pochissima VRAM
        resp = chat(model="gemma2:2b", messages=[{
            "role": "user", 
            "content": (
                "Task: Is this financial text containing specific entities, quantitative data "
                "or causal rules worth a Knowledge Graph? Answer ONLY 'YES' or 'NO'.\n\n"
                f"Text: {text[:600]}"
            )
        }])
        decision = resp['message']['content'].upper()
        return "YES" in decision
    except Exception as e:
        # In caso di errore di Ollama, meglio estrarre che perdere dati
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    # --- reset Vision stats (per-document) ---
    ...
    try:
        ...
        def process_page_worker(p_idx):
            ...
            # --- Vision stats: conteggio pagine (thread-safe) ---
            with _VISION_STATS_LOCK:
            ...
            # ATTIVAZIONE VISION: solo se non è pagina “di servizio”
                    ...
                    try:
                        with t_conn.cursor() as t_cur:
                            ...
                            if do_c:
                                ...
                                # pagina risolta SOLO con crop (nessun full-page)
                                if has_imgs and found_valid_crop and not need_fullpage:
                                    with _VISION_STATS_LOCK:
        ...
        # --- Vision stats report (per-document) ---
        with _VISION_STATS_LOCK:
            print(
                "📊 Vision stats | "
                f"pagine totali={VISION_STATS['pages_total']} | "
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            if KG_ENABLED:
                ...
                for p_no, indexed_chunks in pages_map.items():
                    ...
                    # 1. Analisi Simbolica (Velocissima)
                    unique_keys = count_unique_keywords(combined_text)
                    is_struct = is_structural_page(combined_text)

                    # 2. Definizione Regole di Selettività
                    if is_struct:
                        # Indici, Bibliografie e Ringraziamenti vengono sempre saltati
                        should_extract_kg = False
                    elif tier == "A":
                        # Tier A (Metodologia): Sempre se c'è almeno 1 concetto e testo sufficiente
                        should_extract_kg = len(combined_text) > 400 and unique_keys >= 1
                    else:
                        # Tier B/C (News/Ref): Solo se densi (>3 keyword e >600 caratteri)
                        should_extract_kg = len(combined_text) > 600 and unique_keys >= 3

                    # 3. AI Gatekeeper (Solo per News/Ref che hanno superato i filtri iniziali)
                    if should_extract_kg and tier != "A":
                        # Verifica finale con modello leggero per evitare 'false flag'
                        should_extract_kg = ai_gatekeeper_decision(combined_text)

                    # 4. Esecuzione (Se approvato)
                    if should_extract_kg:
                        # Invia al modello pesante (Gemma 3 12B) tramite l'executor seriale
                        fut = kg_executor.submit(llm_extract_kg, filename, p_no, combined_text, model_for_kg)
                        futures_kg[fut] = indexed_chunks
                
                # Raccolta Risultati (Invariata)
                if futures_kg:
                    for fut in as_completed(futures_kg):
                        ...
                        try:
                            n, e = fut.result()
                            if n or e:
                                e = canonicalize_edges_to_verb_object(
                                    canonicalize_edges_by_base_presence(e)
                                )
