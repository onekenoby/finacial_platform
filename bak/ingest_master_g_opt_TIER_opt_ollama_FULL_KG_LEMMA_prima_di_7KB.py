...
# =========================
def llm_extract_kg(filename, page_no, text, model_name):
    """Estrazione KG con num_predict dinamico per Pascal P5000."""
    t0 = time.perf_counter()
    nchars = len(text)
    dynamic_predict = 1200 if nchars < 2500 else 2000
    
    try:
        resp = chat(
            model=model_name,
            messages=[{"role": "system", "content": KG_PROMPT}, {"role": "user", "content": text[:6000]}],
            format="json",
            options={"temperature": 0.1, "num_predict": dynamic_predict, "num_ctx": 3072}
        )
        raw = resp["message"]["content"]
        js = safe_json_extract(raw)
        dt = time.perf_counter() - t0
        print(f"   ⏱️ KG p{page_no}: {dt:.2f}s (predict: {dynamic_predict})")
...
def extract_pdf_as_markdown_assets(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """Trasforma PDF in MD virtuale e parcheggia immagini in RAM (Asset Parking)."""
    doc = fitz.open(file_path)
    filename = os.path.basename(file_path)
    asset_park = {} 
    virtual_md_content = []
    ...
    print(f"   🔄 Asset Pipeline: Trasformazione {total_pages} pagine in RAM...")
    ...
    for p_idx in range(total_pages):
        ...
        # 2. Selezionatore Immagini (Filtro > 7KB)
        ...
        for img_idx, img in enumerate(image_list):
            ...
            if len(img_bytes) < 7000: continue # Scarta icone e loghi
            
            asset_id = f"img_p{p_no}_{img_idx}.jpg"
            asset_park[asset_id] = img_bytes
            virtual_md_content.append(f"\n![{asset_id}]({asset_id})\n")
            
        # Log di progresso real-time
        if p_no % 5 == 0 or p_no == total_pages:
...
def process_virtual_md_chunks(content: str, asset_park: dict, filename: str, log_id: int) -> List[Dict[str, Any]]:
    """Converte il MD virtuale in chunk ed esegue Vision AI solo sugli asset parcheggiati."""
    out_chunks = []
    ...
    for idx, para in enumerate(raw_paras):
        ...
        # Cerca asset in RAM (Vision Chirurgica)
        ...
        if img_match and PDF_VISION_ENABLED:
            ...
            if img_bytes:
                ...
                if c_js and c_js.get("kind") != "other":
                    chunk_data["toon_type"] = "immagine"
                    chunk_data["text_sem"] = build_chart_semantic_chunk(1, c_js, prefix="ASSET")
                    # Salvataggio binario in Postgres
                    conn = pg_get_conn()
                    try:
                        with conn.cursor() as cur:
                            chunk_data["image_id"] = pg_save_image(log_id, img_bytes, "image/jpeg", f"RAM_{asset_id}", cur)
                        conn.commit()
