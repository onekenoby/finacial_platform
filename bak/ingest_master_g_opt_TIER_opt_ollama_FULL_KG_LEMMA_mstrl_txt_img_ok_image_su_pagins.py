...
VISION_PARALLEL_WORKERS = 4 #int(os.getenv("VISION_PARALLEL_WORKERS", "4"))  # 4-6 di solito ok
...
def force_restart_ollama(num_parallel: str = "4") -> bool:
    """
    Chiude ogni istanza di Ollama e la riavvia con il parallelismo attivo.
    FIX:
    - env values devono essere SOLO stringhe (Windows)
    - non sovrascrivere num_parallel passato
    - healthcheck corretto su /api/tags (GET)
    - shell=False (più stabile con lista argomenti)
    """
    print(f"🔄 Resetting Ollama Server (NUM_PARALLEL={num_parallel})...")

    # 1) Kill processi esistenti
    try:
    ...
    # 2) Configura l'ambiente (SOLO STRINGHE)
    env = os.environ.copy()
    ...
    # 5) Verifica disponibilità (healthcheck corretto)
    for i in range(15):
        try:
...
def llm_extract_kg(filename: str, page_no, text: str, model_name: str):
    ...
    # ------------------------------------------------------------
    # Helper: Regex JSON Extraction
    # ------------------------------------------------------------
    def _extract_json_substring(raw_text: str):
        # Cerca il pattern più ampio possibile tra graffe esterne
        # Gestisce anche il caso ```json ... ```
        try:
            # 1. Cerca blocco markdown
            match = re.search(r'```(?:json)?\s*(\{.*?\})\s*```', raw_text, re.DOTALL)
            if match: return json.loads(match.group(1))
            
            # 2. Cerca graffa aperta/chiusa
            start = raw_text.find('{')
            end = raw_text.rfind('}')
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Strategia 'Vision-First':
    1. Renderizza l'intera pagina come immagine.
    2. Ministral (Vision) la trascrive tutta in una volta (Testo + Descrizione Grafici).
    3. Output: Markdown pulito classificato come 'text' per il KG.
    """
    out_chunks = []
    filename = os.path.basename(file_path)
    ...
    def process_page_worker(p_idx: int):
        ...
        try:
            ...
            # 1. Rendering (120 DPI per velocità/qualità ottimale su Ministral)
            zoom = VISION_DPI / 72
            mat = fitz.Matrix(zoom, zoom)
            pix = page.get_pixmap(matrix=mat, alpha=False)
            img_bytes = pix.tobytes("png")
            
            # 2. Salva img nel DB (per UI/Frontend)
            img_id = None
            t_conn = pg_get_conn()
            if t_conn:
                try:
            ...
            # 4. Creazione Chunk e Classificazione Intelligente
            if chunk_text and "NO_CONTENT" not in chunk_text and len(chunk_text) > 10:
                
                # --- LOGICA DI RILEVAMENTO RAFFORZATA (Deep Check) ---
                is_visual = False
                
                # Livello 1: Match Esatto (Il caso ideale, se l'LLM obbedisce al prompt)
                if "### 🖼️ VISUAL ANALYSIS" in chunk_text:
                    is_visual = True
                
                # Livello 2: Match Strutturale (Se l'LLM dimentica l'header ma usa i bullet points richiesti)
                elif "*Visual Elements:*" in chunk_text or "*Data Insights:*" in chunk_text:
                    is_visual = True
                    
                # Livello 3: Euristica Semantica (Se l'LLM descrive un grafico in linguaggio naturale)
                else:
                    # A. Cerca riferimento a figura (Fig. 1, Figura 2.1, Figure 3)
                    has_fig_ref = bool(re.search(r"\b(Fig\.|Figura|Figure)\s*\d", chunk_text, re.IGNORECASE))
                    
                    # B. Cerca vocabolario tecnico VISIVO (almeno 2 termini per evitare falsi positivi)
                    ...
                    has_visual_terms = len(re.findall(visual_keywords, chunk_text, re.IGNORECASE)) >= 2
                    
                    # C. Cerca riferimenti a colori (spesso usati per descrivere linee/barre)
                    has_colors = bool(re.search(r"\b(rosso|verde|blu|giallo|nero|red|green|blue|yellow|black)\b", chunk_text, re.IGNORECASE))

                    # DECISIONE: È un grafico se ha (Ref Figura + Termini Tecnici) OPPURE (Termini Tecnici + Colori)
                    if (has_fig_ref and has_visual_terms) or (has_visual_terms and has_colors):
                        is_visual = True

                detected_type = "immagine" if is_visual else "text"
                
                # --- FINE LOGICA RAFFORZATA ---
                ...
                # FIX 1: Passiamo chunk_text puro (senza header manuali concatenati qui)
                # La funzione 'recurse_text_chunking' aggiungerà l'header "Doc: ... | Pag: ..." 
                # solo al campo 'content_semantic', lasciando 'content_raw' pulito.
                chunks = recurse_text_chunking(chunk_text, meta, max_chars=CHUNK_MAX_CHARS)
                local_res.extend(chunks)
