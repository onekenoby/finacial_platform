...
DB_FLUSH_SIZE = int(os.getenv("DB_FLUSH_SIZE", "96"))          # un po' più alto per meno flush - 
...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "80")) #
MAX_KG_CHUNKS_PER_DOC = int(os.getenv("MAX_KG_CHUNKS_PER_DOC", "100"))
...
KG_PROMPT = """
You are a Linguistic Knowledge Engineer and Financial Knowledge Engineer. Your task is to transform text into a Knowledge Graph.

Extraction Rules:
1. NODES (Entities): Focus on NOUNS. 
   - Identify People, Organizations, Cities, Countries, Products, or Technical Concepts.
   - Label: The specific name (e.g., "Xi Jinping", "Beijing", "Colza").
   - Type: Categorize as Person, Organization, Location, Commodity, or Concept.

2. EDGES (Relationships): Focus on VERBS and Predicates.
   - Extract the action or state connecting two nodes.
   - Relation: Use uppercase with underscores (e.g., "DISCUSSES", "IMPOSES_TARIFFS_ON", "LOCATED_IN").
   - Properties: Include tense or specific values if available.

Return ONLY JSON:
{"nodes":[{"id":"..","label":"..","type":"..","properties":{}}],"edges":[{"source":"..","target":"..","relation":".."}]}
"""
...
def is_keyword_candidate(text: str) -> bool:
    """Trigger ibrido: entità proprie O keyword finanziarie O lunghezza significativa."""
    if not text or len(text) < KG_MIN_LEN:
        return False
    
    clean_text = safe_normalize_text(text)
    
    # 1. Ha nomi propri? (Cina, Canada, Ottawa...)
    proper_nouns = _ENTITY_PROPER_NOUNS.findall(clean_text)
    if len(set(proper_nouns)) >= 2:
        return True
    
    # 2. Ha keyword finanziarie?
    if _KG_PAT.search(clean_text):
        return True
    
    # 3. Chunk lungo e strutturato? (es. procedure, definizioni)
    if len(clean_text) >= 400 and clean_text.count('.') >= 3:
        return True
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for page_idx in range(total_pages):
            ...
            # 2) Formula & Chart full-page vision
            do_formula_vision = PDF_VISION_ENABLED and is_formula_candidate_page(page_text)
            do_chart_vision = PDF_VISION_ENABLED and is_chart_candidate_page(page_text) # <-- AGGIUNTO

            # Ottimizzazione: se c'è molto testo, evita la vision per risparmiare tempo
            if PDF_VISION_ONLY_IF_TEXT_SCARSO and len(page_text) >= PDF_MIN_TEXT_LEN_FOR_NO_VISION:
                do_formula_vision = False
                do_chart_vision = False

            if do_formula_vision or do_chart_vision:
                try:
                    ...
                    # Gestione Grafici/Tabelle (Full Page)
                    if do_chart_vision:
                        chart_json = extract_chart_via_vision(full_jpg)
                        if chart_json and chart_json.get("kind") not in ("other", None):
                            _ = pg_save_image(log_id, full_jpg, "image/jpeg", f"PDF page {page_no} chart vision", cur=pg_cur)
                            sem = build_chart_semantic_chunk(page_no, chart_json, prefix="PAGE_VISUAL")
                            out_chunks.append({
                                "text_raw": sem,
                                "text_sem": sem,
                                "page_no": page_no,
                                "toon_type": "image_vision", # Così viene processato dal KG
                                "section_hint": section_hint,
                                "metadata_override": chart_json
                            })
                except Exception as e:
                    print(f"   ⚠️ Page vision failed p.{page_no}: {e}")
    ...
    try:
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            for j, ch in enumerate(batch):
                ...
                # ===== Neo4j part =====
                # ===== Neo4j part - VERSIONE CON LATEX RAW E GRANULARITÀ =====
# ===== Neo4j part - STRUTTURA PULITA E FUNZIONANTE =====
# ===== Neo4j part - VERSIONE PULITA E OTTIMIZZATA =====
# ===== Neo4j part - VERSIONE PULITA E CORRETTA =====
                # ===== Neo4j part - VERSIONE OTTIMIZZATA PER MASSIMA ESPLOSIONE =====
                kg_nodes, kg_edges = [], []

                # A) Gestione Formule (dedicata)
                if toon_type == "formula_page":
                    ...
                    for f in formulas[:VISION_MAX_FORMULAS_PER_PAGE]:
                        ...
                        neo4j_formula_rows.append({
                            "chunk_id": c_uuid, "fid": fid, "latex": raw_latex, 
                            "latex_raw": raw_latex, "plain": f.get("plain"),
                            "meaning_it": f.get("meaning_it"), "keywords": f.get("keywords", []),
                            "page_no": page_no, "filename": filename
                        })

                # B) Estrazione KG (Testo + Vision) - LOGICA UNIFICATA
                if KG_ENABLED and toon_type in ("text", "image_vision"):
                    # Normalizza SOLO per analisi (non modifica il DB)
                    analysis_text = safe_normalize_text(ch["text_sem"])
                    
                    # Trigger: Vision SEMPRE processata, Testo solo se ricco
                    should_extract = (
                        toon_type == "image_vision" or  # Vision sempre
                        (len(analysis_text) >= KG_MIN_LEN and is_keyword_candidate(analysis_text))  # Testo selettivo
                    )
                    
                    # Rispetta il limite per documento (alzalo a 50-100 per massima copertura)
                    if should_extract and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                        kg_nodes, kg_edges = llm_extract_kg(filename, page_no, analysis_text)
                        kg_chunks_used += 1

                    # C) Salvataggio record Neo4j (SEMPRE, anche se KG vuoto)
                    if NEO4J_ENABLED:
                        neo4j_rows.append({
                            "doc_id": doc_id, "filename": filename, "doc_type": doc_type,
                            "log_id": log_id, "chunk_id": c_uuid, "chunk_index": c_idx,
                            "toon_type": toon_type, "page_no": page_no, "section_hint": section_hint,
                            "text_sem": ch["text_sem"], "ontology": ontology,
                            "nodes": kg_nodes, "edges": kg_edges  # Può essere vuoto
                        })

                         # ===== Neo4j part - VERSIONE GRANULARE CON PROTEZIONE FORMULE =====
                        if KG_ENABLED and toon_type in ("image_vision", "text"):
                            
                            # 1. Creiamo una versione pulita solo per l'analisi LLM/Regex
                            # Il testo originale ch["text_sem"] resta intatto per il DB
                            temp_clean_text = safe_normalize_text(ch["text_sem"])
                            
                            # 2. Il trigger ora lavora sul testo "comprensibile"
                            rich = (
                                toon_type == "image_vision" 
                                or is_keyword_candidate(temp_clean_text)
                            )
                            
                            if rich and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                                # 3. Passiamo all'LLM il testo normalizzato per evitare allucinazioni su caratteri strani
                                kg_nodes, kg_edges = llm_extract_kg(filename, page_no, temp_clean_text)
                                kg_chunks_used += 1

                        # Gestione Knowledge Graph (Esplosione Chunk)
                        if KG_ENABLED and toon_type in ("image_vision", "text"):
                            # Verifichiamo se il chunk è "ricco" (Vision o contiene Keyword)
                            rich = (
                                toon_type == "image_vision" 
                                or is_keyword_candidate(ch["text_sem"])
                            )
                            
                            # Se è ricco e non abbiamo superato il limite massimo per documento
                            if rich and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                                kg_nodes, kg_edges = llm_extract_kg(filename, page_no, ch["text_sem"])
                                kg_chunks_used += 1
                                # NOTA: kg_pages_done non viene più usato qui per permettere più chunk per pagina
                        ...
                        if NEO4J_ENABLED:
                            neo4j_rows.append({
                                ...
                                "nodes": kg_nodes,
                                "edges": kg_edges,
                                "ontology": ontology # Aggiunto per coerenza con la query Cypher
                            })

                # Logica KG modificata: rimosso il vincolo 'page_no in kg_pages_done'
                if KG_ENABLED and toon_type in ("image_vision", "text"):
                    rich = (
                        toon_type == "image_vision"
                    ...
                    # Estraiamo KG per OGNI chunk che supera il filtro, fino al limite massimo
                    if rich and kg_chunks_used < MAX_KG_CHUNKS_PER_DOC:
                        kg_nodes, kg_edges = llm_extract_kg(filename, page_no, ch["text_sem"])
                        kg_chunks_used += 1
                        # kg_pages_done.add(page_no)  <-- Rimosso per permettere altri chunk sulla stessa pagina
                    else:
                        rich = (
                            toon_type == "image_vision"
                            or (toon_type == "text" and is_keyword_candidate(ch["text_sem"]))
