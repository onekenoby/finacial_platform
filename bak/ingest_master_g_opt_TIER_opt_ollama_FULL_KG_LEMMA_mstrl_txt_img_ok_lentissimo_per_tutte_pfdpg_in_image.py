...
MIN_ASSET_SIZE = int(os.getenv("MIN_ASSET_SIZE", "15000"))
...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "1200")) #
MAX_KG_CHUNKS_PER_DOC = int(os.getenv("MAX_KG_CHUNKS_PER_DOC", "1000"))
...
def ai_gatekeeper_decision(text: str) -> bool:
    """Gemma 2 9b agisce come 'buttafuori' ultra-veloce."""
    try:
        resp = chat(
            model=LLM_MODEL_NAME, 
            messages=[{"role": "user", "content": f"Classify: extractable financial data? YES or NO.\n\n{text[:500]}"}],
...
def force_restart_ollama(num_parallel: str = "2") -> bool:
    """
    Riavvio Ottimizzato per GPU 16GB (P5000).
    Forza OLLAMA_NUM_PARALLEL=2 per bilanciare Vision e Chat senza OOM.
    """
    ...
    # Su 16GB, max 2 richieste contemporanee (1 Vision + 1 Chat oppure 2 Vision)
    ...
    print(f"   🚀 Starting Ollama form: {ollama_path} with P5000 optimizations...")
    ...
    for i in range(20): # Aumentato timeout a 20 per dare tempo al caricamento VRAM
        try:
            ...
            time.sleep(1.5)
            print(f"   ...waiting for Ollama VRAM init ({i+1}/20)")
...
def llm_chat_multimodal(prompt: str, image_bytes: bytes, model: str, max_tokens: int = 4000) -> str:
    """
    Esegue la chiamata Vision usando la libreria ufficiale Ollama.
    Gestisce correttamente sia Ministral (Visione) che Qwen (se servisse).
    """
    if not image_bytes:
        return ""
    ...
    try:
        ...
        print(f"   ❌ Errore Base64 Encoding: {e}")
        return ""

    try:
        # 2. Chiamata Nativa
        # Temperature 0.0 per massima fedeltà OCR
        # Num_ctx 4096 è vitale per le immagini a pagina intera
        ...
        # 3. Estrazione Contenuto Robusta
        content = ""
        ...
        if "not found" in str(e).lower():
            print(f"   ❌ Modello '{model}' non trovato! Scaricalo con 'ollama pull {model}'")
        else:
            print(f"   ⚠️ Errore Ollama API: {e}")
        return ""
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(process_page_worker, i) for i in range(total_pages)]
...
def process_single_file(file_path: str, source_type: str, doc_meta: dict):
    """
    Pipeline v2.5 - Optimized for Gemma 2:9b & P5000 (Final Fix)
    """
    t0 = time.time()
    filename = os.path.basename(file_path)
    ...
    for i in range(0, num_chunks_totali, EMBED_BATCH_SIZE):
        ...
        # 1. Embeddings
        vecs = embedder.encode(texts, batch_size=EMBED_BATCH_SIZE, show_progress_bar=False)

        # 2. Knowledge Graph (Parallel)
        batch_kg_results = {}
        if KG_ENABLED:
            pages_map = {}
            ...
            for p_no, indexed_chunks in pages_map.items():
                # Skip KG if page is marked (e.g. formulas) or by Gatekeeper
                combined_text = "\n".join([ch.get("text_sem", "") for _, ch in indexed_chunks])
                text_clean = safe_normalize_text(combined_text)[:KG_TEXT_MAX_CHARS]
                
                # Applichiamo il Gatekeeper per risparmiare tempo su pagine vuote/inutili
            ...
            # Raccolta con timeout gestito
            for p_no, fut in futures_kg.items():
                try:
                    # Aumentato a 120s per Gemma 2 9b su P5000 se il batch è denso
                    ...
                    # Silenzioso nei log per non sporcare la console, l'errore è già gestito in llm_extract_kg
                    pass

        # 3. Costruzione Record
        for j, ch in enumerate(batch):
            g_idx = i + j
            vector = vecs[j]
            chunk_id = deterministic_chunk_id(doc_id, ch.get("page_no", 1), g_idx, ch.get("toon_type"), ch.get("text_sem"))
            ...
            # Neo4j (FIXED: Ora include tutti i campi necessari)
            k_nodes, k_edges = batch_kg_results.get(g_idx, ([], []))
            neo4j_rows.append({
                "doc_id": doc_id,
                "filename": filename,       # <--- FIX IMPORTANTE
                "doc_type": source_type,    # <--- FIX IMPORTANTE
                "log_id": log_id,           # <--- FIX IMPORTANTE
                "chunk_id": chunk_id, 
        ...
        print(f"   📦 Batch {int(i/EMBED_BATCH_SIZE)+1} | {percentuale}% completato | {_ms(batch_t0)}ms")

    # 5. Chiusura e Calcolo Tempi (FINALE)
    total_ms = _ms(t0)
    
    # Aggiorna il log su Postgres
    pg_close_log(log_id, "DONE", total_chunks, total_ms)

    # (OPZIONALE) Aggiorna il tempo di elaborazione anche nel nodo Document su Neo4j
    if NEO4J_ENABLED:
        try:
            with neo4j_driver.session() as session:
                session.run(
                    "MATCH (d:Document {doc_id: $did}) SET d.processing_time_ms = $ms",
                    did=doc_id, ms=total_ms
                )
        except Exception as e:
            print(f"   ⚠️ Errore aggiornamento tempo Neo4j: {e}")
    ...
    # Print finale completo
    seconds = total_ms / 1000
    print(f"   ✅ Completed: {filename} | chunks={total_chunks} | time={seconds:.2f}s")
