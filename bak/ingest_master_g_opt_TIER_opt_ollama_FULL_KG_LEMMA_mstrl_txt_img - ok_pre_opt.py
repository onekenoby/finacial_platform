...
VISION_DPI = int(os.getenv("VISION_DPI", "110"))
...
MIN_ASSET_SIZE = int(os.getenv("MIN_ASSET_SIZE", "6000"))
...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "1000")) #
...
def llm_chat_multimodal(
    ...
    image_bytes: bytes,
    model: str,
    max_tokens: int = 4000,     # ✅ retro-compat: nel tuo codice viene passato
    num_ctx: int = 4096,        # contesto
) -> str:
    """
    Vision via Ollama /api/generate (robusto).
    - max_tokens -> options.num_predict (compatibile con chiamate esistenti)
    - timeout + retry
    ...
    payload = {
        ...
        "options": {
            "temperature": 0.0,
            "num_ctx": int(num_ctx),
            "num_predict": int(max_tokens) if max_tokens is not None else 4000,  # ✅ mapping corretto
        },
        "stream": False,
    }
...
def extract_chart_via_vision(img_bytes: bytes, context_hint: str = "") -> Optional[Dict[str, Any]]:
    ...
    # ✅ FIX CRITICO: prima validiamo js, POI usiamo .get()
    if not isinstance(js, dict):
        return None
    ...
    if not legend_ok:
        # remove claims about what colors represent (robusto: o può non essere stringa)
        ...
        for o in obs:
            s = str(o)
            if "color" not in s.lower() and "colore" not in s.lower():
                cleaned_obs.append(s)
        js["observations_it"] = cleaned_obs
...
def build_chart_semantic_chunk(page_no: int, chart_json: Dict[str, Any], prefix: str = "VISUAL") -> str:
    if not isinstance(chart_json, dict):
        return normalize_ws(f"--- CONTENUTO VISUALE (immagine) - Pagina {page_no} ---\n[chart_json non valido]")
    ...
    lines = [f"--- CONTENUTO VISUALE ({to_text(kind)}) - Pagina {page_no} ---"]
    if title:
    ...
    if subtitle and subtitle != "NOT READABLE":
        lines.append(f"Sottotitolo: {to_text(subtitle)}")
    ...
    # ✅ vis può essere list -> normalizzato
    vis_s = to_text(vis)
    if vis_s.strip():
    ...
    if nums:
        ...
        for n in nums[:10]:
            if isinstance(n, dict):
                ...
                unit = to_text(n.get("unit", ""))
                period = to_text(n.get("period", ""))
                tail = f" ({period})" if period else ""
                lines.append(f" - {label}: {value} {unit}{tail}".strip())
            else:
                s = to_text(n)
                if s.strip():
                    lines.append(f" - {s}")
    ...
    analysis_s = to_text(analysis_it)
    if analysis_s.strip():
        lines.append("Analisi (IT) - interpretazione qualitativa:")
        lines.append(analysis_s)
...
def llm_extract_kg(filename: str, page_no, text: str, model_name: str):
    ...
    try:
        ...
        # AUMENTATO num_predict a 2500 per evitare "JSON troncato"
        # AUMENTATO num_ctx a 8192 per dare più memoria a Gemma
        resp = chat(
            model=model_name,
            messages=[
            ...
            options={
                "temperature": 0.0, 
                "num_predict": 2500, # <--- MODIFICA CRITICA QUI
                "num_ctx": 8192      # <--- MODIFICA CRITICA QUI
            }
        )
    ...
    def process_page_worker(p_idx: int):
        ...
        try:
            ...
            if chunk_text and "NO_CONTENT" not in chunk_text and len(chunk_text) > 10:
                ...
                # --- POST-PROCESSING: CLASSIFICAZIONE GRANULARE ---
                for ch in raw_chunks:
                    txt_content = ch.get("text_raw", "")
                    
                    # LOGICA DI RILEVAMENTO (Applicata al singolo chunk)
                    is_visual = False
                    
                    # A. Tag Esplicito (Priorità assoluta)
                    if "### 🖼️ VISUAL ANALYSIS" in txt_content:
                        is_visual = True
                    
                    # B. Struttura del Prompt Vision
                    elif "*Visual Elements:*" in txt_content or "*Data Insights:*" in txt_content:
                        is_visual = True
                        
                    # C. Euristica Semantica (Deep Check)
                    else:
                        # Regex per termini visivi specifici
                        visual_keywords = r"\b(asse|assi|axis|axes|legenda|legend|grafico|chart|plot|pendenza|slope|curva|curve|barre|bars|candela|candlestick)\b"
                        has_visual_terms = len(re.findall(visual_keywords, txt_content, re.IGNORECASE)) >= 2
                        
                        # Regex per colori (forte indizio visivo)
                        has_colors = bool(re.search(r"\b(rosso|verde|blu|giallo|nero|red|green|blue|yellow|black)\b", txt_content, re.IGNORECASE))
                        
                        # Regex per riferimento Figura nello stesso chunk
                        has_fig_ref = bool(re.search(r"\b(Fig\.|Figura|Figure)\s*\d", txt_content, re.IGNORECASE))

                        # Logica combinata
                        if (has_fig_ref and has_visual_terms) or (has_visual_terms and has_colors):
                            is_visual = True

                    # D. Anti-Hallucination per Formule (Safety Net)
                    # Se c'è tanto LaTeX ($$) e mancano termini visivi forti, forziamo TEXT.
                    # Questo protegge il tuo chunk "Algoritmo VECM" che contiene formule.
                    if "$$" in txt_content or "\\[" in txt_content:
                         if not ("grafico" in txt_content.lower() or "chart" in txt_content.lower()):
                             is_visual = False

                    # Applica il tipo rilevato
                    ch["toon_type"] = "immagine" if is_visual else "text"
                    local_res.append(ch)
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    if ext == ".pdf":
        mode = os.getenv("PDF_PIPELINE", "assets").strip().lower()
        # assets = testo layer + Vision solo sugli asset (consigliato)
        # fullpage = Vision su pagina intera (più lenta, più rischiosa)
        if mode == "fullpage":
...
def extract_pdf_as_markdown_assets(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    Versione ottimizzata per P5000: Scarica la Vision AI dalla VRAM dopo ogni pagina
    e forza il Garbage Collector per evitare freeze su documenti lunghi.
    """
    chunks_payload = []
    ...
    # 0. Pulizia preventiva VRAM prima di iniziare il loop
    if PDF_VISION_ENABLED and VISION_MODEL_NAME:
        force_unload_ollama(VISION_MODEL_NAME)

    for i, page in enumerate(doc):
        try:
            # 1. Estrazione Testo Base
            page_text = page.get_text()
            
            # Pulizia base del testo
            text_sem = safe_normalize_text(page_text)
            
            # Se la pagina è vuota o quasi, skippala (opzionale, qui la teniamo per contesto)
            if len(text_sem) < 50 and not PDF_VISION_ENABLED:
                continue

            # 2. Gestione Immagini / Grafici (Vision AI)
            vision_data = {}
            if PDF_VISION_ENABLED:
                # Estrae immagini dalla pagina
                images = page.get_images(full=True)
                
                # Se ci sono immagini significative
                if images:
                    for img_index, img in enumerate(images):
                        ...
                        try:
                            ...
                            # Filtro dimensione: ignoriamo icone piccole (< 5KB)
                            if len(image_bytes) < 5000:
                                continue

                            # Salvataggio Asset su Postgres (Asset Parking)
                            img_name = f"PDF_{doc_id}_P{i+1}_IMG{img_index}"
                            conn = pg_get_conn()
                            try:
                                with conn.cursor() as cur:
                                    pg_save_image(log_id, image_bytes, "image/jpeg", img_name, cur)
                                conn.commit()
                            finally:
                                pg_put_conn(conn)

                            # Vision AI (Solo se abilitata e modello presente)
                            if VISION_MODEL_NAME:
                                # Qui chiamiamo la funzione che usa minicpm-v
                                chart_json = extract_chart_via_vision(image_bytes)
                                
                                # Se abbiamo trovato dati utili, li aggiungiamo al testo
                                if chart_json and chart_json.get("kind") != "other":
                                    desc = build_chart_semantic_chunk(i+1, chart_json)
                                    # Aggiungiamo la descrizione visuale al testo della pagina
                                    text_sem += f"\n\n[ANALISI VISIVA GRAFICO/TABELLA]:\n{desc}\n"
                                    vision_data = chart_json # Salviamo i metadati
                                    # print(f"   👁️ Vision success (Pg {i+1})")
                                    
                        except Exception as e_img:
                            print(
                                f"   ⚠️ Err. Img {img_index} pg {i+1}: {e_img} | "
                                f"types: vis={type((chart_json or {}).get('what_is_visible_it')).__name__}, "
                                f"obs={type((chart_json or {}).get('observations_it')).__name__}, "
                                f"unread={type((chart_json or {}).get('unreadable_parts')).__name__}"
                            )
                            # ===== BONUS DEBUG (QUI, ESATTAMENTE QUI) =====
                            try:
                                nums = (chart_json or {}).get("numbers", None)
                                if isinstance(nums, list):
                                    none_cnt = sum(1 for x in nums if x is None)
                                    first_type = type(nums[0]).__name__ if nums else "EMPTY"
                                    print(
                                        f"   [DEBUG] numbers: type=list, len={len(nums)}, "
                                        f"none_cnt={none_cnt}, first_type={first_type}"
                                    )
                                else:
                                    print(f"   [DEBUG] numbers type={type(nums).__name__}")
                            except Exception as _dbg:
                                print(f"   [DEBUG] numbers inspect failed: {_dbg}")
                            # ===== FINE BONUS DEBUG =====


                            # 🔍 BONUS DEBUG — ESATTAMENTE QUI
                            try:
                                mapp = (chart_json or {}).get("legend_it", {}).get("mapping", None)
                                if isinstance(mapp, list):
                                    none_cnt = sum(1 for x in mapp if x is None)
                                    print(
                                        f"   [DEBUG] legend.mapping: type=list, len={len(mapp)}, none_cnt={none_cnt}"
                                    )
                                else:
                                    print(
                                        f"   [DEBUG] legend.mapping type={type(mapp).__name__}"
                                    )
                            except Exception as _dbg:
                                print(f"   [DEBUG] legend.mapping inspect failed: {_dbg}")
            ...
            chunks_payload.append({
                "chunk_id": chunk_id,
                "text_raw": page_text, # Testo originale
                "text_sem": text_sem,  # Testo arricchito dalla Vision
                "page_no": i + 1,
                "toon_type": "pdf_page",
                "section_hint": f"Page {i+1}",
                "metadata": vision_data # Include i dati strutturati del grafico se presenti
            })

        except Exception as e_page:
            print(f"   ⚠️ Errore pagina {i+1}: {e_page}")
            continue
        
        # =========================================================
        # FIX VRAM P5000: PULIZIA TOTALE DOPO OGNI PAGINA
        # =========================================================
        
        print(f"   🔄 Processed {i+1}/{total_pages}...", end="\r")
        
        # A. Pulizia variabili Python (RAM Sistema)
        try:
        ...
        # B. Garbage Collector (Cruciale per liberare i buffer delle immagini)
        gc.collect()

        # C. Scaricamento VRAM (Cruciale per la GPU)
        if PDF_VISION_ENABLED and VISION_MODEL_NAME:
            force_unload_ollama(VISION_MODEL_NAME)
...
def normalize_toon_type(ch: dict) -> str:
    """
    Regola HARD:
    - 'imagine' se il chunk rappresenta un asset immagine
    - 'testo' altrimenti
    """
    # nel tuo codice i chunk immagine hanno SEMPRE image_id
...
def process_virtual_md_chunks(content: str, asset_park: dict, filename: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    # ✅ Cap Vision per documento (salvavita sui PDF pieni di immagini)
    vision_done = 0
...
def process_single_file(file_path: str, source_type: str, doc_meta: dict):
    ...
    for idx, ch in enumerate(chunks):
        ...
        meta.update({
            "doc_id": doc_id, "filename": filename, "tier": tier, "ontology": ontology
        })
        ch["metadata"] = meta
