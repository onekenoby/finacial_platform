...
def extract_chart_via_vision(img_bytes: bytes, context_hint: str = "") -> Optional[Dict[str, Any]]:
    """
    Vision-first, generico, anti-allucinazioni.

    Pipeline:
    - Multi-eye: overview + tiles (grid 3 + grid 5)
    - OCR support su più tiles (best effort)
    - PASS 1: schema/struttura (dinamica)
    - PASS 2: dati (records con dims dinamiche)
    - Se struttura dichiara dimensioni non-temporali ma records non le riportano:
        -> PASS 2b: focus labels (solo lettura etichette) e re-try PASS 2 con labels in prompt
    - Se non affidabile: mode=generic + nessun numero.
    """
    if not img_bytes:
        return None

    # ------------------------------------------------------------
    # page number da hint (come fai già)
    # ------------------------------------------------------------
    page_no = 0
    if context_hint:
    ...
    # ------------------------------------------------------------
    # Multi-eye: tiles più aggressivi (grid 3 + grid 5)
    # ------------------------------------------------------------
    # NOTE: manteniamo tile_image_for_vision invariata, ma la chiamiamo più volte
    # e uniamo le immagini. Questo aumenta molto le chance di leggere etichette piccole.
    imgs_g3 = tile_image_for_vision(img_bytes, max_side=2800, grid=3)
    imgs_g5 = tile_image_for_vision(img_bytes, max_side=3200, grid=5)

    # dedup semplice per hash (evita invii duplicati)
    def _dedup_imgs(img_list: List[bytes]) -> List[bytes]:
        seen = set()
        out = []
        for b in img_list:
            try:
    ...
    multi_imgs = _dedup_imgs((imgs_g3 or []) + (imgs_g5 or []))
    if not multi_imgs:
    ...
    # Cache
    # ------------------------------------------------------------
    cache_key = sha256_hex(multi_imgs[0]) + "::chart_multieye_v2"
    cached = _vision_cache_get(cache_key)
    if cached:
        return cached

    # ------------------------------------------------------------
    # OCR support (best effort) su più tiles
    # ------------------------------------------------------------
    # NON è un vincolo, serve solo ad aiutare il prompt quando le etichette sono minuscole.
    ocr_text = ""
    try:
        # prendi OCR su 1) overview e 2) alcune tiles "dense"
        ocr_parts = []
        sample = multi_imgs[:6]  # limite per non rallentare
        for b in sample:
            try:
                t = ocr_extract_text(b)
                if t and t.strip():
    ...
    # ------------------------------------------------------------
    # Helpers: controllo "completezza" dims
    # ------------------------------------------------------------
    TIME_KEYS = {"anno", "year", "years", "date", "data", "time", "tempo", "periodo", "period", "mese", "month",
                "giorno", "day", "quarter", "trimestre"}
    ...
    def _has_non_time_dims_in_records(records: Any) -> bool:
        ...
        for r in records:
            ...
            for k, v in dims.items():
                ...
                if not sv:
                    continue
                if _is_time_key(k) or _looks_like_year(sv):
                    continue
                return True
        return False

    def _structure_declares_non_time_dims(schema: Dict[str, Any]) -> bool:
        st = schema.get("structure")
        ...
        if not isinstance(dims, list):
            return False
        for d in dims:
            dd = to_text(d).strip().lower()
            if not dd:
                continue
            if _is_time_key(dd):
                continue
            # se c'è almeno una dimensione non-time dichiarata
            return True
        return False
    ...
    if ocr_text:
        schema_prompt += f"\nVISIBLE OCR (support only, may be partial): \"\"\"{ocr_text}\"\"\""
    ...
    if not isinstance(schema_js, dict):
        schema_js = {"mode": "generic", "kind": "unknown", "confidence": 0.2}

    # normalizza chiavi minime (senza assumere schema fisso)
    schema_mode = to_text(schema_js.get("mode", "")).strip().lower()
    schema_conf = float(schema_js.get("confidence", 0.0) or 0.0)

    # se schema non affidabile -> generic subito
    if schema_mode != "structured" or schema_conf < 0.45:
        generic_out = {
            "mode": "generic",
            "kind": schema_js.get("kind") or schema_js.get("chart_kind") or "unknown",
            "page_no": page_no,
            "confidence": float(schema_conf or 0.25),
            ...
            "data_points": [],
            "numbers": [],
            "unreadable_parts": ["Etichette/valori non leggibili con affidabilità sufficiente."],
            "schema": schema_js
        }
        generic_out["semantic_description"] = build_chart_semantic_chunk(page_no, generic_out)
        generic_out["toon_type"] = "immagine"
    ...
    if not isinstance(data_js, dict):
        data_js = {"mode": "generic", "confidence": 0.2, "summary": "Output non strutturato."}

    data_mode = to_text(data_js.get("mode", "")).strip().lower()

    # fallback generic se dati non structured
    if data_mode != "structured":
        generic_out = {
            ...
            "what_is_visible_it": to_text(data_js.get("summary", "")).strip() or "Grafico non digitizzabile con affidabilità sufficiente.",
            "analysis_it": "",
            "records": [],
            "data_points": [],
            "numbers": [],
            "unreadable_parts": ["Output dati non strutturato / incompleto."],
            "schema": schema_js
        }
        generic_out["semantic_description"] = build_chart_semantic_chunk(page_no, generic_out)
        generic_out["toon_type"] = "immagine"
    ...
    # ------------------------------------------------------------
    # PASS 2b (RETRY): se schema dichiara non-time dims ma records non le riportano
    # ------------------------------------------------------------
    # Qui non inventiamo nulla: chiediamo SOLO lettura etichette "category-like" visibili.
    need_labels_retry = False
    try:
        declared_non_time = _structure_declares_non_time_dims(schema_js)
        has_non_time = _has_non_time_dims_in_records(data_js.get("records"))
        if declared_non_time and not has_non_time:
            need_labels_retry = True
    except Exception:
        need_labels_retry = False

    labels_js = None
    if need_labels_retry:
        labels_prompt = (
            "You are a visual reading engine.\n"
            "TASK: Read ONLY the category/entity labels visible in the chart (e.g., countries, products, sectors, groups).\n"
            "Do NOT guess. If a label is not clearly readable, omit it.\n"
            "Return JSON with keys:\n"
            "{\n"
            '  "mode": "labels_only" | "generic",\n'
            '  "labels": [ ... ],\n'
            '  "notes": "...",\n'
            '  "confidence": 0.0\n'
            "}\n"
        ...
        raw_labels = llm_chat_multimodal(
        ...
        labels_js = safe_json_extract(raw_labels)
        if not isinstance(labels_js, dict):
            labels_js = {"mode": "generic", "labels": [], "confidence": 0.2}

        # se abbiamo labels affidabili, rifacciamo PASS 2 con labels in prompt
        labels = labels_js.get("labels") if isinstance(labels_js.get("labels"), list) else []
        labels = [to_text(x).strip() for x in labels if to_text(x).strip()]
        if labels:
            retry_prompt = CHART_DATA_PROMPT_TEMPLATE.format(schema_json=schema_json_str)
            retry_prompt += "\n\nIMPORTANT: The following category labels are clearly visible in the chart.\n"
            retry_prompt += "Use them when building records.dims; do NOT invent additional labels.\n"
            retry_prompt += f'VISIBLE_LABELS: {json.dumps(labels, ensure_ascii=False)}\n'

            if context_hint:
                retry_prompt += f"\nCONTEXT HINT: {context_hint[:600]}\n"
            if ocr_text:
                retry_prompt += f"\nVISIBLE OCR (support only, may be partial): \"\"\"{ocr_text[:2000]}\"\"\""

            raw_retry = llm_chat_multimodal(
                prompt=retry_prompt,
                model=VISION_MODEL_NAME,
                image_bytes_list=multi_imgs,
                max_tokens=3200,
            ...
            retry_js = safe_json_extract(raw_retry)
            if isinstance(retry_js, dict) and to_text(retry_js.get("mode", "")).strip().lower() == "structured":
                data_js = retry_js  # sostituisci con versione migliore

    # ------------------------------------------------------------
    # Normalizzazione: unit + records -> numbers[] (compatibilità PG/Neo4j)
    # ------------------------------------------------------------
    unit = data_js.get("unit_of_measure") or data_js.get("unit") or schema_js.get("unit_of_measure") or ""
    unit = to_text(unit).strip()

    numbers = []
    records = data_js.get("records") if isinstance(data_js.get("records"), list) else []
    ...
    for r in records:
        ...
        # label/period compat: se troviamo una dim non-time usala come label, e una time come period (se esistono)
        label = "NOT_VISIBLE"
        period = ""

        # period: prima dim time trovata
        for k, v in dims.items():
            sv = to_text(v).strip()
            if not sv:
                continue
        ...
        # label: prima dim non-time trovata
        for k, v in dims.items():
            sv = to_text(v).strip()
            if not sv:
                continue
    ...
    # ------------------------------------------------------------
    # Confidence: evita "1.0" quando mancano dims non-temporali
    # ------------------------------------------------------------
    final_conf = float(data_js.get("confidence", schema_conf) or schema_conf or 0.6)
    try:
        declared_non_time = _structure_declares_non_time_dims(schema_js) or _structure_declares_non_time_dims(data_js)
        has_non_time = _has_non_time_dims_in_records(records)
        if declared_non_time and not has_non_time:
            # il modello “dice” che ci sono Paesi ma non li legge: abbassa conf e segnala unreadable
            final_conf = min(final_conf, 0.55)
            up = data_js.get("unreadable_parts")
            if not isinstance(up, list):
                up = []
            up.append("Etichette di categoria (es. Paesi) non leggibili con affidabilità sufficiente.")
            data_js["unreadable_parts"] = up
    except Exception:
        pass

    # ------------------------------------------------------------
    # Final object: NON imporre schema fisso
    # ------------------------------------------------------------
    final = {}
    final.update(schema_js if isinstance(schema_js, dict) else {})
    final.update(data_js if isinstance(data_js, dict) else {})

    final["page_no"] = page_no
    final["unit_of_measure"] = unit
    final["numbers"] = numbers
    final["confidence"] = final_conf
    final["toon_type"] = "immagine"

    # semantic: usa builder (che ora è robusto e non inventa)
    final["semantic_description"] = build_chart_semantic_chunk(page_no, final)

    # metti anche labels pass se eseguito (debug utile)
    if isinstance(labels_js, dict):
        final["labels_probe"] = labels_js
...
def build_chart_semantic_chunk(page_no: int, a: Dict[str, Any]) -> str:
    ...
    lines.append(f"--- CONTENUTO VISUALE ({kind}) - Pagina {page_no} ---")
    ...
    if a.get("unit_of_measure"):
        lines.append(f"Unità di misura: {a['unit_of_measure']}")

    lines.append(f"Affidabilità (confidence): {a.get('confidence', 0.0)}")

    if a.get("what_is_visible_it"):
        lines.append("Descrizione fattuale (IT):")
        lines.append(a["what_is_visible_it"])
    ...
    if records:
        ...
        for i, r in enumerate(records, 1):
            dims_str = ", ".join([f"{k}={v}" for k, v in r.get("dims", {}).items()])
            val = r.get("value")
            unit = r.get("unit", "")
            evidence = r.get("evidence", "")

            lines.append(f" - Record {i}: {val} {unit}")
            if dims_str:
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    Estrazione chunk da PDF con:
    - Analisi immagini embedded
    - Fallback render pagina intera (grafici vettoriali)
    - Scrittura sempre coerente in DB
    """
    ...
    try:
        ...
        for page_no in range(len(doc)):
            page = doc[page_no]
            page_human = page_no + 1

            found_any_analysis = False

            # -------------------------------------------------
            # 1️⃣ ESTRAZIONE IMMAGINI EMBEDDED
            # -------------------------------------------------
            ...
            for img_idx, img in enumerate(img_list):
                try:
                    ...
                    asset_name = f"Img_P{page_human}_{img_idx}"
                    ...
                    try:
                        analysis = extract_chart_via_vision(
                            img_bytes,
                            context_hint=f"Page {page_human} of {filename}"
                        )
                    except Exception as e:
                        print(f"⚠️ Vision error embedded: {e}")
                    ...
                    if isinstance(analysis, dict):
                        ...
                        sem_text = (
                            f"--- CONTENUTO VISIVO (immagine) - Pagina {page_human} ---\n"
                            f"Asset: {asset_name}\n"
                    ...
                    print(f"⚠️ Errore embedded img: {e}")

            # -------------------------------------------------
            # 2️⃣ FALLBACK RENDER PAGINA (FIX B)
            # -------------------------------------------------
            if PDF_VISION_ENABLED and not found_any_analysis:
                try:
                    pix = page.get_pixmap(dpi=280)
                    page_png = pix.tobytes("png")

                    asset_name = f"PageRender_P{page_human}"
                    ...
                    try:
                        analysis = extract_chart_via_vision(
                            page_png,
                            context_hint=f"Page {page_human} of {filename} (full-page render)"
                        )
                    except Exception as e:
                        print(f"⚠️ Vision error page render: {e}")
                    ...
                    if isinstance(analysis, dict):
                        ...
                        sem_text = (
                            f"--- CONTENUTO VISIVO (pagina render) - Pagina {page_human} ---\n"
                            f"Asset: {asset_name}\n"
                            f"Stato: renderizzata ma non interpretata automaticamente."
                        )
                        raw = sem_text
                        meta = {"asset_name": asset_name}
                    ...
                    print(f"⚠️ Page render fallback fail: {e}")

        return out_chunks

    except Exception as e:
        print(f"❌ Errore estrazione file: {e}")
        import traceback
        traceback.print_exc()
        return []
...
def extract_pdf_as_markdown_assets(
    ...
    """
    Estrae testo + immagini da PDF.
    Le immagini vengono analizzate via Vision e trasformate in chunk semantici.
    """
    ...
    try:
        ...
        for page_index in range(len(doc)):
            ...
            if text and len(text) > MIN_CHUNK_LEN:
                out_chunks.append({
                    "text_raw": text,
                    "text_sem": f"Doc: {filename} | Pagina: {page_no}\n{text}",
                    "page_no": page_no,
                    "toon_type": "text",
                    "section_hint": f"page_{page_no}",
                    "metadata": {}
                })

            # ======================================================
            # 2️⃣ ESTRAZIONE IMMAGINI (VISION)
            # ======================================================
            ...
            for img_index, img in enumerate(image_list):
                ...
                try:
                    ...
                    # Filtro dimensione minima (evita icone/loghi)
                    if len(img_bytes) < 5000:
                        continue

                    print(f"   🖼️ Vision PDF p{page_no} img{img_index}")

                    vision_out = extract_chart_via_vision(
                        img_bytes,
                    ...
                    # Se Vision restituisce generic → comunque salva chunk
                    semantic_text = vision_out.get(
                    ...
                    chunk_data = {
                        "text_raw": "",
                        "text_sem": semantic_text,
                        "page_no": page_no,
                        "toon_type": "immagine",
                        "section_hint": f"Img_P{page_no}_{img_index}",
                        "metadata": vision_out
                    }
                    ...
                    print(f"   ⚠️ Vision PDF error p{page_no} img{img_index}: {e_img}")

        print(f"   📄 PDF Ingested: {len(out_chunks)} chunk totali.")
        return out_chunks
