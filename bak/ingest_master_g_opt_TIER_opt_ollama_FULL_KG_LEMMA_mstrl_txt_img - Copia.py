...
def force_unload_ollama(model_name: str):
    """
    Forza lo scaricamento immediato di un modello dalla VRAM chiamando l'API
    con keep_alive=0. Essenziale per P5000 con MiniCPM-V.
    """
    try:
        # URL di default di Ollama
        url = "http://localhost:11434/api/generate"
        payload = {
            "model": model_name,
            "prompt": "",
            "keep_alive": 0  # <--- QUESTO È IL SEGRETO
        }
        requests.post(url, json=payload, timeout=2)
        # Breve pausa per dare tempo al driver GPU di riorganizzarsi
        time.sleep(0.5)
    except Exception as e:
        print(f"   ⚠️ Warning: Impossibile scaricare {model_name}: {e}")
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    if ext == ".pdf":
        # ✅ FAST PATH: testo layer + Vision SOLO sugli asset
...
def extract_pdf_as_markdown_assets(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for p_idx in range(total_pages):
            ...
            # Label "Figura/Fig/Figure" (cheap)
            has_figure_label = bool(re.search(r"\b(Fig|Figure|Figura)\b", p_text))

            # CHIAMA get_drawings() SOLO se:
            # - non ci sono immagini raster
            # - e c'è un riferimento figura
            has_vector_content = False
            if (not image_list) and has_figure_label:
                try:
            ...
            for img_idx, img in enumerate(image_list):
                ...
                # ✅ filtri anti-icone/loghi (velocità + qualità)
                w = int(base_image.get("width") or 0)
                h = int(base_image.get("height") or 0)

                if w * h < 120_000:          # ~ sotto 400x300 => spesso inutile
                    continue
                if min(w, h) < 220:
                    continue
