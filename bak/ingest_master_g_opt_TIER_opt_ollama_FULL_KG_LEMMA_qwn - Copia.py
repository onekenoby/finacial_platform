...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # ✅ Supporta lo schema del prompt: {"page_type": "...", "content":[...]}
            content = []
            ...
            formulas = [x for x in content if isinstance(x, dict) and x.get("type") == "formula"]
            tables   = [x for x in content if isinstance(x, dict) and x.get("type") == "table"]
            texts    = [x for x in content if isinstance(x, dict) and x.get("type") == "text"]

            has_any = bool(formulas or tables or texts)

            if has_any:
                page_type = (f_js.get("page_type") if isinstance(f_js, dict) else "") or "unknown"
                chunk_text += f"Tipo pagina: {page_type}\n\n"

                if formulas:
                    chunk_text += f"--- FORMULE - Pagina {p_no} ---\n"
                    for i, form in enumerate(formulas[:VISION_MAX_FORMULAS_PER_PAGE], start=1):
                        latex = _latex_clean(form.get("latex", ""))
                        desc  = (form.get("description") or "").strip()
                        if latex:
                            chunk_text += f"{i}) LaTeX: $${latex}$$\n"
                        if desc:
                            chunk_text += f"   Descrizione: {desc}\n"
                        chunk_text += "\n"
                ...
                if texts:
                    chunk_text += f"--- TESTO - Pagina {p_no} ---\n"
                    for tx in texts[:8]:
                        v = (tx.get("value") or "").strip()
                        if v:
                            chunk_text += v + "\n"
            else:
                # fallback controllato: non buttare tutto raw, ma un estratto piccolo
