...
def process_single_file(file_path: str, doc_type: str, doc_meta: dict):
    ...
    # --- CORREZIONE: Chiamata al dispatcher ---
    t_extract = time.time()
    ...
    try:
        ...
        print(f"   🚀 Chunks: {len(valid_chunks)}. Processing Batches...")
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
                ...
                with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as executor:
                    ...
                    for p_no, indexed_chunks in pages_map.items():
                        ...
                        if len(combined_text) > 200: 
                            fut = executor.submit(llm_extract_kg, filename, p_no, combined_text, model_for_kg)
            ...
            # 3) Costruzione Righe e Iniezione Proprietà
            for j, ch in enumerate(batch):
                c_idx, p_no = i + j, ch.get("page_no", 1)
                c_uuid = deterministic_chunk_id(doc_id, p_no, c_idx, ch["toon_type"], ch["text_sem"])
                
                payload = {
                    "tier": tier, "ontology": ontology, "filename": filename, "page": p_no, 
                    "text_sem": ch["text_sem"][:QDRANT_TEXT_MAX_CHARS], "effective_date": effective_date
                }
                qdrant_points.append(models.PointStruct(id=c_uuid, vector=vecs[j].tolist(), payload=payload))
                pg_rows.append((log_id, c_idx, ch["toon_type"], ch["text_raw"], ch["text_sem"], Json(payload), c_uuid))

                # --- RIPRISTINO ENTITÀ (Verdi) ---
                kg_nodes, kg_edges = batch_kg_results.get(j, ([], []))
                for node in kg_nodes:
                    node["props"].update({
                        "source_tier": tier, "ontology": ontology,
                        "document_date": effective_date, "source_file": filename
                    })

                if NEO4J_ENABLED:
                    neo4j_rows.append({
                        "doc_id": doc_id, "filename": filename, "doc_type": doc_type, "log_id": log_id,
                        "chunk_id": c_uuid, "chunk_index": c_idx, "toon_type": ch["toon_type"], "page_no": p_no,
                        "section_hint": ch.get("section_hint", ""), "text_sem": ch["text_sem"], "ontology": ontology,
        ...
        print(f"✅ Done: {filename} in {elapsed}ms")
...
def main():
    ...
    if not force_restart_ollama(num_parallel="4"):
