...
def recurse_text_chunking(text: str, base_meta: Dict[str, Any], max_chars: int = 1000) -> List[Dict[str, Any]]:
    ...
    # =========================
    # PDF extraction (FAST & SMART)
    # =========================
    def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
        """
        Versione Thread-Safe v3.1:
        - Parallelismo reale: ogni worker apre una sua istanza di fitz.Document.
        - Vision Supremacy: Priorità ai dati estratti dall'AI (Formule) rispetto al testo raw.
        - Smart Cleaning: Ignora intestazioni e caratteri corrotti.
        """
        out_chunks: List[Dict[str, Any]] = []
        filename = os.path.basename(file_path)

        # Reset statistiche globali per questo documento
        ...
        # 1. Leggi numero pagine (apertura rapida main thread)
        try:
        ...
        # =========================================================================
        # WORKER INTERNO (Eseguito in parallelo)
        # =========================================================================
        def process_page_worker(p_idx: int) -> List[Dict[str, Any]]:
            p_no = p_idx + 1
            local_results: List[Dict[str, Any]] = []
            
            # APERTURA LOCALIZZATA: Creiamo un oggetto fitz NUOVO per questo thread.
            # Questo previene i crash "File closed" o race conditions di PyMuPDF.
            try:
                doc_worker = fitz.open(file_path)
            except Exception as e:
                print(f"   ❌ Errore worker apertura file p.{p_no}: {e}")
                return []

            try:
                # Carica la pagina specifica
                ...
                # --- A. ANALISI CONTENUTO ---
                p_text_raw = normalize_ws(p_obj.get_text("text", sort=True))
                s_hint = find_section_hint(p_text_raw)
                
                image_info = p_obj.get_image_info(hashes=True)
                has_imgs = len(image_info) > 0

                # Regex e Euristiche
                is_struct = is_structural_page(p_text_raw, p_no=p_no)
                is_math_page = bool(MATH_CANDIDATE_PAT.search(p_text_raw))
                is_visual_page = bool(CHART_CANDIDATE_PAT.search(p_text_raw))
                is_low_text_page = len(p_text_raw.strip()) < 250

                # Decisione Strategia Visione
                # SE è una pagina matematica, ignoriamo il fatto che sembri strutturata (indice).
                # La Visione deve avere la precedenza assoluta sulle formule.
                # LOGICA V3.3: La matematica ha la precedenza sulla struttura.
                # Se il regex math scatta, ignoriamo is_struct.
                do_f = PDF_VISION_ENABLED and (is_math_page or (is_low_text_page and not is_struct))

                # Per i grafici manteniamo la prudenza
                do_c = PDF_VISION_ENABLED and (is_visual_page or has_imgs) and not is_struct
                vision_solved_page = False

                # --- B. VISIONE & STORAGE DB ---
                if do_f or do_c:
                    t_conn = pg_get_conn()
                    if t_conn:
                        try:
                            with t_conn.cursor() as t_cur:
                                # B1. FORMULE (Priorità Alta)
                                if do_f:
                                    full_jpg = render_full_page_jpeg(p_obj, dpi=200) # HD per formule
                                    f_js = extract_formulas_vision(full_jpg) # <--- Funzione Helper Esterna
                                    
                                    if f_js and f_js.get("formulas"):
                                        img_id = pg_save_image(log_id, full_jpg, "image/jpeg", f"Formula_Page_p{p_no}", cur=t_cur)
                                        sem_text = build_formula_semantic_chunk(p_no, f_js) # <--- Funzione Helper Esterna
                                        
                                        local_results.append({
                                            "text_raw": f"Formule Pagina {p_no} (Vedi Metadati)",
                                            "text_sem": sem_text,
                                            "page_no": p_no,
                                            "toon_type": "formula",
                                            "section_hint": s_hint,
                                            ...
                                            "metadata_override": {
                                                ...
                                                "skip_kg": True # Speedup
                                            }
                                        })
                                        vision_solved_page = True
                                        do_c = False # Stop ricerca chart
                                ...
                                if do_c:
                                    ...
                                    # Crop Immagini
                                    if has_imgs:
                                        if "VISION_STATS" in globals():
                                            with _VISION_STATS_LOCK: VISION_STATS["pages_with_imgs"] += 1
                                        ...
                                        for img_meta in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                            try:
                                                ...
                                                if len(crop_jpg) < MIN_IMAGE_BYTES: continue
                                                
                                                c_js = extract_chart_via_vision(crop_jpg) or {}
                                                meta_pl = c_js.copy()
                                                meta_pl["skip_kg"] = False
                                                if c_js.get("data_table_md"): meta_pl["is_vector_ready"] = True

                                                img_id = pg_save_image(log_id, crop_jpg, "image/jpeg", f"Crop p.{p_no}", cur=t_cur)
                                                local_results.append({
                                                    "text_raw": f"Grafico: {c_js.get('title','')}",
                                                    "text_sem": build_chart_semantic_chunk(p_no, c_js, prefix="IMAGE"),
                                                    "page_no": p_no, 
                                                    "toon_type": "immagine", 
                                                    "image_id": img_id, 
                                                    "metadata_override": meta_pl
                                                })
                                                if c_js.get("kind") != "other": found_valid_crop = True
                                            except: continue
                                    
                                    # Fullpage Fallback
                                    if (not found_valid_crop) and (is_low_text_page or (is_visual_page and not has_imgs)):
                                        if "VISION_STATS" in globals():
                                            with _VISION_STATS_LOCK: VISION_STATS["pages_fullpage"] += 1
                                        
                                        full_jpg = render_full_page_jpeg(p_obj, dpi=130)
                                        c_js = extract_chart_via_vision(full_jpg) or {}
                                        if c_js.get("kind") != "other":
                                            img_id = pg_save_image(log_id, full_jpg, "image/jpeg", f"Visual Full p.{p_no}", cur=t_cur)
                                            meta_pl = c_js.copy()
                                            if c_js.get("data_table_md"): meta_pl["is_vector_ready"] = True
                                            
                                            local_results.append({
                                                "text_raw": f"Pagina Visuale: {c_js.get('title','')}",
                                                "text_sem": build_chart_semantic_chunk(p_no, c_js, prefix="VISUAL"),
                                                "page_no": p_no, "toon_type": "immagine", "image_id": img_id, "metadata_override": meta_pl
                                            })
                                            if is_low_text_page: vision_solved_page = True
                ...
                # --- C. TESTO & CHUNKING (Smart Mode) ---
                if vision_solved_page:
                    pass # Ignora testo raw se Visione ha risolto (evita "• – 3 3")
                else:
                    # Clipping: Taglia Header (8%) e Footer (8%)
                    page_w = p_obj.rect.width
                    page_h = p_obj.rect.height
                    clip_rect = fitz.Rect(0, page_h * 0.08, page_w, page_h * 0.92)
                    
                    blocks = p_obj.get_text("blocks", clip=clip_rect)
                    clean_blocks = []
                    for b in blocks:
                        # Smart Cleaning (Funzione Helper Esterna)
                        clean_txt = smart_clean_text(b[4]) 
                        if len(clean_txt) > 10:
                            clean_blocks.append(clean_txt)
                    
                    page_text_clean = "\n\n".join(clean_blocks)

                    if len(page_text_clean) > 50:
                        base_meta = {"source": filename, "page": p_no, "type": "text"}
                        text_chunks = recurse_text_chunking(page_text_clean, base_meta, max_chars=CHUNK_MAX_CHARS)
                        local_results.extend(text_chunks)

            except Exception as e:
                print(f"   ❌ Errore generico worker p.{p_no}: {e}")
            
            finally:
                # === CHIUSURA CRUCIALE DEL DOCUMENTO LOCALE ===
                # Questo chiude l'istanza 'doc_worker' creata all'inizio del thread.
                # Se non lo fai, esaurisci la memoria/handle.
    ...
    # =========================================================================
    with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as executor:
        future_to_page = {executor.submit(recurse_text_chunking, i): i for i in range(total_pages)}
