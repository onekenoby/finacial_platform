...
def build_chart_semantic_chunk(page_no: int, chart_json: Dict[str, Any], prefix: str = "VISUAL") -> str:
    """
    Costruisce una descrizione testuale per RAG del contenuto visivo.
    - page_no in input è 0-based -> qui lo esponiamo 1-based per leggibilità.
    - supporta sia schema "vecchio" (what_is_visible_it, numbers, ecc.)
      sia schema "nuovo" (title, unit_of_measure, x-axis_labels, data_points, visual_trend_description, semantic_description...).
    """
    page_human = (page_no + 1) if isinstance(page_no, int) else page_no

    if not isinstance(chart_json, dict):
        return normalize_ws(f"--- CONTENUTO VISUALE (immagine) - Pagina {page_human} ---\n[chart_json non valido]")

    # Se il modello ha già prodotto una semantic_description pronta, usala (ma correggi pagina se errata)
    sem = chart_json.get("semantic_description")
    if isinstance(sem, str) and sem.strip():
        sem_fixed = sem.replace(f"Pagina 0 ---", f"Pagina {page_human} ---")
        sem_fixed = sem_fixed.replace(f"Pagina {page_no} ---", f"Pagina {page_human} ---")
        return normalize_ws(sem_fixed)

    # kind/toon_type: normalizza
    ...
    # --- Schema “nuovo”
    unit = chart_json.get("unit_of_measure", "")
    xlabels = chart_json.get("x-axis_labels") or chart_json.get("x_axis_labels") or []
    yscale = chart_json.get("y-axis_scale") or chart_json.get("y_axis_scale") or []
    datap = chart_json.get("data_points") or []
    trend = chart_json.get("visual_trend_description", "") or ""
    ...
    # confidence fallback (se assente)
    if conf is None:
        # euristica semplice: se abbiamo datapoints + title -> almeno 0.5
        has_data = bool(datap) or bool(nums)
        conf = 0.7 if (title and has_data) else (0.4 if title else 0.2)

    # timeframe
    raw_timeframe = chart_json.get("timeframe", "") or ""
    years = re.findall(r"\b(19\d{2}|20\d{2})\b", " ".join([to_text(raw_timeframe)] + [to_text(x) for x in xlabels]))
    timeframe = " vs ".join(sorted(set(years))) if years else "NOT READABLE"

    lines = [f"--- CONTENUTO VISUALE ({to_text(kind)}) - Pagina {page_human} ---"]
    if title:
        lines.append(f"Titolo: {to_text(title)}")
    if source:
        lines.append(f"Fonte: {to_text(source)}")
    if timeframe:
        lines.append(f"Periodo: {to_text(timeframe)}")
    if unit:
        lines.append(f"Unità di misura: {to_text(unit)}")

    # legenda (schema vecchio)
    if isinstance(legend, dict) and legend.get("is_readable") and legend.get("mapping"):
        lines.append("Legenda (visibile):")
        mapping = legend.get("mapping") or []
        if not isinstance(mapping, list):
            mapping = [mapping]
        for m in mapping[:8]:
            if isinstance(m, dict):
                lines.append(f" - {to_text(m.get('label',''))}: {to_text(m.get('color_or_style',''))}")
            else:
                s = to_text(m)
                if s.strip():
                    lines.append(f" - {s}")

    lines.append(f"Affidabilità (confidence): {to_text(conf)}")

    # descrizione fattuale
    if to_text(vis).strip():
        lines.append("Descrizione fattuale (IT):")
        lines.append(to_text(vis))

    # trend (schema nuovo)
    if to_text(trend).strip():
        lines.append("Trend / lettura qualitativa:")
        lines.append(to_text(trend))

    # datapoints (schema nuovo)
    if datap:
        lines.append("Dati leggibili (estratti):")
        if not isinstance(datap, list):
            datap = [datap]
        for d in datap[:12]:
            if isinstance(d, dict):
                cat = to_text(d.get("category", ""))
                val = to_text(d.get("value", ""))
                est = d.get("is_estimate", False)
                tail = " (stima)" if est else ""
                if cat or val:
                    lines.append(f" - {cat}: {val}{tail}".strip())
            else:
                s = to_text(d)
                if s.strip():
                    lines.append(f" - {s}")

    # obs / nums / unread (schema vecchio)
    if obs:
    ...
    if nums:
        ...
        for n in nums[:10]:
            if isinstance(n, dict):
                label = to_text(n.get("label", ""))
                value = to_text(n.get("value", ""))
                unit2 = to_text(n.get("unit", ""))
                period = to_text(n.get("period", ""))
                tail = f" ({period})" if period else ""
                lines.append(f" - {label}: {value} {unit2}{tail}".strip())
            else:
                s = to_text(n)
                if s.strip():
                    lines.append(f" - {s}")
    ...
    if to_text(analysis_it).strip():
        lines.append("Analisi (IT) - interpretazione qualitativa:")
        lines.append(to_text(analysis_it))

    lines = [to_text(x) for x in lines]
    return normalize_ws("\n".join(lines))
