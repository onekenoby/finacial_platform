...
KG_MAX_TRIPLES = int(os.getenv("KG_MAX_TRIPLES", "10"))           # soft cap (sanitize already caps)
...
VISION_DPI = int(os.getenv("VISION_DPI", "250"))
...
KG_MIN_LEN = int(os.getenv("KG_MIN_LEN", "800")) #
MAX_KG_CHUNKS_PER_DOC = int(os.getenv("MAX_KG_CHUNKS_PER_DOC", "20"))
...
CHART_SCHEMA_PROMPT = r"""
You are a VISUAL chart analyst with a strict anti-hallucination policy.
You will receive multiple images of the SAME chart (overview + zoom tiles).
Return ONLY valid JSON. No markdown.

Task: infer the chart STRUCTURE based ONLY on what you can SEE.
Do NOT guess missing labels. If not visible, use null.

Return schema:
{
  "mode": "structured" | "generic",
  "chart_kind": "bar"|"line"|"area"|"scatter"|"pie"|"table"|"map"|"diagram"|"unknown",
  "layout": "single_series"|"grouped"|"stacked"|"multi_line"|"unknown",
  "axes": {
    "x_label": null|string,
    "y_label": null|string,
    "x_ticks": [string]|[],
  ...
  "legend": {
    ...
    "is_visible": true|false|null
  },
  "categories": {
    "type": "countries"|"sectors"|"products"|"dates"|"other"|"unknown",
    "labels": [string]|[]
  },
  "measures": {
    "count": number,
...
Rules:
- If you cannot reliably identify axes/legend/categories, set mode="generic".
- In generic mode, do not include axes ticks or category labels unless clearly visible.
"""

CHART_DATA_PROMPT_TEMPLATE = r"""
You are a VISUAL chart digitizer with strict anti-hallucination.
You will receive multiple images of the SAME chart (overview + zoom tiles).
Return ONLY valid JSON. No markdown.

You MUST follow this inferred schema (do not change it):
SCHEMA_JSON:
{schema_json}

Goal: extract ALL visible data points WITHOUT guessing.
- If a value is not readable but bar/point position allows estimation using axis ticks, you MAY estimate and mark is_estimate=true.
- If you cannot even estimate, set value=null and add a note.
...
Return:
{
  ...
  "series": [
    {
      "series_label": null|string,
      "category": null|string,
      "x": null|string|number,
      "y": null|number,
      "is_estimate": true|false,
      "evidence": "short reason (e.g., 'estimated from bar height vs y-ticks')"
    }
  ],
  "summary": "short neutral description of what is visible",
  "confidence": 0.0
}

Rules:
- Do not invent categories/series labels not visible.
- If the chart is not digitizable, return mode="generic", empty series[], and a general description.
"""
...
CHART_DATA_PROMPT = """
You are a precision-oriented Data Extraction AI. 
Analyze the provided image and extract numerical data with 100% factual accuracy.

STRICT RULES:
1. DO NOT invent or hallucinate numbers. If a value is not explicitly written, use your best visual estimate but mark it clearly as "estimate".
2. IDENTIFY: Title, Source, X-Axis labels, Y-Axis scale, and Unit of Measure (e.g., millions, %, USD).
3. DATA POINTS: Extract every visible category and its corresponding value.
4. UNCERTAINTY: If a label or value is unreadable, return "NOT_READABLE".
...
Return ONLY a valid JSON object with the following structure:
{
  ...
  "source": "...",
  "data_points": [
    {"category": "...", "value": "...", "is_estimate": true/false}
  ],
  "visual_trend_description": "Short factual description of the chart's direction (e.g., 'Steep increase from 2020 to 2024')."
}
"""
...
def tile_image_for_vision(img_bytes: bytes, max_side: int = 2600, grid: int = 3) -> List[bytes]:
    """
    Restituisce una lista di immagini: [overview, tiles...]
    grid=3 => 3x3 tiles (9) + overview => 10 immagini (ottimo compromesso).
    """
    from PIL import Image
    import io

    # overview in PNG nitido
    overview = _downscale_and_compress_for_vision(img_bytes, max_side=max_side, output_format="PNG")

    img = Image.open(io.BytesIO(overview)).convert("RGB")
    w, h = img.size

    out: List[bytes] = [overview]
    ...
    for gy in range(grid):
        for gx in range(grid):
            left = gx * tile_w
            upper = gy * tile_h
            right = w if gx == grid - 1 else (gx + 1) * tile_w
            lower = h if gy == grid - 1 else (gy + 1) * tile_h
    ...
    # ROI “semplici” (non adattativi): assi e legenda spesso lì
    # asse Y (sinistra)
    y_axis = img.crop((0, 0, int(w * 0.22), h))
    # asse X (basso)
    x_axis = img.crop((0, int(h * 0.78), w, h))
    # zona legenda (alto-destra)
    legend = img.crop((int(w * 0.55), 0, w, int(h * 0.35)))

    for roi in (y_axis, x_axis, legend):
        buf = io.BytesIO()
        roi.save(buf, format="PNG", optimize=True)
        out.append(buf.getvalue())
...
def extract_chart_via_vision(img_bytes: bytes, context_hint: str = "") -> Optional[Dict[str, Any]]:
    ...
    # page number from hint (come fai già)
    page_no = 0
    if context_hint:
    ...
    # ✅ 100 occhi: overview + tiles + ROI
    multi_imgs = tile_image_for_vision(img_bytes, max_side=2800, grid=3)

    h = hashlib.sha256()
    h.update((VISION_MODEL_NAME or "").encode("utf-8"))
    h.update(b"::chart_multieye_v2::")
    for b in multi_imgs:
        h.update(sha256_hex(b).encode("utf-8"))
    ...
    # OCR di supporto (non obbligatorio)
    try:
        ocr_text = ocr_extract_text(multi_imgs[0])
    except Exception:
        ocr_text = ""

    # ---- PASS 1: SCHEMA ----
    schema_prompt = CHART_SCHEMA_PROMPT
    if context_hint:
        schema_prompt += f"\n\nCONTEXT HINT: {context_hint[:600]}\n"
    if ocr_text:
        schema_prompt += f"\nVISIBLE OCR (support only): \"\"\"{ocr_text[:1400]}\"\"\""

    raw_schema = llm_chat_multimodal(
        prompt=schema_prompt,
        model=VISION_MODEL_NAME,
        image_bytes_list=multi_imgs,
        max_tokens=2000,
        response_format_json=True
    )
    schema_js = safe_json_extract(raw_schema)
    if not isinstance(schema_js, dict):
        schema_js = {"mode": "generic", "chart_kind": "unknown", "confidence": 0.2}

    # se schema è già generic o troppo incerto, chiudiamo con descrizione generica
    if schema_js.get("mode") != "structured" or float(schema_js.get("confidence", 0.0) or 0.0) < 0.45:
        generic_out = {
            "mode": "generic",
            "kind": schema_js.get("chart_kind", "unknown"),
            "page_no": page_no,
            "confidence": float(schema_js.get("confidence", 0.25) or 0.25),
            "what_is_visible_it": "Grafico/immagine non digitizzabile con affidabilità sufficiente. Descrizione qualitativa fornita senza numeri.",
            "analysis_it": "",
            "data_points": [],
            "numbers": [],
            "semantic_description": build_chart_semantic_chunk(page_no, {
                "kind": schema_js.get("chart_kind", "unknown"),
                "title": None,
                "source": None,
                "unit_of_measure": None,
                "confidence": schema_js.get("confidence", 0.25),
                "series": []
            }),
            "schema": schema_js
        }
        _vision_cache_put(cache_key, generic_out)
        return generic_out

    # ---- PASS 2: DATI ----
    schema_json_str = json.dumps(schema_js, ensure_ascii=False)

    data_prompt = CHART_DATA_PROMPT_TEMPLATE.format(schema_json=schema_json_str)
    if context_hint:
        data_prompt += f"\n\nCONTEXT HINT: {context_hint[:600]}\n"
    if ocr_text:
        data_prompt += f"\nVISIBLE OCR (support only): \"\"\"{ocr_text[:1800]}\"\"\""

    raw_data = llm_chat_multimodal(
        prompt=data_prompt,
        model=VISION_MODEL_NAME,
        image_bytes_list=multi_imgs,
        max_tokens=2600,
    ...
    data_js = safe_json_extract(raw_data)
    if not isinstance(data_js, dict):
        data_js = {"mode": "generic", "series": [], "confidence": 0.2, "summary": "Output non strutturato."}

    # fallback generic se dati incoerenti
    if data_js.get("mode") != "structured":
        generic_out = {
            "mode": "generic",
            "kind": schema_js.get("chart_kind", "unknown"),
            "page_no": page_no,
            "confidence": float(data_js.get("confidence", 0.25) or 0.25),
            "what_is_visible_it": data_js.get("summary", "Grafico non digitizzabile con affidabilità sufficiente."),
            "analysis_it": "",
            "data_points": [],
            "numbers": [],
            "semantic_description": build_chart_semantic_chunk(page_no, {
                ...
                "source": None,
                "unit_of_measure": None,
                "confidence": data_js.get("confidence", 0.25),
                "series": []
            }),
            "schema": schema_js
        }
    ...
    # normalizza in un formato che già usi (numbers[] piatto per PG/Neo4j)
    # qui “series” del data_js è una lista di punti (series_label/category/x/y)
    # per compatibilità, trasformiamo in numbers[]: label=(category or series_label), period=x, value=y
    unit = data_js.get("unit") or schema_js.get("axes", {}).get("unit")
    numbers = []
    for p in data_js.get("series", []) if isinstance(data_js.get("series", []), list) else []:
        if not isinstance(p, dict):
            continue
        label = p.get("category") or p.get("series_label") or "NOT_VISIBLE"
        period = p.get("x")
        val = p.get("y")
        if val is None:
            continue
    ...
    # semantic (usa il tuo builder; se vuoi, puoi estenderlo per mostrare numbers)
    final["semantic_description"] = build_chart_semantic_chunk(page_no, final)
    final["toon_type"] = "immagine"

    _vision_cache_put(cache_key, final)
    return final
...
def extract_markdown_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    - Auto-cleaning
    - Chunking strutturato + sub-chunking (anti-paragrafi giganti)
    - FIX 1: le immagini Markdown diventano SEMPRE chunk dedicati 'imagine' (con image_id + metadata)
    - FIX 2: i chunk di testo restano SEMPRE 'testo' (no chart_analysis) + page_no coerente (0-based)
    - Vision prudente: se non affidabile -> mode=generic e nessun numero (anti-allucinazioni)
    """
    ...
    try:
        ...
        #    - separa su header "# " oppure su doppio newline
        raw_paras = re.split(r'\n(?=# )|\n\n', content)

        for idx, macro_para in enumerate(raw_paras):
            macro_para = (macro_para or "").strip()
            if len(macro_para) < MIN_CHUNK_LEN:
                continue

            # =========================================================
            # FIX 1 — immagini MD => chunk "imagine" dedicato (sempre)
            # =========================================================
            vision_assets: List[Dict[str, Any]] = []
            img_matches = re.findall(r'!\[.*?\]\((.*?)\)', macro_para)

            if img_matches and PDF_VISION_ENABLED:
                for img_i, img_path in enumerate(img_matches):
                    try:
                        img_path = (img_path or "").strip().strip('"').strip("'")
                        if not img_path:
                            continue

                        full_img_path = os.path.normpath(os.path.join(os.path.dirname(file_path), img_path))

                        # Esiste? dimensione minima?
                        if (not os.path.exists(full_img_path)) or os.path.getsize(full_img_path) <= 5000:
                            continue

                        with open(full_img_path, "rb") as img_f:
                            img_bytes = img_f.read()

                        # 1) Salva SEMPRE l'immagine in PG (asset management)
                        conn = pg_get_conn()
                        img_id = None
                        asset_name = f"MD_{filename}_P1_{idx}_{img_i}"
                        try:
                        ...
                        # 2) Vision: se non affidabile => generic (no numeri)
                        ...
                        try:
                            c_js = extract_chart_via_vision(
                                img_bytes,
                                context_hint=f"MD:{filename} block={idx} img={img_i}"
                            )
                        except Exception:
                            c_js = None

                        conf = float((c_js or {}).get("confidence") or 0.0)
                        mode = to_text((c_js or {}).get("mode", "")).strip().lower()
                        is_generic = (mode == "generic")

                        # “good” = JSON presente + conf >= soglia + non generic
                        is_good = bool(c_js) and (conf >= CHART_MIN_CONF) and (not is_generic)

                        if is_good:
                            sem = build_chart_semantic_chunk(0, c_js)
                            raw = json.dumps(c_js, ensure_ascii=False)
                            meta = dict(c_js) if isinstance(c_js, dict) else {}
                        else:
                            sem = normalize_ws(
                                f"--- CONTENUTO VISIVO (immagine) - Pagina 1 ---\n"
                                f"Asset: {asset_name}\n"
                                f"Nota: contenuto non digitizzabile con affidabilità sufficiente.\n"
                                f"Nessun numero estratto per evitare allucinazioni."
                            )
                            raw = sem
                            meta = {
                                "mode": "generic",
                        ...
                        # 3) Arricchisci metadata con link forte all'asset
                        meta.update({
                            "image_id": img_id,
                            "asset_name": asset_name,
                            "md_image_path": img_path,
                            "filename": filename,
                        })

                        # 4) Chunk IMMAGINE dedicato
                        out_chunks.append({
                            "text_raw": raw,
                            "text_sem": sem,
                            "page_no": 0,                 # MD: coerente 0-based
                            "toon_type": "imagine",       # coerente col tuo DB
                            "section_hint": f"md_image_{idx}_{img_i}",
                            "metadata": meta
                        })

                        vision_assets.append(meta)

                    except Exception as e_img:
                        print(f"   ⚠️ Vision/Asset Error (MD img): {e_img}")
                        continue

            # =========================================================
            # FIX 2 — SUB-CHUNKING: testo sempre 'testo', no ibridi
            # =========================================================
            sub_chunks_text = fast_chunk_text(macro_para, max_tokens=1024) or [macro_para]

            vision_note = ""
            if vision_assets:
                vision_note = normalize_ws(
                    "Nota: questo blocco include immagini; vedi i chunk separati di tipo 'imagine' "
                    "per analisi, numeri e metadata (image_id)."
                )
            ...
            for sub_i, txt in enumerate(sub_chunks_text):
                ...
                if sub_i == 0 and vision_note:
                    current_text_sem = vision_note + "\n\n" + current_text_sem

                out_chunks.append({
                    "text_raw": txt,
                    "text_sem": current_text_sem,
                    "page_no": 0,           # MD: 0-based coerente
                    "toon_type": "testo",   # standard
...
def llm_chat_multimodal(
    ...
    image_bytes_list: Optional[List[bytes]] = None,   # ✅ NEW
) -> str:
    if force_json is not None:
        response_format_json = bool(force_json)

    # normalizza lista immagini
    images_b64: List[str] = []
    if image_bytes_list:
...
def extract_formulas_vision(img_bytes: bytes) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # ✅ accetta sia schema vecchio ("formulas") che nuovo ("content")
        if not js:
            return None

        if ("formulas" not in js) and ("content" not in js):
            return None
...
def normalize_chart_json_for_semantics(js: Dict[str, Any], page_no: int, context_hint: str = "") -> Dict[str, Any]:
    """
    Normalizza output Vision eterogeneo in uno schema compatibile con build_chart_semantic_chunk().
    Gestisce casi come:
      - x-axis_labels / y-axis_scale / unit_of_measure / data_points
      - confidence mancante
      - timeframe mancante ma anni presenti in x-axis_labels
    """
    if not isinstance(js, dict):
        return {}
    ...
    # Kind / toon_type
    # ----------------------------
    # build_chart_semantic_chunk legge "kind"
    if not out.get("kind"):
        # prova inferenze semplici
        if out.get("data_points") and out.get("x-axis_labels"):
            out["kind"] = "bar_chart"
        else:
            out["kind"] = out.get("toon_type") or "immagine"

    # uniforma toon_type al tuo mondo
    ...
    if not tf or str(tf).strip() in ("", "NOT READABLE", "None"):
        # se hai x-axis_labels con anni, costruisci timeframe
        ...
        if isinstance(xlab, (list, tuple)):
            ...
            years = list(dict.fromkeys(years))  # unique keep order
            if len(years) >= 2:
    ...
    if conf is None:
        # euristica semplice ma utile: evita 0.0 fisso
        score = 0.25
        ...
        if isinstance(dps, list) and len(dps) >= 2: score += 0.25
        if isinstance(dps, list) and len(dps) >= 4: score += 0.10
    ...
    # ----------------------------
    # what_is_visible_it / observations_it / numbers
    # ----------------------------
    if not out.get("what_is_visible_it"):
        # fallback: descrizione fattuale breve
    ...
    if not out.get("observations_it"):
        # se hai visual_trend_description (inglese), lo metti come osservazione
    ...
    # build_chart_semantic_chunk legge "numbers": lista di dict label/value/unit/period
        ...
        if isinstance(dps, list):
            for dp in dps[:12]:
                if isinstance(dp, dict):
                    numbers.append({
                        "label": dp.get("category") or dp.get("label") or "",
                        "value": dp.get("value"),
    ...
    # analisi_it: se non c’è, non inventiamo; ok lasciare vuoto
...
def extract_chart_via_vision(img_bytes: bytes, context_hint: str = "") -> Optional[Dict[str, Any]]:
    """
    Estrazione VISIVA (no fix adattativi al tipo di grafico).
    Approccio "100 occhi": multi-tiles a più scale + prompt unico che:
      - descrive ciò che vede
      - se possibile digitalizza TUTTI i valori visibili
      - se NON è sicuro -> mode="generic" e NIENTE numeri (anti-allucinazioni)

    Output: dict compatibile col tuo DB (numbers[] + data_points[] + semantic_description).
    """
    if not img_bytes:
        return None
    ...
    # ------------------------------------------------------------
    # page number from hint (manteniamo la tua logica)
    # NB: page_no qui resta quello che passi nel context_hint (nel tuo caso 0-based)
    # build_chart_semantic_chunk lo espone 1-based
    # ------------------------------------------------------------
    page_no = 0
    if context_hint:
        m = re.search(r"\bpage\s+(\d+)\b", context_hint, re.I)
    ...
    # ------------------------------------------------------------
    # ✅ 100 occhi (solo visione): tiles multi-scala
    # (nessun crop "adattativo": generiamo SEMPRE più viste)
    # ------------------------------------------------------------
    multi_imgs = []
    try:
        # overview + griglie diverse + risoluzioni diverse
        multi_imgs.extend(tile_image_for_vision(img_bytes, max_side=2200, grid=1))
        multi_imgs.extend(tile_image_for_vision(img_bytes, max_side=2800, grid=2))
        multi_imgs.extend(tile_image_for_vision(img_bytes, max_side=3200, grid=3))
        multi_imgs.extend(tile_image_for_vision(img_bytes, max_side=3600, grid=4))
    except Exception:
        # fallback minimale
        multi_imgs = [img_bytes]

    # de-dup per non sprecare token/costi
    seen = set()
    dedup = []
    for b in multi_imgs:
        try:
        ...
        seen.add(k)
        dedup.append(b)
    multi_imgs = dedup[:16]  # limite prudente

    # ------------------------------------------------------------
    # ✅ FIX 1 — cache key robusta (NON solo multi_imgs[0])
    # ------------------------------------------------------------
    h = hashlib.sha256()
    h.update((VISION_MODEL_NAME or "").encode("utf-8"))
    h.update(b"::chart_multieye_v2::")
    ...
    # ------------------------------------------------------------
    # Prompt unico, **non vincolato** a un tipo di grafico.
    # Il modello deve:
    #  - dichiarare se può digitizzare (structured) o no (generic)
    #  - NON inventare numeri
    #  - produrre records "dinamici": dims + value + unit + evidence
    # ------------------------------------------------------------
    prompt = """
You are a VISUAL EXTRACTION engine.
You MUST extract ONLY what you can actually SEE in the image(s). Do NOT guess.

Return JSON ONLY with this shape (do not add extra keys at top-level):

{
  "mode": "structured" | "generic",
  "confidence": 0.0-1.0,
  "what_is_visible_it": "short factual description in Italian of what is visible",
  "title": "if visible else empty",
  "source": "if visible else empty",
  "unit_of_measure": "if visible else empty",
  "structure": {
    "dimensions": ["list of visible dimensions you can infer from labels, e.g. year, country, category, series, scenario..."],
    "measures": ["list of visible measures, e.g. exports, percent, units..."],
    "legend": {
      "is_readable": true|false,
      "mapping": [{"label":"...", "style":"color/pattern/line"}]
    }
  },
  "records": [
    {
      ...
      "value": number,
      "unit": "unit if visible else empty",
      "is_estimate": true|false,
      "evidence": "VERY short: what you used to read the value (tick marks / printed label / bar height)."
    }
  ],
  "unreadable_parts": ["list of parts that are not readable / ambiguous"]
}

Rules:
- If you cannot reliably extract numeric values for most items, set mode="generic" and records=[]
- If you see multiple values per category (e.g., 2020 and 2024), you MUST create multiple records with dims including the year (or other dimension you see).
- If the chart has 2 bars per country, output 2 records per country with dims {"country": "...", "year":"2020"} and {"country":"...","year":"2024"} (ONLY if you can see it).
- Never hallucinate.
"""

    if context_hint:
        prompt += f"\n\nCONTEXT HINT (may help locate page): {context_hint[:600]}\n"
    if ocr_text:
        prompt += f'\nVISIBLE OCR (support only, may contain noise): """{ocr_text[:1800]}"""\n'
    ...
    if not isinstance(js, dict):
        js = {
            "mode": "generic",
            "confidence": 0.2,
            "what_is_visible_it": "Output non strutturato. Immagine/grafico non digitizzabile con affidabilità sufficiente.",
            "title": "",
            "source": "",
            "unit_of_measure": "",
            "structure": {"dimensions": [], "measures": [], "legend": {"is_readable": False, "mapping": []}},
            "records": [],
            "unreadable_parts": ["JSON non valido dal modello."]
        }

    # normalizza campi base
    mode = (js.get("mode") or "generic").strip().lower()
    conf = float(js.get("confidence", 0.0) or 0.0)
    title = js.get("title") or ""
    source = js.get("source") or ""
    unit = js.get("unit_of_measure") or ""
    what = js.get("what_is_visible_it") or ""

    records = js.get("records") if isinstance(js.get("records"), list) else []
    unread = js.get("unreadable_parts") if isinstance(js.get("unreadable_parts"), list) else []

    # ------------------------------------------------------------
    # Anti-allucinazioni: se generic o conf bassa -> niente numeri
    # ------------------------------------------------------------
    MIN_CONF = float(os.getenv("CHART_MIN_CONF", "0.55"))
    if mode != "structured" or conf < MIN_CONF:
        final = {
            "mode": "generic",
            "kind": (js.get("kind") or js.get("chart_kind") or "unknown"),
            "page_no": page_no,
            "confidence": conf if conf > 0 else 0.25,
            "title": title,
            "source": source,
            "unit_of_measure": unit,
            "what_is_visible_it": what or "Grafico/immagine non digitizzabile con affidabilità sufficiente. Descrizione qualitativa senza numeri.",
            "analysis_it": "",
            "data_points": [],
            "numbers": [],
            "unreadable_parts": unread,
            "schema": js.get("structure", {}),
            "toon_type": "immagine"
        }
        # ✅ FIX 2: semantic_description dal final reale (include mode)
        final["semantic_description"] = build_chart_semantic_chunk(page_no, final)
        _vision_cache_put(cache_key, final)
        return final

    # ------------------------------------------------------------
    # Structured: convertiamo records -> numbers[] + data_points[]
    # (compatibile con la tua pipeline/PG)
    # ------------------------------------------------------------
    numbers = []
    data_points = []
    ...
    for r in records:
        ...
        # label compatibile col tuo DB (category/country ecc.)
        label = (
            dims.get("country")
            or dims.get("categoria")
            or dims.get("category")
            or dims.get("label")
            or "NOT_VISIBLE"
        )

        # period = dimensione più tipica (year / x / periodo)
        period = dims.get("year") or dims.get("anno") or dims.get("period") or dims.get("x") or ""

        is_est = bool(r.get("is_estimate", False))
        ev = (r.get("evidence") or "").strip()

        numbers.append({
            "label": str(label),
            "value": v,
            "unit": str(r.get("unit") or unit or ""),
            "period": str(period),
            "is_estimate": is_est,
            "evidence": ev[:220]
        })

        # data_points (formato semplice)
        data_points.append({
            "category": str(label),
            "value": v,
            "period": str(period),
    ...
    final = {
        ...
        "source": source,
        "unit_of_measure": unit,
        "what_is_visible_it": what,
        "analysis_it": "",
        "structure": js.get("structure", {}),
        "unreadable_parts": unread,
        "records": records,        # tienilo: è oro per debug “visivo”
        "data_points": data_points,
        "numbers": numbers,
        "toon_type": "immagine"
    }

    # ✅ FIX 2: semantic_description costruita dal final completo (così mostra i numeri se vuoi)
    final["semantic_description"] = build_chart_semantic_chunk(page_no, final)
...
def build_chart_semantic_chunk(page_no: int, chart_json: Dict[str, Any], prefix: str = "VISUAL") -> str:
    """
    Descrizione RAG del contenuto visivo con approccio GENERICO e anti-allucinazioni.

    Regole:
    - Se chart_json["mode"] == "generic" => NON stampa numeri.
    - Non assume tipo di grafico.
    - Se records/numbers/data_points esistono, stampa quello che c'è.
    - NON usa dimensioni temporali (Anno/Year/Date/Time) come "categoria".
      Se manca una categoria non-temporale, stampa i record in forma neutra.
    """
    page_human = (page_no + 1) if isinstance(page_no, int) else page_no

    if not isinstance(chart_json, dict):
        return normalize_ws(
            f"--- CONTENUTO VISUALE (immagine) - Pagina {page_human} ---\n[chart_json non valido]"
        )

    # ----------------------------
    # (0) semantic_description già pronta
    # ----------------------------
    sem = chart_json.get("semantic_description")
    if isinstance(sem, str) and sem.strip():
        sem_fixed = sem.replace("Pagina 0 ---", f"Pagina {page_human} ---")
        sem_fixed = sem_fixed.replace(f"Pagina {page_no} ---", f"Pagina {page_human} ---")
        return normalize_ws(sem_fixed)

    # ----------------------------
    # (1) Normalizzazioni base
    # ----------------------------
    mode = to_text(chart_json.get("mode", "")).strip().lower()

    kind = (chart_json.get("kind") or chart_json.get("toon_type") or "image")
    ...
    unit = to_text(chart_json.get("unit_of_measure", "")).strip()
    what = to_text(chart_json.get("what_is_visible_it", "")).strip()

    conf = chart_json.get("confidence", None)
    if conf is None:
        has_any_data = bool(chart_json.get("records")) or bool(chart_json.get("numbers")) or bool(chart_json.get("data_points"))
        conf = 0.6 if has_any_data else 0.25
    try:
        conf_f = float(conf)
    except Exception:
        conf_f = 0.25
    ...
    # (2) Header
    # ----------------------------
    lines = [f"--- CONTENUTO VISUALE ({kind}) - Pagina {page_human} ---"]
    if title:
        lines.append(f"Titolo: {title}")
    if source:
        lines.append(f"Fonte: {source}")
    if unit:
        lines.append(f"Unità di misura: {unit}")
    lines.append(f"Affidabilità (confidence): {to_text(conf_f)}")

    if what:
        lines.append("Descrizione fattuale (IT):")
        lines.append(what)

    # structure info (se presente)
    structure = chart_json.get("structure")
    if isinstance(structure, dict):
        dims = structure.get("dimensions")
        meas = structure.get("measures")
        if isinstance(dims, list) and any(to_text(x).strip() for x in dims):
            lines.append("Dimensioni individuate (visibili):")
            lines.append(" - " + ", ".join([to_text(x).strip() for x in dims if to_text(x).strip()][:12]))
        if isinstance(meas, list) and any(to_text(x).strip() for x in meas):
            lines.append("Misure individuate (visibili):")
            lines.append(" - " + ", ".join([to_text(x).strip() for x in meas if to_text(x).strip()][:12]))

    unread = chart_json.get("unreadable_parts")
    if isinstance(unread, list) and any(to_text(x).strip() for x in unread):
        lines.append("Parti non leggibili / ambigue:")
        for u in unread[:10]:
            s = to_text(u).strip()
            if s:
                lines.append(f" - {s}")
    ...
    # ----------------------------
    # (3) mode == generic => stop numeri
    # ----------------------------
    if mode == "generic":
        if not what:
            lines.append("Descrizione qualitativa: grafico/immagine non digitizzabile con affidabilità sufficiente.")
        lines.append("Nessun numero estratto per evitare allucinazioni.")
        return normalize_ws("\n".join([to_text(x) for x in lines]))

    # ----------------------------
    # (4) Helpers “generici” per records
    # ----------------------------
    TIME_KEYS = {
        "anno", "year", "years", "date", "data", "time", "tempo",
        "periodo", "period", "mese", "month", "giorno", "day", "quarter", "trimestre"
    }

    def _is_time_dim_key(k: str) -> bool:
        kk = to_text(k).strip().lower()
        return any(t in kk for t in TIME_KEYS)

    def _looks_like_year(v: Any) -> bool:
        sv = to_text(v).strip()
        return bool(re.fullmatch(r"(19\d{2}|20\d{2})", sv))
    ...
    def _best_category_from_dims(dims: Dict[str, Any]) -> str:
        """
        Categoria = dimensione NON temporale più informativa.
        Se non c'è, ritorna NOT_VISIBLE (non usare Anno come categoria).
        """
        if not isinstance(dims, dict) or not dims:
            return "NOT_VISIBLE"

        # 1) priorità a chiavi semanticamente "categoria"
        preferred = ("country", "paese", "categoria", "category", "label", "serie", "series", "name")
        for k in preferred:
            v = to_text(dims.get(k, "")).strip()
            if v:
                return v

        # 2) qualsiasi dimensione NON time
        for k, v in dims.items():
            sv = to_text(v).strip()
            if not sv:
                continue
            if _is_time_dim_key(k) or _looks_like_year(sv):
                continue
            return sv
    ...
    # ----------------------------
    # (5) Colleziona righe dati da records > numbers > data_points
    # ----------------------------
    rows = []

    recs = chart_json.get("records")
    if isinstance(recs, list) and recs:
        for r in recs:
            ...
            if val is None:
                continue
            rows.append({
                "category": _best_category_from_dims(dims),
                "dims_sig": _dims_signature(dims),
                "value": val,
                "unit": to_text(r.get("unit") or unit or "").strip(),
                "is_estimate": bool(r.get("is_estimate", False)),
                "evidence": to_text(r.get("evidence", "")).strip(),
            })
    ...
    if not rows:
        ...
        if isinstance(nums, list) and nums:
            for n in nums:
                ...
                if val is None:
                    continue
                label = to_text(n.get("label", "")).strip()
                period = to_text(n.get("period", "")).strip()
                # se label è NOT_VISIBLE o vuoto, non lo usiamo come categoria
                cat = label if (label and label != "NOT_VISIBLE") else "NOT_VISIBLE"
                sig = "; ".join([p for p in [f"period={period}" if period else ""] if p])
                rows.append({
                    "category": cat,
                    "dims_sig": sig,
                    "value": val,
                    "unit": to_text(n.get("unit") or unit or "").strip(),
                    "is_estimate": bool(n.get("is_estimate", False)),
                    "evidence": to_text(n.get("evidence", "")).strip(),
                })

    if not rows:
        dps = chart_json.get("data_points")
        if isinstance(dps, list) and dps:
            for d in dps:
                ...
                if val is None:
                    continue
                cat = to_text(d.get("category", "")).strip() or "NOT_VISIBLE"
                if cat == "NOT_VISIBLE":
                    cat = "NOT_VISIBLE"
                period = to_text(d.get("period", "")).strip()
                sig = "; ".join([p for p in [f"period={period}" if period else ""] if p])
                rows.append({
                    "category": cat,
                    "dims_sig": sig,
                    "value": val,
                    "unit": to_text(d.get("unit") or unit or "").strip(),
                    "is_estimate": bool(d.get("is_estimate", False)),
                    "evidence": "",
                })

    # ----------------------------
    # (6) Rendering: se categoria manca => stampa record neutri (no “2020 come categoria”)
    # ----------------------------
    if rows:
        lines.append("Dati leggibili (estratti):")

        # se la maggior parte delle categorie è NOT_VISIBLE => output neutro record-by-record
        not_visible_count = sum(1 for r in rows if r.get("category") == "NOT_VISIBLE")
        if not_visible_count >= max(1, int(0.6 * len(rows))):
            for i, r in enumerate(rows[:40], start=1):
                sig = r.get("dims_sig", "")
                u = (" " + r.get("unit", "").strip()).rstrip()
                est = " (stima)" if r.get("is_estimate") else ""
                lines.append(f" - Record {i}: {to_text(r.get('value'))}{u}{est}")
                if sig:
                    lines.append(f"   · dims: {sig}")
                ev = (r.get("evidence") or "").strip()
                if ev:
                    lines.append(f"   · evidenza: {ev[:220]}")
        else:
            # raggruppa per categoria reale
            grouped = {}
            for r in rows:
                grouped.setdefault(r["category"], []).append(r)

            for cat in sorted(grouped.keys())[:30]:
                items = grouped[cat]
                items = sorted(items, key=lambda x: x.get("dims_sig", ""))
                for it in items[:25]:
                    sig = it.get("dims_sig", "")
                    tail_sig = f" [{sig}]" if sig else ""
                    u = (" " + it.get("unit", "").strip()).rstrip()
                    est = " (stima)" if it.get("is_estimate") else ""
                    lines.append(f" - {cat}{tail_sig}: {to_text(it.get('value'))}{u}{est}".strip())
                    ev = (it.get("evidence") or "").strip()
                    if ev:
                        lines.append(f"   · evidenza: {ev[:220]}")
    else:
        lines.append("Nessun valore numerico leggibile in modo affidabile da questa immagine.")

    # trend / analisi se presenti (solo se già forniti dal modello)
    trend = to_text(chart_json.get("visual_trend_description", "")).strip()
    if trend:
        lines.append("Trend / lettura qualitativa:")
        lines.append(trend)

    analysis_it = to_text(chart_json.get("analysis_it", "")).strip()
    if analysis_it:
...
def llm_extract_kg(filename: str, page_no, text: str, model_name: str):
    ...
    try:
        # Chiamata a Ollama con JSON Mode attivo
        resp = chat(
            model=model_name,
            messages=[
            ...
            options={
                "temperature": 0.0, 
                "num_predict": 1500, # <--- MODIFICA CRITICA QUI
                "num_ctx": 4096      # <--- MODIFICA CRITICA QUI
            }
        )
        ...
        # Con format="json", il contenuto è già un JSON puro
        ...
        print(f"   ⚠️ [KG-RETRY] {base} p{page_no}: JSON incompleto (troncato).")
        return [], []
    except Exception as e:
        return [], []
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    ...
    workers = int(os.environ.get("VISION_PARALLEL_WORKERS", "3"))
...
def extract_file_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    # 1. Estrazione del testo nativo (veloce e preciso per il KG)
    native_text_pages = extract_pdf_text_by_page_pdfminer(file_path)

    filename = os.path.basename(file_path)
    final_chunks: List[Dict[str, Any]] = []
    doc = None  # handle per chiusura sicura

    # --- helper: detect grafica vettoriale (grafici/diagrammi) ---
    def _vector_ops_count(p: fitz.Page) -> int:
        try:
    ...
    try:
        # 2. Rilevamento Asset (Immagini/Grafici) con PyMuPDF
        ...
        for i, page_text in enumerate(native_text_pages):
            ...
            # A) CHUNK TESTO
            # =========================================================
            if len(page_text.strip()) > MIN_CHUNK_LEN:
                final_chunks.append({
                    "text_raw": page_text,
                    "text_sem": f"Page {page_no} content: {page_text[:100]}...\n{page_text}",
                    "toon_type": "testo",
                    "page_no": page_no,
            ...
            # =========================================================
            # B) ASSET VISIVI (embedded + vector fallback)
            # =========================================================
            if i < len(doc):
                page = doc[i]

                # B1) Embedded images (raster)
                page_objs = page.get_images(full=True) or []
                found_any_asset = False
                    ...
                    with t_conn.cursor() as t_cur:
                        ...
                        for img_idx, img in enumerate(page_objs):
                            ...
                            try:
                                ...
                                found_any_asset = True

                                asset_name = f"Img_P{page_no}_{img_idx}"
                                img_id = pg_save_image(log_id, img_bytes, "image/png", asset_name, cur=t_cur)
                                ...
                                if PDF_VISION_ENABLED and VISION_MODEL_NAME:
                                    try:
                                        ...
                                        print(f"   ⚠️ Vision FAIL su {asset_name} (pag {page_no}): {e}")
                                        analysis = None
                                ...
                                if analysis:
                                    ...
                                    sem_text = normalize_ws(
                                        f"--- CONTENUTO VISIVO (immagine) - Pagina {page_no} ---\n"
                                        f"Asset: {asset_name}\n"
                                        f"Stato: estratta ma non interpretata automaticamente."
                                    )
                                    raw = sem_text
                                    meta = {"asset_name": asset_name, "source": filename}

                                # ✅ CHUNK IMMAGINE (sempre, appena hai image_id)
                                final_chunks.append({
                                    "text_raw": raw,
                                    "text_sem": sem_text,
                                    "toon_type": "imagine",   # coerente con normalize_toon_type
                                    "page_no": page_no,
                                    "image_id": img_id,
                                    "metadata": meta
                                })
                                ...
                                print(f"   ⚠️ Errore processamento immagine {img_idx} a pag {page_no}: {e_inner}")
                                continue

                        # ------------------------------
                        # B2) FIX: Grafici vettoriali
                        # ------------------------------
                        # Se NON ci sono immagini embedded ma ci sono molti drawings,
                        # renderizziamo la pagina in JPEG e la trattiamo come asset.
                        if (not found_any_asset) and PDF_VISION_ENABLED:
                            ops = _vector_ops_count(page)

                            # soglia: 20-40 tipico; alzala se prende troppi falsi positivi
                            VECTOR_DRAWINGS_THRESHOLD = int(os.getenv("VECTOR_DRAWINGS_THRESHOLD", "20"))

                            if ops >= VECTOR_DRAWINGS_THRESHOLD:
                                try:
                                    full_bytes = render_full_page_jpeg(page, dpi=VISION_DPI)

                                    # Gatekeeper: evita chiamate Vision inutili
                                    if ai_vision_gatekeeper(full_bytes):
                                        asset_name = f"Vec_P{page_no}_FULLPAGE"
                                        img_id = pg_save_image(log_id, full_bytes, "image/jpeg", asset_name, cur=t_cur)
                                        ...
                                        if VISION_MODEL_NAME:
                                            try:
                                                analysis = extract_chart_via_vision(
                                                    full_bytes,
                                                    context_hint=f"VECTOR page {page_no} of {filename}"
                                                )
                                            except Exception:
                                                analysis = None
                                        ...
                                        if analysis:
                                            ...
                                            sem_text = normalize_ws(
                                                f"--- CONTENUTO VISIVO (fullpage) - Pagina {page_no} ---\n"
                                                f"Asset: {asset_name}\n"
                                                f"Nota: pagina renderizzata (grafico vettoriale probabile), ma non interpretata.\n"
                                                f"Hint: aumenta VISION_DPI (es. 200/250) se serve."
                                            )
                                            raw = sem_text
                                            meta = {"asset_name": asset_name, "source": filename, "vector": True, "vector_ops": ops}

                                        # ✅ CHUNK IMMAGINE VETTORIALE
                                        final_chunks.append({
                                            "text_raw": raw,
                                            "text_sem": sem_text,
                                            "toon_type": "imagine",
                                    ...
                                    print(f"   ⚠️ Errore fallback vettoriale pag {page_no}: {e_vec}")

                    t_conn.commit()
                except Exception as e_db:
                    t_conn.rollback()
                    print(f"   ⚠️ Errore DB pagina {page_no}: {e_db}")
                finally:
                    pg_put_conn(t_conn)

    except Exception as e:
        print(f"   ❌ Errore critico estrazione PDF: {e}")
    finally:
        if doc:
            doc.close()
