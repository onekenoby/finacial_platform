...
DB_FLUSH_SIZE = int(os.getenv("DB_FLUSH_SIZE", "256"))          # un po' più alto per meno flush - 
EMBED_BATCH_SIZE = int(os.getenv("EMBED_BATCH_SIZE", "128"))    # se la tua VRAM regge, aumenta velocità
...
PDF_MAX_IMAGES_PER_PAGE = int(os.getenv("PDF_MAX_IMAGES_PER_PAGE", "15"))
...
KG_PROMPT = """
...
1. NODES: Extract entities. For each entity, populate the 'properties' field with relevant attributes found in the text, such as:
   - For Companies: sector, ticker, headquarters.
   - For Metrics: value, unit, timeframe, trend.
   - For Events: date, impact_level (high/med/low).
...
"""
...
# =========================
def llm_extract_kg(filename: str, page_no: int, text: str, model_name: str = LLM_MODEL_NAME) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Estrae il Knowledge Graph utilizzando le opzioni di memoria ottimizzate per GPU da 16GB.
    """
    t0 = time.perf_counter()
    kg_text = f"ANALYSIS OF {filename} PAGE {page_no}:\n{text[:6000]}"
    
    try:
        resp = chat(
            model=model_name,
            messages=[{"role": "system", "content": KG_PROMPT}, {"role": "user", "content": kg_text}],
            format="json",
            options={
                "temperature": 0.1,    # Stabilità nelle estrazioni 
                "num_predict": 3072,   # Ampio spazio per grafi complessi 
                "num_ctx": 4096,       # Risparmio critico di VRAM per P5000 
                "num_thread": 8        # Ottimizzazione CPU 
            }
        )
    ...
    print(f"   ⏱️ {filename} | KG ({model_name}) p{page_no}: {dt:.2f}s")
...
def extract_pdf_chunks(file_path: str, log_id: int) -> List[Dict[str, Any]]:
    """
    VERSIONE UPGRADE 2.5: PARALLELISMO + CROP VISION (OTTIMIZZATA)
    - Rimozione integrale PDFMiner per velocità massima
    - Vision via Clipping (bbox) per grafici e tabelle
    - Parallelizzazione mantenuta su ThreadPoolExecutor
    """
    out_chunks: List[Dict[str, Any]] = []
    doc = fitz.open(file_path) # Utilizzo di PyMuPDF per l'apertura del file

    try:
        total_pages = len(doc)
        print(f"   🔍 PDF pages: {total_pages} | High-Speed Parallel Vision (Workers: {VISION_PARALLEL_WORKERS})")
        ...
        def process_page_worker(p_idx):
            ...
            # 1. Estrazione Testo ultra-rapida con fitz
            p_text = normalize_ws(p_obj.get_text("text"))
            s_hint = find_section_hint(p_text)
            
            # Recupero info immagini per il cropping selettivo
            image_info = p_obj.get_image_info(hashes=True)
            has_imgs = len(image_info) > 0

            # 2. Creazione Chunk Testuali
            if p_text:
                for ch in split_paragraphs(p_text, CHUNK_MAX_CHARS, CHUNK_OVERLAP_CHARS):
                    local_results.append({
                        "text_raw": ch, "text_sem": ch, "page_no": p_no,
                        "toon_type": "text", "section_hint": s_hint
                    })

            # 3. Logica Vision Strategica
            do_f = PDF_VISION_ENABLED and is_formula_candidate_page(p_text)
            do_c = PDF_VISION_ENABLED and (is_chart_candidate_page(p_text) or has_imgs)
            
            # Filtro per testo scarso
            if PDF_VISION_ONLY_IF_TEXT_SCARSO and len(p_text) >= PDF_MIN_TEXT_LEN_FOR_NO_VISION and not has_imgs:
                do_f = False
                do_c = False
                ...
                try:
                    ...
                    try:
                        with t_conn.cursor() as t_cur:
                            # --- VISION FORMULE (Pagina intera) ---
                            if do_f:
                                full_jpg = render_full_page_jpeg(p_obj, dpi=VISION_DPI)
                                f_js = extract_formulas_via_vision(full_jpg)
                                if f_js and f_js.get("has_formulas") and f_js.get("formulas"):
                                    pg_save_image(log_id, full_jpg, "image/jpeg", f"PDF p.{p_no} formulas", cur=t_cur)
                                    sem = build_formula_semantic_chunk(p_no, f_js)
                                    local_results.append({
                                        "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                        "toon_type": "formula_page", "section_hint": s_hint,
                                        "metadata_override": f_js
                                    })
                            
                            # --- VISION GRAFICI (Cropping selettivo via BBox) ---
                            if do_c:
                                if has_imgs:
                                    # Analizziamo solo i ritagli delle immagini
                                    for img in image_info[:PDF_MAX_IMAGES_PER_PAGE]:
                                        bbox = img["bbox"]
                                        crop_pix = p_obj.get_pixmap(clip=bbox, dpi=VISION_DPI)
                                        crop_jpg = crop_pix.tobytes("jpg")
                                        
                                        if len(crop_jpg) >= MIN_IMAGE_BYTES:
                                            c_js = extract_chart_via_vision(crop_jpg)
                                            if c_js and c_js.get("kind") not in ("other", None):
                                                pg_save_image(log_id, crop_jpg, "image/jpeg", f"PDF p.{p_no} crop", cur=t_cur)
                                                sem = build_chart_semantic_chunk(p_no, c_js, prefix="CROP_VISUAL")
                                                local_results.append({
                                                    "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                                    "toon_type": "image_vision", "section_hint": s_hint,
                                                    "metadata_override": c_js
                                                })
                                # Fallback per grafici rilevati via testo
                                elif is_chart_candidate_page(p_text):
                                    full_jpg = render_full_page_jpeg(p_obj, dpi=VISION_DPI)
                                    c_js = extract_chart_via_vision(full_jpg)
                                    if c_js and c_js.get("kind") not in ("other", None):
                                        pg_save_image(log_id, full_jpg, "image/jpeg", f"PDF p.{p_no} chart vision", cur=t_cur)
                                        sem = build_chart_semantic_chunk(p_no, c_js, prefix="PAGE_VISUAL")
                                        local_results.append({
                                            "text_raw": sem, "text_sem": sem, "page_no": p_no,
                                            "toon_type": "image_vision", "section_hint": s_hint,
                                            "metadata_override": c_js
                    ...
                    print(f"   ⚠️ Page vision failed p.{p_no}: {e}")
        ...
        out_chunks.sort(key=lambda x: (x['page_no'], x.get('chunk_index', 0)))
...
# TIME CHECKER END
    ...
    try:
        ...
        # --- INIZIO BLOCCO OTTIMIZZATO: EMBEDDING + KG PARALLELO ---
        # Inizializza executor per i flush asincroni in background
        ...
        for i in range(0, len(valid_chunks), EMBED_BATCH_SIZE):
            ...
            log_phase(filename, f"embed_batch_{len(texts)}", _ms(t_embed))

            # 2) KG EXTRACTION PARALLELA
            batch_kg_results = {}
            # --- LOGICA KG BATCHING ---
            # --- LOGICA KG MEGA-BATCHING (Per Pagina) ---
            # --- LOGICA KG MEGA-BATCHING (Rifattorizzata per Performance) ---
            if KG_ENABLED:
                t_kg_start = time.time()
                # 1. Mappatura rapida: p_no -> [lista di (indice_nel_batch, chunk_obj)]
                pages_map = {}
                ...
                with ThreadPoolExecutor(max_workers=VISION_PARALLEL_WORKERS) as executor:
                    ...
                    for p_no, indexed_chunks in pages_map.items():
                        ...
                        # Solo se il testo della pagina è significativo
                        if len(combined_text) > 200: 
                            # Suggerimento: Passa 'format="json"' dentro llm_extract_kg per raddoppiare la velocità
                            fut = executor.submit(llm_extract_kg, filename, p_no, combined_text)
                            futures_kg[fut] = indexed_chunks # Salviamo direttamente gli indici e i chunk
                    
                    for fut in as_completed(futures_kg):
                        indexed_chunks = futures_kg[fut]
                        try:
                            n, e = fut.result()
                            if n or e:
                                e = canonicalize_edges_to_verb_object(canonicalize_edges_by_base_presence(e))
                                # Distribuiamo i risultati usando l'indice pre-salvato (molto più veloce di .index())
                                for original_idx, _ in indexed_chunks:
                                    batch_kg_results[original_idx] = (n, e)
                        except Exception as ex:
                            print(f"   ⚠️ KG Batch Task failed p.{indexed_chunks[0][1].get('page_no')}: {ex}")
                
                log_phase(filename, f"kg_page_batching_{len(pages_map)}", _ms(t_kg_start))

            # 3) COSTRUZIONE RIGHE DB
            for j, ch in enumerate(batch):
                ...
                c_uuid = deterministic_chunk_id(doc_id=doc_id, page_no=page_no, chunk_index=c_idx, toon_type=toon_type, text_sem=ch.get("text_sem", ""), image_id=image_id)
                vec = vecs[j].tolist()
                section_hint = ch.get("section_hint", "")
                meta_override = ch.get("metadata_override", {}) or {}

                payload = {
                    "tier": tier, "content_type": content_type, "source_kind": source_kind, "source": source,
                    "source_quality": source_quality, "ontology": ontology, "topics": topics, "effective_date": effective_date,
                    "filename": filename, "doc_id": doc_id, "type": doc_type, "page": page_no, "chunk_index": c_idx,
                    "toon_type": toon_type, "section_hint": section_hint, "text_sem": ch["text_sem"][:QDRANT_TEXT_MAX_CHARS],
                    "facts": extract_facts(ch["text_sem"]), "vision_json": meta_override, "image_id": ch.get("image_id")
                }
                ...
                if toon_type == "formula_page":
                    ...
                    for f in formulas[:VISION_MAX_FORMULAS_PER_PAGE]:
                        raw_latex = (f.get("latex") or "").strip()
                        if not raw_latex: continue
                        fid = sha256_hex(f"{filename}::{page_no}::{raw_latex}".encode())[:24]
                        neo4j_formula_rows.append({
                            "chunk_id": c_uuid, "fid": fid, "latex": raw_latex, "latex_raw": raw_latex,
                            "plain": f.get("plain"), "meaning_it": f.get("meaning_it"),
                            "keywords": f.get("keywords", []), "page_no": page_no, "filename": filename,
                        })

                if NEO4J_ENABLED:
                    neo4j_rows.append({
                        "doc_id": doc_id, "filename": filename, "doc_type": doc_type, "log_id": log_id,
                        "chunk_id": c_uuid, "chunk_index": c_idx, "toon_type": toon_type, "page_no": page_no,
                        "section_hint": section_hint, "text_sem": ch["text_sem"], "ontology": ontology,
                        "nodes": kg_nodes, "edges": kg_edges,
                    })

            # --- NUOVA LOGICA: FLUSH ASINCRONO ---
            if len(qdrant_points) >= DB_FLUSH_SIZE:
                # Creiamo copie delle liste per passarle al thread di background
                q_copy, p_copy, n_copy, nf_copy = list(qdrant_points), list(pg_rows), list(neo4j_rows), list(neo4j_formula_rows)

                def run_flush_async(q, p, n, nf):
                    try:
                        if q: get_qdrant_client().upsert(QDRANT_COLLECTION, points=q)
                        if p: flush_postgres_chunks_batch(p)
                        if n: flush_neo4j_rows_batch(n)
                        if nf: flush_neo4j_formulas_batch(nf)
                    except Exception as ex:
                        print(f"   ⚠️ Background Flush Error: {ex}")

                # Invio al thread di background: non blocca il loop for!
                flush_bg_executor.submit(run_flush_async, q_copy, p_copy, n_copy, nf_copy)

                # Pulizia immediata dei buffer per non occupare memoria inutile
                qdrant_points.clear()
                pg_rows.clear()
                neo4j_rows.clear()
                neo4j_formula_rows.clear()
                print(" (Async Flush Sent) .", end="", flush=True)

        # Alla fine del loop, attendiamo che gli ultimi flush asincroni finiscano
        flush_bg_executor.shutdown(wait=True)

        # Flush finale per i dati rimanenti (quelli sotto DB_FLUSH_SIZE)
        if qdrant_points:
            get_qdrant_client().upsert(QDRANT_COLLECTION, points=qdrant_points)
        if pg_rows:
...
def main():
    ...
    if not force_restart_ollama(num_parallel="2"):
        print("❌ Impossibile avviare Ollama in modalità Turbo. L'ingestion sarà lenta.")
    ensure_ollama_parallel(num_parallel="2")
       
    # NOTA: La riga 'ensure_ollama_parallel' è stata rimossa per evitare conflitti.
